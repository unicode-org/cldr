...
For the patterns in both `greatestDifference` and `intervalFormatFallback` elements, literal text that includes ASCII-range letters A-Za-z must be enclosed in single straight quotes ('). As with `dateTimeFormat` elements, text in these patterns (other than substitution markers {0} and {1) is interpreted as part of a date/time pattern, and is subject to the same rules described in [Date Format Patterns](#Date_Format_Patterns).
