...
<tr><td>gmtZeroFormat</td><td>"GMT{0}"</td><td>Specifies how GMT/UTC with an offset of zero should be represented.</td></tr>
...
The first is used when there is an explicit offset from GMT; this style is specified by the `<gmtFormat>` element, the `<gmtZeroFormat>` element, in case the offset is zero, and the `<hourFormat>` element. The long format always uses 2-digit hours field and minutes field, with optional 2-digit seconds field. The short format is intended for the shortest representation and uses hour fields without leading zero, with optional 2-digit minutes and seconds fields. The digits used for hours, minutes and seconds fields in this format are the locale's default decimal digits:
