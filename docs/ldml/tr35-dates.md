...
> **Note:** In LDML 48, week-of-year calculation will change to always apply week numbers based on the ISO-8601 convention where week 1 is the week with the first Thursday of the year, ignoring the `minDays` and `firstDay` variables for the purpose of week-of-year calculations.

> **Note:** For week-of-year calculations, Gregorian years may have 52 or 53 weeks. Implementations that parse dates using week-of-year formats should be prepared to handle such cases. For example, when parsing a date in week 53 of a year that is computed to have only 52 weeks, that date should be treated as in the first week of the following year.
