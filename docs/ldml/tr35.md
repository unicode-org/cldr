...
| `var`          | <pre>= '$' \[:ID_Start:\] \[:ID_Continue:\]\*</pre>                 | $a, $elt5  (optional support)            |
...
2. Quoted characters in strings must resolve to exactly one character. That is, **\[\{\x{61\}\}\]** is valid and equivalent to **\[\{a\}\]**, but **\[\{\\x\{61 62\}\}\]** is invalid and in particular not equivalent to **\[\\x\{61 62\}\]**, which is equivalent to **\[\{ab\}\]**.
3. Ranges (**\[X-Y\]**) are only supported in the case that X and Y resolve to single code points. That is, **\[a-b\]** and **\[\{a\}-\{b\}\]** are supported, while **\[a-{bz}\]** and **\[\{ax\}-\{bz\}\]** are not.
...
4. If an unescaped '\$' is neither followed by a character of type \[:ID_Start:\] nor a '\]', it is a syntax error.
