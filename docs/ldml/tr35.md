...
The `cf` key ID defines a type of calendar. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ca" in bcp47/[calendar.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/calendar.xml).
...
An `mu` key ID defines an override for measurement unit preference. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="mu" in bcp47/[measure.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/measure.xml). *For information about preferred units and unit conversion, see [Unit Conversion](tr35-info.html#Unit_Conversion) and [Unit Preferences](tr35-info.html#Unit_Preferences).*
...
An `nu` key ID defines a type of number system. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of bcp47/[number.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/number.xml).
...
An `sd` key ID uses a [unicode\_subdivision\_id](#unicode_subdivision_id) to represent the particular usage of a language in a part of a region, such as *English as used in Scotland*. Well-formed values match [`uvalue`](#uvalue). The valid values are based on the *subdivisionContainment* element as described in *Section [3.6.5 Subdivision Codes](#Unicode_Subdivision_Codes)*, and are: a [unicode\_region\_subtag](#unicode_region_subtag) concatenated with a unicode\_subdivision\_suffix.
...
An `ss` key ID defines a set of data to be used for suppressing certain sentence breaks that would otherwise be found by UAX \#14 rules. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ss" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).
...
A `tz` key ID defines a timezone. Well-formed values match [`uvalue`](#uvalue). The valid values are those name attribute values in the *type* elements of bcp47/[timezone.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/timezone.xml). These are short identifiers defined in terms of a TZ time zone database \[[Olson](#Olson)\] identifier in the common/bcp47/timezone.xml file, plus a few extra values. Note that CLDR provides data for normalizing timezone codes.
...
A `va` key ID defines a special variant used for locales. Well-formed values match [`uvalue`](#uvalue). The valid values are those name attribute values in the *type* elements of bcp47/[variant.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/variant.xml).
