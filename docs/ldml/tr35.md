...
2. **\[a^\]** is a syntax error.
2. **\[\^a\]** is the set of 'a' and '^'.
1. **\[:\]** is a syntax error.
1. **\[ :\]** is the set of ':'.
1. **\[L:\]** is the set of 'L' and ':'.
1. **\[:L\]** is a syntax error.
1. **\[:L:\]** is the set of all code points with Unicode property 'General_Category' equal to 'Letter'.
1. **\[-\]** is the set of '-'.
1. **\[  - \]** is the set of '-'.
1. **\[a-\]** is the set of 'a' and '-'.
1. **\[-b\]** is the set of 'b' and '-'.
1. **\[a -b\]** is the set of all code points between 'a' and 'b' (inclusive).
1. **\[^  - \]** is the set of all Unicode code points except '-'.
1. **\[ ^  - \]** is a syntax error.
1. **\[\$\]** is the set of U+FFFF.
1. **\[ \$  \]** is the set of U+FFFF.
1. **\[\$ a\]** is a syntax error.
1. **\[\$a\]** is the set corresponding to the value of the variable '\$a', which could be either a full UnicodeSet, a string, or a char.
1. **\[\$a\$\]** is the set of U+FFFF and the value of the variable '\$a', which could be either a full UnicodeSet, a string, or a char.
1. **\[a\$\]** is the set of 'a' and U+FFFF.
1. **\[\}\]** is the set of '\}'.
1. **\[\{\}\]** is the set of the empty string, ''.
1. **\[\{\]** is a syntax error.
1. **\[\\x\{10FFFE10FFFF\}\]** is a syntax error.
1. **\[\\x\{10FFFE 10FFFF\}\]** is the set of the two-code-point string '\u10FFFE\u10FFFF'. <!-- TODO: Multi escape semantics? -->
