...
* [Unicode Language and Locale Identifiers](#Unicode_Language_and_Locale_Identifiers)
  ...
  * [Unicode BCP 47 U Extension](#u_Extension)
    * [Key And Type Definitions](#Key_And_Type_Definitions)
      * [Unicode Calendar Identifier](#unicode-calendar-identifier)
      * [Unicode Currency Format Identifier](#unicode-currency-format-identifier)
      * [Unicode Collation Identifier](#unicode-collation-identifier)
      * [Unicode Currency Identifier](#unicode-currency-identifier)
      * [Unicode Dictionary Break Exclusion Identifier](#unicode-dictionary-break-exclusion-identifier)
      * [Unicode Emoji Presentation Style Identifier](#unicode-emoji-presentation-style-identifier)
      * [Unicode First Day Identifier](#unicode-first-day-identifier)
      * [Unicode Hour Cycle Identifier](#unicode-hour-cycle-identifier)
      * [Unicode Line Break Style Identifier](#unicode-line-break-style-identifier)
      * [Unicode Line Break Word Identifier](#unicode-line-break-word-identifier)
      * [Unicode Measurement System Identifier](#unicode-measurement-system-identifier)
      * [Measurement Unit Preference Override](#measurement-unit-preference-override)
      * [Unicode Number System Identifier](#unicode-number-system-identifier)
      * [Region Override](#region-override)
      * [Unicode Subdivision Identifier](#unicode-subdivision-identifier)
      * [Unicode Sentence Break Suppressions Identifier](#unicode-sentence-break-suppressions-identifier)
      * [Unicode Timezone Identifier](#unicode-timezone-identifier)
      * [Unicode Variant Identifier](#unicode-variant-identifier)
...
<a name="Key_Type_Definitions"></a>
<a name="key-type-definitions"></a>
<a name="Key_And_Type_Definitions_"></a>
#### <a name="Key_And_Type_Definitions" href="#Key_And_Type_Definitions">Key And Type Definitions</a>
...
In the following subsections, key IDs and example type IDs may be followed by a parenthesized “backwards compatibility” alias.
...
…
			...
			<type key="calendar" type="persian" scope="core">Persies</type>
```

However, *all** of the following keys' types have localized names _constructed_ according to the descriptions below:
...
Constructed values are used for **some** of the following keys’ types, while other type values are explicit.

* nu (numbers)  
  * Uses script names where the key value is a [`unicode_script_subtag`](#unicode_script_subtag)
  * Others are expllcit  
* kr (Collation reorder codes)  
  * Uses script names in place of `<type name="REORDER_CODE"…>`  
  * Others are explicit  
    
For keys with values "true" or "false", there is a display name type for `typeValues`, which is generally the equivalent of ‘on’ or ‘off’, for use in menus. When constructing locale names, the binary values can be omitted. For example:
...
<a name='UnicodeCalendarIdentifier'></a>
##### Unicode Calendar Identifier 

The `ca` key ID defines a type of calendar. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ca" in bcp47/[calendar.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/calendar.xml).

This selects calendar-specific data within a locale used for formatting and parsing, such as date/time symbols and patterns; it also selects supplemental calendarData used for calendrical calculations. The value can affect the computation of the first day of the week: see [First Day Overrides](tr35-dates.md#first-day-overrides). (For information on the calendar algorithms associated with the data used with these, see \[Calendars\].)
...
<a name='UnicodeCurrencyFormatIdentifier'></a>
##### Unicode Currency Format Identifier

The `cf` key ID defines a style for currency formatting. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="cf" in bcp47/[currency.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/currency.xml).
...
<a name='UnicodeCollationIdentifier'></a>
##### Unicode Collation Identifier

The `co` key ID (alias `collation`) defines a type of collation (sort order). Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of bcp47/[collation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/collation.xml). *For information on each collation setting parameter, from **ka** to **vt**, see [Setting Options](tr35-collation.md#Setting_Options)*

The `standard` and `search` types are general — other [`ufield`](#ufield)s provide additional choices for certain locales; *they only have effect in certain locales.*'

There key IDs `ka`, `kb`, `kc`, `kf`, `kk`, `kn`, `kr`, `ks`, and `kv` are fine-grained controls for collation. They are described in the `common/bcp47/collation.xml` file.

| example type ID | type description |
| :---- | :---- |
| `standard` | The default ordering for each language. For root it is based on the \[[DUCET](#DUCET)\] (Default Unicode Collation Element Table): see [*Root Collation*](tr35-collation.md#Root_Collation). Each other locale is based on that, except for appropriate modifications to certain characters for that language. |
...
<a name='UnicodeCurrencyIdentifier'></a>
##### Unicode Currency Identifier

The `cu` key ID (alias `currency`) defines a type of currency. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="cu" in bcp47/[currency.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/currency.xml).

| example type ID | type description |
| :---- | :---- |
| *ISO 4217 code, plus others in common use* | Well-formed codes are of the form `[A-Za-z]{3}`, with the canonical format being `[A-Z]{3}`. The valid codes are ones that are or have been valid in ISO 4217, plus certain additional codes that are or have been in common use. [Supplemental Currency Data](tr35-numbers.md#Supplemental_Currency_Data) provides the list of countries (regions) and time periods associated with each currency code. It also supplies the default number of decimals. The XXX code is given a broader interpretation than in ISO 4217, as *Unknown or Invalid Currency*. |

<a name='UnicodeDictionaryBreakExclusionIdentifier'></a>
##### Unicode Dictionary Break Exclusion Identifier

The `dx` key ID specifies scripts to be excluded from dictionary-based text break (for words and lines). Well-formed values match [`uvalue`](#uvalue). The valid values are of one or more items of type SCRIPT\_CODE as specified in the *name* attribute value in the *type* element of key name="dx" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).

This affects break iteration regardless of locale.

| example type ID | type description |
| :---- | :---- |
| *[`unicode_script_subtag`](#unicode_script_subtag) values* | One or more items of type SCRIPT\_CODE (as usual, separated by hyphens), which are valid [`unicode_script_subtag`](#unicode_script_subtag) values. Each of the values for the DX key must be a short script property value in the UCD, or one of the compound script values like jpan. The compound script values are expanded when interpreted, eg, \-dx-jpan \= \-dx-hani-hira-kata<br/>The values may be in any order, eg, \-dx-thai-hani \= dx-hani-thai. However, the canonical order for the bcp47 subtag is alphabetical, eg, dx-hani-thai<br/>Dictionary-based break iterators will ignore each character whose Script\_Extension value set intersects with the DX value set.<br/>The code Zyyy (Common) can be specified to exclude all scripts, if and only if it is the only SCRIPT\_CODE value specified. If it is not the only script code, Zyyy has the normal meaning: excluding Script\_Extension=Common. |

<a name='UnicodeEmojiPresentationStyleIdentifier'></a>
##### Unicode Emoji Presentation Style Identifier

The `em` key ID specifies a request for the preferred emoji presentation style. This can be used as part of the value for an HTML lang attribute, for example `<html lang="sr-Latn-u-em-emoji">`. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="em" in bcp47/[variant.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/variant.xml).
...
<a name='UnicodeFirstDayIdentifier'></a>
##### Unicode First Day Identifier
	
The `fw` key ID defines the preferred first day of the week for calendar display. Specifying "fw" in a locale identifier overrides the default value specified by supplemental week data for the region (see Part 4 Dates, [Week Data](tr35-dates.md#Week_Data)). Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="fw" in bcp47/[calendar.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/calendar.xml). The value can affect the computation of the first day of the week: see [First Day Overrides](tr35-dates.md#first-day-overrides).
...
<a name='UnicodeHourCycleIdentifier'></a>
##### Unicode Hour Cycle Identifier
	
The `hc` key ID defines the preferred time cycle. Specifying "hc" in a locale identifier overrides the default value specified by supplemental time data for the region (see Part 4 Dates, [Time Data](tr35-dates.md#Time_Data)). Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="hc" in bcp47/[calendar.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/calendar.xml).
...
| `c12` | **Technical Preview:** Locale-preferred 12-hour cycle; resolves to either h11 or h12. First, select the supplemental time data hours element according to the [Region Override](#RegionOverride); if that keyword is not present, use the [Unicode Region Subtag](#unicode_region_subtag); if neither is present, or if the region does not have an entry in supplemental time data, use the region 001\. Then, iterate through the allowed list of hour cycle symbols in preference order. If there is an entry with symbol K before an entry with symbol h, use h11; otherwise, use h12. |
...
<a name='UnicodeLineBreakStyleIdentifier'></a>
##### Unicode Line Break Style Identifier

The `lb` key ID defines a preferred line break style corresponding to the CSS level 3 [line-break option](https://drafts.csswg.org/css-text/#line-break-property). Specifying "lb" in a locale identifier overrides the locale’s default style (which may correspond to "normal" or "strict"). Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="lb" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).
...
<a name='UnicodeMeasurementSystemIdentifier'></a>
##### Unicode Measurement System Identifier

The `ms` key ID defines a preferred measurement system. Specifying "ms" in a locale identifier overrides the default value specified by supplemental measurement system data for the region (see Part 2 General, [Measurement System Data](tr35-general.md#Measurement_System_Data)). Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ms" in bcp47/[measure.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/measure.xml). 

The determination of preferred units depends on the locale identifer: the keys `ms`, `mu`, `rg`, the base locale (language, script, region) and the user preferences. *For information about preferred units and unit conversion, see [Unit Conversion](tr35-info.md#Unit_Conversion) and [Unit Preferences](tr35-info.md#Unit_Preferences).*
...
<a name='MeasurementUnitPreferenceOverride'></a>
##### Measurement Unit Preference Override

The `mu` key ID defines an override for measurement unit preference. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="mu" in bcp47/[measure.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/measure.xml). *For information about preferred units and unit conversion, see [Unit Conversion](tr35-info.md#Unit_Conversion) and [Unit Preferences](tr35-info.md#Unit_Preferences).*
...
<a name='RegionOverride'></a>
##### Region Override

The `rg` key ID specifies an alternate region to use for obtaining certain region-specific default values (those specified by the [\<rgScope\>](tr35-info.md#rgScope) element), instead of using the region specified by the [unicode\_region\_subtag](#unicode_region_subtag) in the Unicode Language Identifier (or inferred from the [unicode\_language\_subtag](#unicode_language_subtag)).

The valid values are a [unicode\_subdivision\_id](#unicode_subdivision_id) of type “unknown” or “regular”; this consists of a [unicode\_region\_subtag](#unicode_region_subtag) for a regular region (not a macroregion), suffixed either by “zzzz” (case is not significant) to designate the region as a whole, or by a unicode\_subdivision\_suffix to provide more specificity. (Usage of anything other than zzzz would be unusual, though.)
...
The determination of preferred units depends on the locale identifier: the keys ms, mu, rg, the base locale (language, script, region) and the user preferences. The value can affect the computation of the first day of the week: see [First Day Overrides](tr35-dates.md#first-day-overrides). *For information about preferred units and unit conversion, see [Unit Conversion](tr35-info.md#Unit_Conversion) and [Unit Preferences](tr35-info.md#Unit_Preferences).*

| example type | type description |
| :---- | :---- |
| `uszzzz` | The US region as a whole |
| `usca` | The state of California |

<a name='UnicodeSubdivisionIdentifier'></a>
<a name='unicode_subdivision_subtag_validity'></a>
<a name='unicode-subdivision-subtag-validity'></a>
##### Unicode Subdivision Identifier

The `sd` key ID uses a [unicode\_subdivision\_id](#unicode_subdivision_id) to represent the particular usage of a language in a part of a region, such as *English as used in Scotland*. Well-formed values match [`uvalue`](#uvalue). The valid values are based on the *subdivisionContainment* element as described in *Section [3.6.5 Subdivision Codes](#Unicode_Subdivision_Codes)*, and are: a [unicode\_region\_subtag](#unicode_region_subtag) concatenated with a unicode\_subdivision\_suffix.

Both `en-u-sd-usca` and `en-US-u-sd-usca` represent *English as used in California*.

See [***3.6.5 Subdivision Codes***](#Unicode_Subdivision_Codes). The value can affect the computation of the first day of the week: see [First Day Overrides](tr35-dates.md#first-day-overrides).
...
<a name='UnicodeSentenceBreakSuppressionsIdentifier'></a>
##### Unicode Sentence Break Suppressions Identifier

The `ss` key ID defines a set of data to be used for suppressing certain sentence breaks that would otherwise be found by UAX \#14 rules. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ss" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).
...
<a name='UnicodeTimezoneIdentifier'></a>
##### Unicode Timezone Identifier

The `tz` key ID defines a timezone. Well-formed values match [`uvalue`](#uvalue). The valid values are those name attribute values in the *type* elements of bcp47/[timezone.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/timezone.xml). These are short identifiers defined in terms of a TZ time zone database \[[Olson](#Olson)\] identifier in the common/bcp47/timezone.xml file, plus a few extra values. Note that CLDR provides data for normalizing timezone codes.

For more information, including syntactic constraints, see [Time Zone Identifiers](#Time_Zone_Identifiers).

| example type ID | type description |
| :---- | :---- |
| `adalv` | The TZDB code `Europe/Andorra` |

<a name='UnicodeVariantIdentifier'></a>
##### Unicode Variant Identifier

The `va` key ID defines a special variant used for locales. Well-formed values match [`uvalue`](#uvalue). The valid values are those name attribute values in the *type* elements of bcp47/[variant.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/variant.xml).

| example type ID | type description |
| :---- | :---- |
| `posix` | POSIX style locale variant. About handling of the "POSIX" variant see [*Legacy Variants*](#Legacy_Variants). |
...
The localization of the [tlang](#tlang) locale is the same as the localization of the corresponding Unicode locale ID. 
The keys have localized names in Modern coverage, but at this point most of the types do not.
No localized names can be supplied by CLDR for x0 contents, because the usage is defined outside of CLDR.
