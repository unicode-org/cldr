...
A X key ID defines a type of number system. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of bcp47/[number.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/number.xml).
...
An x key ID defines a set of data to be used for suppressing certain sentence breaks that would otherwise be found by UAX \#14 rules. Well-formed values match [`uvalue`](#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ss" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).
