...
Implementations that support variables must require the value of variables to be known at UnicodeSet parse time.
...
If the values of variables were unknown at parse time, this would introduce ambiguities when parsing, thus the values must be known.
For example, it would be unclear whether **[$a-$b]** is a range of strings/characters, or a set difference, because that depends on the values of the variables.
