...
<a name="Key_Type_Definitions">
<a name="key-type-definitions">
#### <a name="Key_And_Type_Definitions_" href="#Key_And_Type_Definitions_">Key And Type Definitions</a>

This section contains the set of U extension key values that are currently available, with a description or sampling of the U extension type values. Each category is associated with an XML file in the bcp47 directory.
...
In the following subsections, key IDs and  example type IDs  may be followed by a parenthesized “backwards compatibility” ID.

Localized names are provided for most of the key and type IDs below, such as:

```
<type key="calendar" type="persian">Persiese kalender</type>
```

However, constructed type names are used for **all** of the following keys' types

* cu (currencies; use currency names)  
* fw (uses calendar data for stand-alone week dates)  
* mu (uses unit display names)  
* sd (subdivisions, names outside the scope of CLDR)  
  * Marked with `<type name="SUBDIVISION_CODE"…>`  
* dx (Scripts to exclude from dictionary break; uses script names)  
  * Marked with `<type name="SCRIPT_CODE"…>`  
* rg (uses region/territory names)  
  * Marked with `<type name="RG_KEY_VALUE"…>`

Constructed type names are used for **some** of the following keys’ types, while other types are explicit.

* nu (numbers)  
  * Uses script names where the key value is /\[a-z\]{4}/  
  * Others are expllcit  
* kr (Collation reorder codes)  
  * Uses script names in place of `<type name="REORDER_CODE"…>`  
  * Others are explicit  
* x0 (`<type name="PRIVATE_USE">`)  
  * names outside the scope of CLDR; up to cooperating systems
    
For keys with values "true" or "false", there is a display name type for `typeValues`, which is generally the equivalent of ‘on’ or ‘off’, for use in menus. When constructing locale names, the binary values can be omitted.

```
		<typeValues>
			<typeValue type="no">Af</typeValue>
			<typeValue type="yes">Aan</typeValue>
		</typeValues>
```

##### [**Unicode Calendar Identifier**](https://www.unicode.org/reports/tr35/#UnicodeCalendarIdentifier) 

The `cf` key ID defines a type of calendar. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ca" in bcp47/[calendar.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/calendar.xml).

This selects calendar-specific data within a locale used for formatting and parsing, such as date/time symbols and patterns; it also selects supplemental calendarData used for calendrical calculations. The value can affect the computation of the first day of the week: see [First Day Overrides](https://www.unicode.org/reports/tr35/tr35-dates.html#first-day-overrides). (For information on the calendar algorithms associated with the data used with these, see \[Calendars\].)
...
| example type ID | type description |
| :---- | :---- |
...
##### [**Unicode Currency Format Identifier**](https://www.unicode.org/reports/tr35/#UnicodeCurrencyFormatIdentifier)

The `cf` key ID defines a style for currency formatting. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="cf" in bcp47/[currency.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/currency.xml).

This selects the specific type of currency formatting pattern within a locale.

| example type ID | type description |
| :---- | :---- |
| `standard` | Negative numbers use the minusSign symbol (the default). |
| `account` | Negative numbers use parentheses or equivalent. |

##### [**Unicode Collation Identifier**](https://www.unicode.org/reports/tr35/#UnicodeCollationIdentifier)

The `co` key ID (collation) defines a type of collation (sort order). Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of bcp47/[collation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/collation.xml). *For information on each collation setting parameter, from **ka** to **vt**, see [Setting Options](https://www.unicode.org/reports/tr35/tr35-collation.html#Setting_Options)*

The `standard` and `search` types are general : other ufields provide additional choices for certain locales; *they only have effect in certain locales.*

| example type ID | type description |
| :---- | :---- |
| `standard` | The default ordering for each language. For root it is based on the \[[DUCET](https://www.unicode.org/reports/tr35/#DUCET)\] (Default Unicode Collation Element Table): see [*Root Collation*](https://www.unicode.org/reports/tr35/tr35-collation.html#Root_Collation). Each other locale is based on that, except for appropriate modifications to certain characters for that language. |
| `search` | A special collation type dedicated for string search — it is not used to determine the relative order of two strings, but only to determine whether they should be considered equivalent for the specified strength, using the string search matching rules appropriate for the language. Compared to the normal collator for the language, this may add or remove primary equivalences, may make additional characters ignorable or change secondary equivalences, and may modify contractions to allow matching within them, depending on the desired behavior. For example, in Czech, the distinction between ‘a’ and ‘á’ is secondary for normal collation, but primary for search; a search for ‘a’ should never match ‘á’ and vice versa. A search collator is normally used with strength set to PRIMARY or SECONDARY (should be SECONDARY if using “asymmetric” search as described in the \[[UCA](https://www.unicode.org/reports/tr41/#UTS10)\] section Asymmetric Search). The search collator in root supplies matching rules that are appropriate for most languages (and which are different than the root collation behavior); language-specific search collators may be provided to override the matching rules for a given language as necessary. |
...
| `searchjl` | Special collation type for a modified string search in which a pattern consisting of a sequence of Hangul initial consonants (jamo lead consonants) will match a sequence of Hangul syllable characters whose initial consonants match the pattern. The jamo lead consonants can be represented using conjoining or compatibility jamo. This search collator is best used at SECONDARY strength with an "asymmetric" search as described in the \[[UCA](https://www.unicode.org/reports/tr41/#UTS10)\] section Asymmetric Search and obtained, for example, using ICU4C's usearch facility with attribute USEARCH\_ELEMENT\_COMPARISON set to value USEARCH\_PATTERN\_BASE\_WEIGHT\_IS\_WILDCARD; this ensures that a full Hangul syllable in the search pattern will only match the same syllable in the searched text (instead of matching any syllable with the same initial consonant), while a Hangul initial consonant in the search pattern will match any Hangul syllable in the searched text with the same initial consonant. |

##### [**Unicode Currency Identifier**](https://www.unicode.org/reports/tr35/#UnicodeCurrencyIdentifier)

The `cu` key ID (currency) defines a type of currency. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="cu" in bcp47/[currency.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/currency.xml).

| example type ID | type description |
| :---- | :---- |
| *ISO 4217 code, plus others in common use* | Well-formed codes are of the form `[A-Za-z]{3}`, with the canonical format being `[A-Z]{3}`. The valid codes are ones that are or have been valid in ISO 4217, plus certain additional codes that are or have been in common use. [Supplemental Currency Data](https://www.unicode.org/reports/tr35/tr35-numbers.html#Supplemental_Currency_Data) provides the list of countries (regions) and time periods associated with each currency code. It also supplies the default number of decimals. The XXX code is given a broader interpretation than in ISO 4217, as *Unknown or Invalid Currency*. |

##### [**Unicode Dictionary Break Exclusion Identifier**](https://www.unicode.org/reports/tr35/#UnicodeDictionaryBreakExclusionIdentifier)

The `dx` key ID specifies scripts to be excluded from dictionary-based text break (for words and lines). Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are of one or more items of type SCRIPT\_CODE as specified in the *name* attribute value in the *type* element of key name="dx" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).

This affects break iteration regardless of locale.

| example type ID | type description |
| :---- | :---- |
| [*`unicode_script_subtag`*](https://www.unicode.org/reports/tr35/#unicode_script_subtag) values* | One or more items of type SCRIPT\_CODE (as usual, separated by hyphens), which are valid [`unicode_script_subtag`](https://www.unicode.org/reports/tr35/#unicode_script_subtag) values. Each of the values for the DX key must be a short script property value in the UCD, or one of the compound script values like jpan. The compound script values are expanded when interpreted, eg, \-dx-jpan \= \-dx-hani-hira-kata The values may be in any order, eg, \-dx-thai-hani \= dx-hani-thai. However, the canonical order for the bcp47 subtag is alphabetical, eg, dx-hani-thai Dictionary-based break iterators will ignore each character whose Script\_Extension value set intersects with the DX value set. The code Zyyy (Common) can be specified to exclude all scripts, if and only if it is the only SCRIPT\_CODE value specified. If it is not the only script code, Zyyy has the normal meaning: excluding Script\_Extension=Common. |

##### [**Unicode Emoji Presentation Style Identifier**](https://www.unicode.org/reports/tr35/#UnicodeEmojiPresentationStyleIdentifier)

The `em` key ID **specifies a request for the preferred emoji presentation style. This can be used as part of the value for an HTML lang attribute, for example `<html lang="sr-Latn-u-em-emoji">`. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="em" in bcp47/[variant.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/variant.xml)**.

| example type ID | type description |
| :---- | :---- |
| `emoji` | Use an emoji presentation for emoji characters if possible. |
| `text` | Use a text presentation for emoji characters if possible. |
| `default` | Use the default presentation for emoji characters as specified in UTR \#51 [Presentation Style](https://www.unicode.org/reports/tr51/#Presentation_Style). |

##### [**Unicode First Day Identifier**](https://www.unicode.org/reports/tr35/#UnicodeFirstDayIdentifier)

An `fw` key ID **defines the preferred first day of the week for calendar display. Specifying "fw" in a locale identifier overrides the default value specified by supplemental week data for the region (see Part 4 Dates, [Week Data](https://www.unicode.org/reports/tr35/tr35-dates.html#Week_Data)). Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="fw" in bcp47/[calendar.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/calendar.xml). The value can affect the computation of the first day of the week: see [First Day Overrides](https://www.unicode.org/reports/tr35/tr35-dates.html#first-day-overrides).**
...
##### [**Unicode Hour Cycle Identifier**](https://www.unicode.org/reports/tr35/#UnicodeHourCycleIdentifier)

An `hc` key ID defines the preferred time cycle. Specifying "hc" in a locale identifier overrides the default value specified by supplemental time data for the region (see Part 4 Dates, [Time Data](https://www.unicode.org/reports/tr35/tr35-dates.html#Time_Data)). Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="hc" in bcp47/[calendar.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/calendar.xml).
...
| `c12` | **Technical Preview:** Locale-preferred 12-hour cycle; resolves to either h11 or h12. First, select the supplemental time data hours element according to the [Region Override](https://www.unicode.org/reports/tr35/#RegionOverride); if that keyword is not present, use the [Unicode Region Subtag](https://www.unicode.org/reports/tr35/#unicode_region_subtag); if neither is present, or if the region does not have an entry in supplemental time data, use the region 001\. Then, iterate through the allowed list of hour cycle symbols in preference order. If there is an entry with symbol K before an entry with symbol h, use h11; otherwise, use h12. |
| `c24` | **Technical Preview:** Locale-preferred 24-hour cycle; resolves to either h23 or h24. First, select the supplemental time data hours element as above. Then, iterate through the allowed list of hour cycle symbols in preference order. If there is an entry with symbol k before an entry with symbol H, use h24; otherwise, use h23. |

##### [**Unicode Line Break Style Identifier**](https://www.unicode.org/reports/tr35/#UnicodeLineBreakStyleIdentifier)

An `lb` key ID **defines a preferred line break style corresponding to the CSS level 3 [line-break option](https://drafts.csswg.org/css-text/#line-break-property). Specifying "lb" in a locale identifier overrides the locale’s default style (which may correspond to "normal" or "strict"). Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="lb" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml)**.
...
##### [**Unicode Line Break Word Identifier**](https://www.unicode.org/reports/tr35/#UnicodeLineBreakWordIdentifier)

An `lw` key ID defines preferred line break word handling behavior corresponding to the CSS level 3 [word-break option](https://drafts.csswg.org/css-text/#word-break-property). Specifying "lw" in a locale identifier overrides the locale’s default style (which may correspond to "normal" or "keepall"). Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="lw" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).

| example type ID | type description |
| :---- | :---- |
| `normal` | CSS level 3 word-break=normal, normal script/language behavior for midword breaks |
...
##### [**Unicode Measurement System Identifier**](https://www.unicode.org/reports/tr35/#UnicodeMeasurementSystemIdentifier)

An `ms` key ID **defines a preferred measurement system. Specifying "ms" in a locale identifier overrides the default value specified by supplemental measurement system data for the region (see Part 2 General, [Measurement System Data](https://www.unicode.org/reports/tr35/tr35-general.html#Measurement_System_Data)). Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ms" in bcp47/[measure.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/measure.xml)**. 

The determination of preferred units depends on the locale identifer: the keys ms, mu, rg, the base locale (language, script, region) and the user preferences. *For information about preferred units and unit conversion, see [Unit Conversion](https://www.unicode.org/reports/tr35/tr35-info.html#Unit_Conversion) and [Unit Preferences](https://www.unicode.org/reports/tr35/tr35-info.html#Unit_Preferences).*
...
##### [**Measurement Unit Preference Override**](https://www.unicode.org/reports/tr35/#MeasurementUnitPreferenceOverride)

An `mu` key ID **defines an override for measurement unit preference. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="mu" in bcp47/[measure.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/measure.xml)**. *For information about preferred units and unit conversion, see [Unit Conversion](https://www.unicode.org/reports/tr35/tr35-info.html#Unit_Conversion) and [Unit Preferences](https://www.unicode.org/reports/tr35/tr35-info.html#Unit_Preferences).*

| example type ID | type description |
| :---- | :---- |
| `celsius` | Celsius as temperature unit |
| `kelvin` | Kelvin as temperature unit |
| `fahrenhe` | Fahrenheit as temperature unit |

##### [**Unicode Number System Identifier**](https://www.unicode.org/reports/tr35/#UnicodeNumberSystemIdentifier)

A X key ID **defines a type of number system. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of bcp47/[number.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/number.xml).**

| example type ID | type description |
| :---- | :---- |
| *Unicode script subtag* | Four-letter types indicating the primary numbering system for the corresponding script represented in Unicode. Unless otherwise specified, it is a decimal numbering system using digits \[:GeneralCategory=Nd:\]. For example, "latn" refers to the ASCII / Western digits 0-9, while "taml" is an algorithmic (non-decimal) numbering system. (The code "tamldec" is indicates the "modern Tamil decimal digits".) For more information, see [Numbering Systems](https://www.unicode.org/reports/tr35/tr35-numbers.html#Numbering_Systems). |
...
[**Region Override**](https://www.unicode.org/reports/tr35/#RegionOverride)

An `rg` key ID specifies an alternate region to use for obtaining certain region-specific default values (those specified by the [\<rgScope\>](https://www.unicode.org/reports/tr35/tr35-info.html#rgScope) element), instead of using the region specified by the [unicode\_region\_subtag](https://www.unicode.org/reports/tr35/#unicode_region_subtag) in the Unicode Language Identifier (or inferred from the [unicode\_language\_subtag](https://www.unicode.org/reports/tr35/#unicode_language_subtag)).

The valid values are a [unicode\_subdivision\_id](https://www.unicode.org/reports/tr35/#unicode_subdivision_id) of type “unknown” or “regular”; this consists of a [unicode\_region\_subtag](https://www.unicode.org/reports/tr35/#unicode_region_subtag) for a regular region (not a macroregion), suffixed either by “zzzz” (case is not significant) to designate the region as a whole, or by a unicode\_subdivision\_suffix to provide more specificity. (Usage of anything other than zzzz would be unusual, though.)
...
The determination of preferred units depends on the locale identifier: the keys ms, mu, rg, the base locale (language, script, region) and the user preferences. The value can affect the computation of the first day of the week: see [First Day Overrides](https://www.unicode.org/reports/tr35/tr35-dates.html#first-day-overrides). *For information about preferred units and unit conversion, see [Unit Conversion](https://www.unicode.org/reports/tr35/tr35-info.html#Unit_Conversion) and [Unit Preferences](https://www.unicode.org/reports/tr35/tr35-info.html#Unit_Preferences).*

| example type ID | type description |
| :---- | :---- |
| `uszzzz` | **The US region as a whole** |
| `usca` | **The state of California** |

[**Unicode Subdivision Identifier**](https://www.unicode.org/reports/tr35/#UnicodeSubdivisionIdentifier)

An `sd` key ID uses a [unicode\_subdivision\_id](https://www.unicode.org/reports/tr35/#unicode_subdivision_id) to represent the particular usage of a language in a part of a region, such as *English as used in Scotland*. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are based on the *subdivisionContainment* element as described in *Section [3.6.5 Subdivision Codes](https://www.unicode.org/reports/tr35/#Unicode_Subdivision_Codes)*, and are: a [unicode\_region\_subtag](https://www.unicode.org/reports/tr35/#unicode_region_subtag) concatenated with a unicode\_subdivision\_suffix.

Both `en-u-sd-usca` and `en-US-u-sd-usca` represent “English as used in California”.

See [***3.6.5 Subdivision Codes***](https://www.unicode.org/reports/tr35/#Unicode_Subdivision_Codes). The value can affect the computation of the first day of the week: see [First Day Overrides](https://www.unicode.org/reports/tr35/tr35-dates.html#first-day-overrides).

| example type ID | type description |
| :---- | :---- |
| `gbsct` | This is “gb”+“sct” (where sct represents the subdivision code for Scotland). Thus “en-GB-u-sd-gbsct” represents the language variant “English as used in Scotland”. |

##### [**Unicode Sentence Break Suppressions Identifier**](https://www.unicode.org/reports/tr35/#UnicodeSentenceBreakSuppressionsIdentifier)

An x key ID defines a set of data to be used for suppressing certain sentence breaks that would otherwise be found by UAX \#14 rules. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those *name* attribute values in the *type* elements of key name="ss" in bcp47/[segmentation.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/segmentation.xml).

| example type ID | type description |
| :---- | :---- |
| `none` | Don’t use sentence break suppressions data (the default). |
| `standard` | Use sentence break suppressions data of type "standard" |

##### [**Unicode Timezone Identifier**](https://www.unicode.org/reports/tr35/#UnicodeTimezoneIdentifier)

A `tz` key ID defines a timezone. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those name attribute values in the *type* elements of bcp47/[timezone.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/timezone.xml). These are short identifiers defined in terms of a TZ time zone database \[[Olson](https://www.unicode.org/reports/tr35/#Olson)\] identifier in the common/bcp47/timezone.xml file, plus a few extra values. Note that CLDR provides data for normalizing timezone codes.

For more information, including syntactic constraints, see [Time Zone Identifiers](https://www.unicode.org/reports/tr35/#Time_Zone_Identifiers).

| example type ID | type description |
| :---- | :---- |
| `adalv` | The TZDB code `Europe/Andorra` |

##### [**Unicode Variant Identifier**](https://www.unicode.org/reports/tr35/#UnicodeVariantIdentifier)

A `va` key ID defines a special variant used for locales. Well-formed values match [`uvalue`](https://www.unicode.org/reports/tr35/#uvalue). The valid values are those name attribute values in the *type* elements of bcp47/[variant.xml](https://github.com/unicode-org/cldr/blob/main/common/bcp47/variant.xml).

| example type ID | type description |
| :---- | :---- |
| `posix` | POSIX style locale variant. About handling of the "POSIX" variant see [*Legacy Variants*](https://www.unicode.org/reports/tr35/#Legacy_Variants). |
