...
Locale data can be _sliced_ to exclude data not needed by a particular implementation. This can be _vertical slicing_: excluding a locale and all the locales inheriting from them, or _horizontal slicing_: excluding particular types of data from all locales. For example:
...
### 7.2 <a name="Horizontal_Slicing" href="#Horizontal_Slicing">Horizontal_Slicing</a>

The Comprehensive coverage level includes data that is above the Modern coverage level. This data is over and above what is typically needed by many applications. That data can be filtered out, at considerable savings in data size. The easiest way to do that is to use the CLDR tooling to filter the data before generating the implementation's data format. That way allows the implementation to have direct access to the CoverageLevel code that can determine the coverage level, for a given locale and path. Once the data is transformed, such as to the JSON format, the CoverageLevel code is no longer accessible. For example:
