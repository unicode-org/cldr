...
A locale identifier can be constructed with the Visual variant script codes for a particular rendering.
For example, ar_Aran can be used to request that ar_Arab data be used, but with a Nastaliq-style font.
...
For example, an implmentation might not recognize "Aran" in uz-Aran at all, and return results in uz-Latn. 
