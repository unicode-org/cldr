...
Paths at the Comprehensive [coverage level](tr35-info.md#Coverage_Levels) include data that is above the Modern coverage level. This data is over and above what is typically needed by many applications. That data can be filtered out, at considerable savings in data size. The easiest way to do that is to use the CLDR Java tooling (the `cldr-code` package) to filter the data before generating the implementation's data format. That way allows the implementation to have direct access to the CoverageLevel code that can determine the coverage level, for a given locale and path. Once the data is transformed, such as to the JSON format, the CoverageLevel code is no longer accessible. For example, here is a code snippet:
