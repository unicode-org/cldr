...
There is a special value called _Inheritance Marker_ used in the main repository, which is ↑↑↑. For example:
```
    <language type="ab">↑↑↑</language>
```
It is used for data submission to indicate that the vetters have verified that the inherited value from the parent locale is correct for the current locale. For example, the above was used in de_CH to indicate that the following was not only correct for de, but also for de_CH.
```
    <language type="ab">Abchasisch</language>
```
It is not needed or used in the released data, because conformant implementations will find the parent locale's value whether the element is present with a value of ↑↑↑, or is completely absent.
