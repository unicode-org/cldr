...
| `bracketedHex` | <pre>= '\{' s hexCodePoint \(s hexCodePoint\)\* s '\}'</pre>        | \{61 2019 62\}, \{61\} <br/> *note: the whitespace between hexCodePoints is required to separate code points*                         |
...
2. Escapes that use multiple code points are equivalent to their flattened representation, i.e., `\x{61 62}` is equivalent to `\x{61}\x{62}`. These can also occur in strings, so **\[\{\\x\{ 061 62 0063\}\}\]** is equivalent to **\[\{abc\}\]**. Furthermore, whitespace is significant here and required to separate code points, i.e., `\x{6162}` is not equal to `\x{61 62}`, which is equal to `\x61\x62`.
...
32. **\[\\x\{10FFFE 10FFFF\}\]** is the set of U+10FFFE and U+10FFFF'.
