...
# <a name="Unicode_Locale_Data_Markup_Language_LDML" id="Unicode_Locale_Data_Markup_Language_LDML" href="#Unicode_Locale_Data_Markup_Language_LDML">Unicode Locale Data Markup Language (LDML)</a>
...
* **XML](https://www.w3.org/TR/REC-xml/) files can**: [XML](https://www.w3.org/TR/REC-xml/) files can have a wide variation in textual form, while representing precisely the same data. By putting the LDML files in the repository into a canonical form, this allows us to use the simple diff tools used widely (and in CVS) to detect differences when vetting changes, without those tools being confused. This is not a requirement on other uses of LDML; just simply a way to manage repository data more easily.
