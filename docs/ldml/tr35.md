...
Signalling an error can be done in various ways, depending on the most consistent approach for APIs in the module. For example:
	...
	3. return the input, but "Zzzz", and/or "ZZ" substituted for empty fields.
	4. "und"
...
1. First get max = AddLikelySubtags(inputLocale). If an error is signaled AddLikelySubtags, signal that same error and stop.
2. Remove the variants and extensions from max.
...
5. If you do not get a match, return max + variants + extensions.
...
A variant of this favors the script over the region, thus using {language, language_script, language_region} in the step #4 above. If that variant is used, then the result in this example would be zh_Hant instead of zh_TW. 
