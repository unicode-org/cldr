...
UnicodeSets may contain variables (`$my_char`, `$the_set`, ...) in place of full UnicodeSets and strings (and therefore by extension characters).

This introduces ambiguities when parsing. For example, it is unclear whether **[$a-$b]** is a range of strings/characters, or a set difference, because that depends on the values of the variables. In particular in the case of **$a-$b**, it must be that **$a** and **$b** are of the same kind, i.e., both are strings/characters or both are UnicodeSets. A variable map with any other combination (e.g., `{ a => '0', b => [:L:] }`) is incompatible with the syntax **$a-$b**. Thus, implementations that support variables may want to require the value of variables to be known at UnicodeSet parse time.
