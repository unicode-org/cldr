...
| -------------- | -------------------------------------------------------------- | --------------------------------------- |
| `root`         | <pre>= prop<br/>\| '\[' '^'? s '-'? s seq\* \[\\$ \\-\]? s '\]' <br/>\| var</pre> | \\p\{x=y\},<br/>[abc],<br/>$myset                     |
| `seq`          | <pre>= root \(s \[\\&\\\-\] s root\)\* s<br/>\| range s</pre>        | \[abc\]\-\[cde\], a                          |
| `range`        | <pre>= element \('\-' element\)?       | a, a\-c, \{abc\}, \{ax\}\-\{bz\}                          | <!-- TODO: Support for string ranges of length != 1? -->
...
| `charInString` | <pre>= \[^ \\\\ \\\} \[:Pat_WS:\]\]<br/>\| '\\' quoted</pre> | a, b, c, \\n, \{, $ <br/> *note: only quoted items that resolve to one code point are allowed* | <!-- TODO: Multi escape semantics? If flattening semantics, this note is unnecessary -->
...
| `bracketedHex` | <pre>= '\{' s hexCodePoint \(s hexCodePoint\)\* s '\}'</pre>        | \{61 2019 62\}  <br/> *note: the whitespace between hexCodePoints is required here*                         | <!-- TODO: Multi escape semantics? If so, add note explaining here or somewhere else -->
| `hexCodePoint` | <pre>= hex\{1,5\} \| '10' hex\{4\}</pre>                           |                                         |
| `hex`          | <pre>= \[0\-9A\-Fa\-f\]</pre>                                       |                                         |
| `var`          | <pre>= '$' \[:ID_Start:\] \[:ID_Continue:\]\*</pre>                 | $a, $elt5  (optional support)            | <!-- TODO: XID* or ID*? -->
...
2. Quoted characters in strings must resolve to exactly one character. That is, **\[\{\x{61\}\}\]** is valid and equivalent to **\[\{a\}\]**, but **\[\{\\x\{61 62\}\}\]** is invalid and in particular not equivalent to **\[\\x\{61 62\}\]**, which is equivalent to **\[\{ab\}\]**. <!-- TODO: Multi escape semantics? -->
3. Ranges (**\[X-Y\]**) are only supported in the case that X and Y resolve to single code points. That is, **\[a-b\]** and **\[\{a\}-\{b\}\]** are supported, while **\[a-{bz}\]** and **\[\{ax\}-\{bz\}\]** are not. <!-- TODO: Support for string ranges of length != 1? -->
...
* Out of all above syntax characters, only \\, \}, and whitespace have a special meaning inside strings (**\[\{\[a-z\]\}\]** is the string '\[a-z\]').
* \$ is equivalent to \uFFFF when appearing at the very end of a set with or without trailing whitespace (**[a-z\$]**, **[a-z\$ ]**), and used as starting indicator for a variable reference elsewhere, in which case the variable name will be the longest match on the `var` nonterminal (**[\$my_set]**).
...
In the following, a list of examples including common sources of confusion concerning the UnicodeSet syntax:
1. **\[^a\]** is the set of all Unicode code points except 'a'.
2. **\[ ^a\]** is a syntax error.
3. **\[a^\]** is a syntax error.
4. **\[\^a\]** is the set of 'a' and '^'.
5. **\[:\]** is a syntax error.
6. **\[ :\]** is the set of ':'.
7. **\[L:\]** is the set of 'L' and ':'.
8. **\[:L\]** is a syntax error.
9. **\[:L:\]** is the set of all code points with Unicode property 'General_Category' equal to 'Letter'.
10. **\[-\]** is the set of '-'.
11. **\[  - \]** is the set of '-'.
12. **\[a-\]** is the set of 'a' and '-'.
13. **\[-b\]** is the set of 'b' and '-'.
14. **\[a -b\]** is the set of all code points between 'a' and 'b' (inclusive).
15. **\[^  - \]** is the set of all Unicode code points except '-'.
16. **\[ ^  - \]** is a syntax error.
17. **\[\$\]** is the set of U+FFFF.
18. **\[ \$  \]** is the set of U+FFFF.
19. **\[\$ a\]** is a syntax error.
20. **\[\$a\]** is the set corresponding to the value of the variable '\$a', which could be either a full UnicodeSet, a string, or a char.
21. **\[\$a\$\]** is the set of U+FFFF and the value of the variable '\$a', which could be either a full UnicodeSet, a string, or a char.
22. **\[a\$\]** is the set of 'a' and U+FFFF.
23. **\[\}\]** is the set of '\}'.
24. **\[\{\}\]** is the set of the empty string, ''.
25. **\[\{\]** is a syntax error.
26. **\[\{\}\}\]** is the set of '\}' and the empty string.
27. **\[\{\{\}\]** is the set of '\}'.
28. **\[\{\$var\}\]** is the set of the string '\$var'.
29. **\[\{\[a-z\}\]** is the set of the string '\[a-z'.
30. **\[\{ \[ a - z\}\]** is the set of the string '\[a-z'.
31. **\[\\x\{10FFFE10FFFF\}\]** is a syntax error.
32. **\[\\x\{10FFFE 10FFFF\}\]** is the set of the two-code-point string '\u10FFFE\u10FFFF'. <!-- TODO: Multi escape semantics? -->
...
UnicodeSets may contain variables (`$my_char`, `$the_set`, ...) in place of full UnicodeSets and strings/characters.

This introduces ambiguities when parsing. For example, it is unclear whether **[$a-$b]** is a range of strings/characters, or a set difference, because that depends on the values of the variables.
In particular in the case of **$a-$b**, it must be that **$a** and **$b** are of the same kind, i.e., both are strings/characters or both are UnicodeSets. A variable map with any other combination (e.g., `{ a => '0', b => [:L:] }`) is incompatible with the syntax **$a-$b**, and would return an error at parse-time.
Thus, implementations that support variables may want to require the value of variables to be known at UnicodeSet parse time.
Parsing a UnicodeSet with variables support requires referenced variables to exist, in other words, parsing a UnicodeSet referring to an unknown variable leads to a parse error.
...
The variable syntax implements UAX31-R1-2 with the profile of using the ID_Start and ID_Continue properties in place of XID_Start and XID_Continue. For more information, see [[UAX31](https://www.unicode.org/reports/tr41/#UAX31)]. <!-- TODO: XID* or ID*? -->
Variables are equivalent normalized identifiers, implementing UAX31-R4. Furthermore, variables are case-sensitive.
...
4. If an unescaped '\$' is neither followed by a character of type \[:ID_Start:\] nor a '\]', it is a syntax error. <!-- TODO: XID* or ID*? -->
...
| [x\\u\{61 2019 62}y] | Equivalent to [x\\u0061\\u2019\\u0062y] (= [xa’by]) | <!-- TODO: Multi escape semantics? -->
