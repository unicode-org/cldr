...
- **Deprecation of pattern-only fields in date/time skeletons**: Pattern-only field symbols—specifically stand-alone fields (`L`, `q`, `c`) and non-standard year fields (`Y`, `u`, `r`)—as well as day period symbols (`a`, `b`, `B`) without an accompanying 12-hour field (`h`, `K`, `j`) or standalone usage, are slated for deprecation in skeletons in CLDR 50 (use `M`, `Q`, `E`/`e`, and `y`/`U` instead). While `availableFormats` and `intervalFormats` already conform to these restrictions, remaining occurrences in stock `<datetimeSkeleton>` elements will be updated in CLDR 50. [CLDR-19757][]
