...
***minimal pair*** <a name="minimal-pair"></a> - minimal pairs are used in CLDR to justify and verify the categories for plurals (both cardinal and ordinal), grammatical gender (masculine, feminine,…), and grammatical case (nominative, accusative,…). A set of minimal pairs provides one example per category, where the grammatical difference (e.g. number for cardinals) changes one or more other words. Although the term used is minimal pairs, this refers to pairwise differences; the set of minimal pairs for a given language might be as many as six examples. There are three general types of appoaches to this: noun changes, verb changes, adjective changes, and pronoun changes.
