...
2. The locale must exist. 
	- If not, see [Adding New Locales](/bug-reports#New_Locales).
3. You must be logged in, and under an account with permission to write to it.
	- If you don't have an account, see [Survey Tool Accounts](/survey-tool/accounts).
...
10. You will see a detailed list of the test results for the items you're submitting.
	...
	- If the message is "Item is not writable in the Survey Tool. Please file a ticket." then you will need to [file a ticket](/bug-reports#TOC-Filing-a-Ticket) instead. These can be filed in a single ticket. Include all the paths and the respective values.
