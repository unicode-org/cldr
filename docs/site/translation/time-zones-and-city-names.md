...
|---|---|---|---|---|
| gmtFormat | GMT{0}<br /> *or*<br /> GMT{ HOURS_FROM_UTC } | GMT-2:00 | **Offset Pattern.** Modify this field if the format for time zone offsets uses different letters, such as  HUA+02:00 or UTC+02:00 instead of GMT+02:00 , or if the letters occur after the time. Make sure you include the  {0} ; that is where the actual time value will go! |  |
| gmtFormat alt="utc" | UTC{0}<br /> *or*<br /> UTC{ HOURS_FROM_UTC } | UTC-2:00 | **UTC Pattern.** Similar to gmtFormat, but this is *required* to use the localized abbreviation for "Coordinated Universal Time". Also set this if gmtFormat already uses the localized abbreviation for "Coordinated Universal Time". Note that this is usually "UTC", which was explicitly chosen as the international designation for "Coordinated Universal Time".  |  |
| gmtZeroFormat | GMT{0}<br /> *or*<br /> GMT{ HOURS_FROM_UTC } | GMT+0:00 | **GMT Zero Pattern.**  *This field must exactly match the GMT Pattern.* |  |
| gmtZeroFormat | UTC{0}<br /> *or*<br /> UTC{ HOURS_FROM_UTC } | UTC+0:00 | **UTC Zero Pattern.**  *This field must exactly match the UTC Pattern.* |  |
| gmtUnknownFormat | GMT+? | GMT+? | **GMT Unknown Pattern.**  *This field must be consistent with the GMT Pattern.* |  |
| gmtUnknownFormat | UTC+? | UTC+? | **UTC Unknown Pattern.**  *This field must be consistent with the UTC Pattern.* |  |
