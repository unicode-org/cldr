...
//        }
    ...
    private void checkAttributes(DtdType dtdType1, DtdType dtdType2) {
        Map<R2<String, String>, R3<Set<String>, String, String>> mainData = ElementAttributeInfo.getInstance(dtdType1)
            .getElementAttribute2Data();
        Map<R2<String, String>, R3<Set<String>, String, String>> suppData = ElementAttributeInfo.getInstance(dtdType2)
            .getElementAttribute2Data();
        Set<R2<String, String>> commonKeys = getCommon(mainData.keySet(), suppData.keySet(),
            new TreeSet<R2<String, String>>());
...
//        checkElements(DtdType.ldmlBCP47, DtdType.supplementalData);
    ...
    private void checkElements(DtdType dtdType1, DtdType dtdType2) {
        ...
        for (String key : commonKeys) {
            ...
            errln("DTD elements have different children\t" + key + "\t" + dtdType1 + ":\t" + mainValues + "\t"
                + dtdType2 + ":\t" + suppValues);
        }
        for (String key : same) {
...
//        }
    ...
    static final Set<String> COLLATION_SINGLETONS = new HashSet<String>(Arrays.asList(new String[] {
        "first_non_ignorable", "first_primary_ignorable", "first_secondary_ignorable", "first_tertiary_ignorable",
        "first_trailing", "first_variable",
        "last_non_ignorable", "last_primary_ignorable", "last_secondary_ignorable", "last_tertiary_ignorable",
    ...
    private void checkEmpty(DtdType type) {
        ...
        Map<R2<String, String>, R3<Set<String>, String, String>> eaData = ElementAttributeInfo.getInstance(type)
            .getElementAttribute2Data();
...
//        }
    ...
    private void checkLeaf(DtdType type, String contained) {
        ...
        for (String element : data) {
            ...
            for (String attribute : attributes) {
                if (attribute.equals("draft") || attribute.equals("references")) continue;
                logln(type + ", " + contained + ", " + element + ", " + attribute + ", "
...
//        }
    ...
    public void checkNodeData(DtdType type) {
        ...
        Relation<String, String> dtdElementToAttributes = ElementAttributeInfo.getInstance(type)
            .getElement2Attributes();
        Map<R2<String, String>, R3<Set<String>, String, String>> attributeData = ElementAttributeInfo.getInstance(type)
            .getElementAttribute2Data();
        ...
        for (String element : dtdElementToAttributes.keySet()) {
            ...
            if (!dtdAttributes.containsAll(actualAttributes) || !dtdAttributes.containsAll(attributesAlwaysFound)) {
                errln(type + "\t" + "Actual attributes exceed DTD attributes:\t" + type + ", " + element + ", "
            ...
            if (notFound.size() != 0) {
                warnln(type + "\tAttributes not found for:\t" + element + "\tnotFound:\t" + notFound + "\tfound:\t"
                    + actualAttributes);
            }

            for (String attributeAlwaysFound : attributesAlwaysFound) {
                // make sure REQUIRED; not really an error, but for now...
                R3<Set<String>, String, String> attributeDatum = attributeData.get(Row
                    .of(element, attributeAlwaysFound));
                ...
                if (!"#REQUIRED".equals(attributeDatum.get1())) {
                    if (attributeDatum.get1() == null) {
                        warnln(type + "\tAttribute not REQUIRED but ALWAYS found, element:\t" + element
                            + "\tattribute:\t" + attributeAlwaysFound);
                    }
                }
            }
        }
        ...
        for (String s : overlap) {
            warnln(type + "\tOverlap in value and branch!!\t" + s + "\tvalue:\t" + data.valueNodes.get(s)
                + "\tbranch:\t" + data.branchNodes.get(s));
        }
        ...
        for (String s : overlap) {
            warnln(type + "\tOverlap in value and valueless!!\t" + s + "\tvalue:\t" + data.valueNodes.get(s)
                + "\tvalueless:\t" + data.valuelessNodes.get(s));
        }
    ...
    static class CurrentData {
        ...
        static void putIfNew(Map<String, R3<String, String, String>> map, String key, String value, String locale,
            String xpath) {
            if (!map.containsKey(key)) {
    ...
    private String checkAttributeStructure(String element, Set<String> attributes, Map<R2<String, String>,
        ...
        String result = "";
        for (String attribute : attributes) {
            if (attribute.equals("alt") || attribute.equals("draft") || attribute.equals("standard")
