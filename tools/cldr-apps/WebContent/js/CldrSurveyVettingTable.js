...
const cldrSurveyTable = (function() {
	...
	function updateRow(tr, theRow) {
		...
		if (tr.checksum !== undefined) {
			const newChecksum = cldrChecksum(JSON.stringify(theRow));
			if (newChecksum === tr.checksum) {
				return;
			}
			tr.checksum = newChecksum;
		} else {
			tr.checksum = cldrChecksum(JSON.stringify(theRow));
		}
		tr.theRow = theRow;
