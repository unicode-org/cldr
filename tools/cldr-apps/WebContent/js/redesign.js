...
function checkLocaleShow(element, size) {
	...
	if ((!element.hasClass('canmodify') && $('#show-read').is(':checked') && !element.hasClass('locked')) ||
...
function unpackMenuSideBar(json) {
	...
	if (!isVisitor) {
		...
		if (tmp) {
			html += '<li class="list-unstyled review-link" data-query="' + tmp.hasQuery +
				'" data-url="' + tmp.url + '"><div>' + tmp.display +
