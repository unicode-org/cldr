...

{
	 ...
	 */
	const sLong =  "{\"xpstrid\":\"525a75da362e7e3f\","
	    + "\"inheritedLocale\":\"de\","
	...
	describe('cldrSurveyTable.test.cldrChecksum', function() {
		...
		it('should not return null or undefined', function() {
			assert(cLong !== null && cLong !== undefined, 'cLong should not be null or undefined');
		});
		...
		it('should be fast enough', function() {
			...
			assert(duration < MAX_MILLISECS, 'Duration was '
					+ duration + ' ms; '
					+ ITERATIONS + ' iterations should take less than '
					+ MAX_MILLISECS + ' ms');
		});
	});
}
