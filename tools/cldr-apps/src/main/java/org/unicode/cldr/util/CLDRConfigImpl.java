...
public class CLDRConfigImpl extends CLDRConfig implements JSONString {
  ...
  CLDRConfigImpl() {
    ...
    if (cldrHomeSet == false) {
      ...
      logger.warning(
        "*** CLDRConfig.getInstance() was called prior to SurveyTool setup ***"
      );
      logger.warning(
        "*** This is probably an error. Look at the stack below, and make  ***"
      );
      logger.warning(
        "*** sure that CLDRConfig.getInstance() is not called at static    ***"
      );
      logger.warning(
        "*** init time.                                                    ***"
      );
  ...
  private synchronized void init() {
    ...
    logger.info(
      CLDRConfigImpl.class.getName() + ".init(), cldrHome=" + cldrHome
    );
    if (cldrHome == null) {
    ...
    survprops.put(
      "CLDR_DATA_HASH",
      CldrUtility.getGitHashForDir(survprops.getProperty("CLDR_DIR", null))
    );
  ...
  private void tryToInitCldrHome() {
    ...
    try {
      cldrHome =
        System.getProperty(CLDRConfigImpl.class.getName() + ".cldrHome", null);
  ...
  public static final String ALL_GIT_HASHES[] = {
    "CLDR_SURVEYTOOL_HASH",
    "CLDR_UTILITIES_HASH",
    "CLDR_DATA_HASH",
  };
  ...
  public static final String getGitHashForSlug(String slug) {
    ...
    try {
      ...
      for (
        final Enumeration<URL> e = classLoader.getResources(
   ...
   */
  private void loadIntoProperties(Properties props, File propFile)
    throws InternalError {
  ...
  public void writeHelperFile(String hostportpath, File helperFile)
    throws IOException {
    if (!helperFile.exists()) {
      try (
        OutputStream file = new BufferedOutputStream(
          new FileOutputStream(helperFile, false)
        ); // Append
  ...
  private void createBasicCldr(File homeFile) {
    logger.info(
      "Attempting to create /cldr  dir at " + homeFile.getAbsolutePath()
    );
    ...
    try {
      ...
      try (
        ...
        pw.println(
          "## generated on " + SurveyMain.localhost() + " at " + new Date()
        );
        pw.println(
          "## see the readme at \n## " +
          SurveyMain.URL_CLDR +
        ...
        pw.println(
          "CLDR_VAP=" + UserRegistry.makePassword(UserRegistry.ADMIN_EMAIL)
        );
        pw.println();
        pw.println("## Special Test Enablement.");
        pw.println(
          "CLDR_TESTPW=" + UserRegistry.makePassword("user@example.com")
        );
        pw.println();
        pw.println(
      ...
      logger.warning(
        "While writing " + homeFile.getAbsolutePath() + " props: " + exception
      );
  ...
  public CLDRURLS internalGetUrls() {
    ...
    return new StaticCLDRURLS(
      this.getProperty(CLDRURLS.CLDR_SURVEY_PATH, contextUrl)
    );
  }
  ...
  public CLDRURLS internalGetAbsoluteUrls() {
    ...
    return new StaticCLDRURLS(
      this.getProperty(CLDRURLS.CLDR_SURVEY_BASE, fullUrl)
    );
  }
