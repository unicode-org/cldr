...
class AbstractCacheManager {
  ...
  public void setup() {
    ...
    final Instant latestGoodInstant = cacheDir
      .getType()
      .getLatestGoodInstant(DEFAULT_DAYS);
    // -- cf. "...ready for requests..." in SurveyMain called later but printed sooner
    logger.info(
      "AbstractCacheManager: Loaded abstracts (last calculated at " + d + ")"
    );
    if (d == null || d.isBefore(latestGoodInstant)) {
      System.out.println(
        "Will reload abstracts: " + d + " with expiry " + latestGoodInstant
      );
