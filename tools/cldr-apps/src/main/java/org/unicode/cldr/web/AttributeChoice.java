  ...
  private AttributeChoice(String type, String baseXpath) {
    ...
    if (value != null) {
      ...
      if (baseXpath.endsWith(attrString)) {
        this.baseXpath =
          baseXpath.substring(0, baseXpath.length() - attrString.length());
      } else {
        throw new InternalError(
          "Can't find " + attrString + " in " + baseXpath
        );
      }
    } else {
      this.baseXpath = baseXpath;
    }
    // System.err.println("Initialized a " + type + " @ " + baseXpath +
    // " ~= " + attribute);
  }

  public static AttributeChoice createChoice(String baseXpath) {
    // System.err.println("cc: " + baseXpath + " - matchers " +
    // allXpaths.size());
  ...
  public String xpathForChoice(String choice) {
    if (!values.contains(choice)) {
      throw new IllegalArgumentException(
        "Invalid choice for " + attribute + " - " + choice
      );
    }
