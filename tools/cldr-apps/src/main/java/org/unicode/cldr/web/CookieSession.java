...
public class CookieSession {
  ...
  public long millisTillKick() {
    ...
    final long remainMillis =
      (1000 * myTimeoutSecs) -
      (nowMillisSinceEpoch - lastActionMillisSinceEpoch);
  ...
  public static Set<CookieSession> getAllSet() {
    synchronized (gHash) {
      TreeSet<CookieSession> sessSet = new TreeSet<>(
  ...
  public static CookieSession newSession(
    boolean isGuest,
    String ip,
    String fromId
  ) {
    ...
    synchronized (gHash) {
      ...
      if (rv != null) {
        ...
        if (!rv.ip.equals(ip)) {
          if (SurveyMain.isUnofficial()) System.out.println(
            "IP changed from " + rv.ip + " to " + ip + " - " + rv
          );
          rv.ip = ip;
          rv.touch();
        }
  ...
  public void remove() {
    ...
    if (DEBUG_INOUT) System.out.println(
      "S: Removing session: " + id + " - " + user
    );
  }
  ...
  public static synchronized String newId(boolean isGuest) {
    try {
      ...
      byte[] outBytes = aDigest.digest(
        new Integer(myRand.nextInt()).toString().getBytes()
      );
  ...
  public Hashtable<String, Hashtable<String, Object>> getLocales() {
    synchronized (stuff) {
      Hashtable<String, Hashtable<String, Object>> l = (Hashtable<String, Hashtable<String, Object>>) get(
        "locales"
      );
      if (l == null) {
        l = new Hashtable<>();
    ...
    public final int value() {
      if (value == null) {
        value =
          CLDRConfig
            .getInstance()
            .getProperty(this.name().toUpperCase(), defVal);
        SurveyLog.warnOnce(
          logger,
          "CookieSession: " +
    ...
    if (limit == 0) {
      ...
      if (count >= limit) {
        if (SurveyMain.isUnofficial()) {
          System.out.println(
            "Too many users. limit=" + limit + ", count=" + count
          );
        }
        return true;
      } else {
        if (false && DEBUG_INOUT && SurveyMain.isUnofficial()) {
          System.out.println(
            "User count OK. . limit=" + limit + ", count=" + count
          );
        }
        return false;
      }
  ...
  public static boolean tooManyGuests() {
    ...
    if (tooManyUsers()) {
      ...
      if (count >= limit) {
        if (SurveyMain.isUnofficial()) System.out.println(
          "TOO MANY GUESTS. limit=" + limit + ", count=" + count
        );
        return true;
      } else {
        if (
          false && DEBUG_INOUT && SurveyMain.isUnofficial()
        ) System.out.println(
          "Guest count OK. . limit=" + limit + ", count=" + count
        );
        return false;
      }
    }
        ...
        if (cs.user == null) { // guest
          if (
            ...
            (
              ...
              Params.CLDR_GUEST_TIMEOUT_SECS.value() *
              1000
            ) ||
          ...
          if (
            (
              KICK_IF_ABSENT &&
              cs.millisSinceLastBrowserCall() >
              Params.CLDR_USER_TIMEOUT_SECS.value() *
              1000
            ) ||
            (KICK_IF_INACTIVE && cs.millisTillKick() <= 0)
          ) {
  ...
  public static synchronized CookieSession checkForAbuseFrom(
    String userIP,
    Hashtable<String, Object> BAD_IPS,
    String userAgent
  ) {
    ...
    synchronized (gHash) {
      for (Object o : gHash.values()) {
        ...
        if (
          (nowMillisSinceEpoch - cs.lastBrowserCallMillisSinceEpoch) <
          (N_MINUTES * 60 * 1000)
        ) {
          noSes++;
        }
  ...
  public String getOrgCoverageLevel(String locale) {
    ...
    if ((myOrg == null) || !WebContext.isCoverageOrganization(myOrg)) {
      ...
      Level coverageLevel = StandardCodes
        .make()
        .getLocaleCoverageLevel(myOrg, locale);
  ...
  public String getEffectiveCoverageLevel(String locale) {
    String level = sm.getListSetting(
      settings,
    ...
    if (
      (level == null) ||
      (level.equals(WebContext.COVLEV_RECOMMENDED)) ||
