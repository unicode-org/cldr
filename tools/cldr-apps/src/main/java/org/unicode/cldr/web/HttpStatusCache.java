...
public class HttpStatusCache {
  ...
  public static final Integer check(final URL url) {
    try {
      ...
      System.err.println(
        "HttpStatusCache: Err in check of URL " + url.toString()
      );
      e.printStackTrace();
      return null;
    }
  ...
  public static final void flush(final URL url) {
    if (url == null) {
      System.err.println(
        "HTTP Status Cache: invalidating all " + urlCache.size()
      );
  ...
  private static Cache<URL, Integer> urlCache = CacheBuilder
    .newBuilder()
    .maximumSize(8192)
