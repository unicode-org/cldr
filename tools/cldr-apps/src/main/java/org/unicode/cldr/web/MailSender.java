...
public class MailSender implements Runnable {
  ...
  private static final class MailConfig {
    ...
    MailConfig() {
      CLDR_SENDMAIL =
        CLDRConfig.getInstance().getProperty("CLDR_SENDMAIL", true);
      CLDR_MAIL_DELAY_BATCH_ITEM =
        CLDRConfig.getInstance().getProperty("CLDR_MAIL_DELAY_BATCH_ITEM", 0);
      CLDR_MAIL_BATCHSIZE =
        CLDRConfig.getInstance().getProperty("CLDR_MAIL_BATCHSIZE", 100);
      ...
      CLDR_MAIL_DELAY_EACH =
        CLDRConfig.getInstance().getProperty("CLDR_MAIL_DELAY_EACH", 60);
    }
  }
  ...
  public JSONObject getMailFor(int user)
  ...
  public boolean setRead(int user, int id) {
    ...
    try {
      ...
      SurveyLog.logException(
        logger,
        se,
        "mark ing as read id#" + id + " user=" + user
      );
      return false;
    } finally {
  ...
  private MailSender() {
    ...
    try {
      ...
      // reap old items
      java.sql.Timestamp aWeekAgo = new java.sql.Timestamp(
        System.currentTimeMillis() - (1000 * 60 * 60 * 24 * 7 * 3)
      ); // reap mail after about 3 weeks
      s3 =
        DBUtils.prepareStatementWithArgs(
          conn,
          "delete from " + CLDR_MAIL + " where queue_date < ? ",
          aWeekAgo
        );
      ...
      if (countDeleted > 0) {
        logger.info(
          "MailSender:  reaped " + countDeleted + " expired messages"
        );
      }
  ...
  public void queue(
    ...
    try (
      ...
      PreparedStatement s2 = DBUtils.prepareStatementWithArgs(
        conn,
        "INSERT INTO " +
        CLDR_MAIL +
        "(sender, " +
      ...
      SurveyLog.logException(
        logger,
        se,
        "Enqueuing mail to #" + toUser + ":" + subject
      );
  ...
  private Transport processAllMail() {
    ...
    for (;;) {
      ...
      if (
        MailConfig.INSTANCE.CLDR_MAIL_BATCHSIZE > 0 &&
      ...
      logger.finest(() ->
        String.format(
          "Sleep %ds",
          MailConfig.INSTANCE.CLDR_MAIL_DELAY_BATCH_ITEM
        )
      );
      try {
  ...
  private Transport processOneMail(Session ourSession, Transport transport) {
    ...
    try (
      ...
      PreparedStatement s = DBUtils.prepareStatementWithArgsUpdateable(
        conn,
        "select * from " +
        CLDR_MAIL +
      ...
      try {
        ...
        MimeMessage ourMessage = createMimeMessage(
          ourSession,
          subject,
          to,
          toUser,
        ...
        SQLException | MessagingException | UnsupportedEncodingException e
      ) {
      ...
      SurveyLog.logException(
        logger,
        se,
        String.format("processing mail (last %d)", lastIdProcessed)
      );
    }
    return transport;
  }
  ...
  private void handleMailException(
    ...
    SurveyLog.logException(
      logger,
      e,
      "Trying to process mail id#" + lastIdProcessed
    );
    int badCount = 0;
  ...
  private void updateMailStatus(
    ...
    if (!DBUtils.updateTimestamp(rs, "sent_date", sqlnow)) {
      ...
      logger.fine(
        "Mail: Row updated: #id " + lastIdProcessed + " to " + toUser
      );
      rs.updateRow();
    }
  ...
  private Transport sendMimeMessage(
    ...
    if (ourSession.getProperties().getProperty("mail.host", null) != null) {
      ...
      if (transport == null) {
        ...
        logger.finest(
          "re/got transport, connecting to " + transport.toString()
        );
        transport.connect();
      }
  ...
  private MimeMessage createMimeMessage(
    ...
    ourMessage.addHeader(
      "X-SurveyTool-Queue-Id",
      Integer.toString(lastIdProcessed)
    );

    final String header = SurveyMain.isUnofficial()
