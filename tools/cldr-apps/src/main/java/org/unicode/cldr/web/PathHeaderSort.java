...
public class PathHeaderSort extends SortMode {
  ...
  @Override
  public DataSection.DisplaySet createDisplaySet(
    XPathMatcher matcher,
    Collection<DataRow> values
  ) {
    // final Set<String> headings = new TreeSet<String>();
    DataRow rows[] = createSortedList(
      new Comparator<DataRow>() {
        @Override
    ...
    return new DataSection.DisplaySet(
      rows,
      this,
      thePartitions.toArray(new Partition[thePartitions.size()])
    );
  }
}
