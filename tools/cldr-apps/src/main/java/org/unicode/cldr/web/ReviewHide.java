...
public class ReviewHide {
    ...
    private static final String GET_ITEM_SQL = "SELECT * FROM " +
            DBUtils.Table.DASH_HIDE +
            " WHERE user_id=? AND locale=? AND subtype=? AND xpath=? AND val=?";

    private static final String INSERT_ITEM_SQL = "INSERT INTO " +
            DBUtils.Table.DASH_HIDE +
            " (user_id,locale,subtype,xpath,val) VALUES(?,?,?,?,?)";
    ...
    private void getData() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                final String xpathHexId = rs.getString("xpath");
                final String value = DBUtils.getStringUTF8(rs, "val");
                this.hiddenNotifications.put(subtype, xpathHexId, value);
            }
    ...
    public void toggleItem(String subtype, String xpathHexId, String value) {
        try {
            Connection conn = null;
            ResultSet rs = null;
            ...
            try {
                ...
                setArgs(ps, userId, localeId, subtype, xpathHexId, value);
                rs = ps.executeQuery();
                if (!rs.next()) {
                ...
                setArgs(updateQuery, userId, localeId, subtype, xpathHexId, value);
    ...
    private void setArgs(PreparedStatement ps, int userId, String localeId,
                         String subtype, String xpathHexId, String value) throws SQLException {
        ps.setInt(1, userId);
        ps.setString(2, localeId);
        ps.setString(3, subtype);
        ps.setString(4, xpathHexId);
        DBUtils.setStringUTF8(ps, 5, value);
    }
}
