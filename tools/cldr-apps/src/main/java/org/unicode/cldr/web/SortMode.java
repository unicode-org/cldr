...
public abstract class SortMode {
  ...
  static String getSortMode(WebContext ctx, String prefix) {
    ...
    sortMode =
      ctx.pref(SurveyMain.PREF_SORTMODE, SurveyMain.PREF_SORTMODE_DEFAULT);
    return sortMode;
  }
   ...
   * @return
   */
  protected static final int categorizeDataRow(
    DataRow p,
  ...
  protected static int compareMembers(
    DataRow p1,
    DataRow p2,
   ...
   * @return
   */
  protected Partition[] createPartitions(
    Membership[] memberships,
    DataRow[] rows
  ) {
    ...
    if (memberships != null) { // something with partitions
      ...
      // catch the last item
      if (
        (testPartitions[lastGood].start != -1) &&
