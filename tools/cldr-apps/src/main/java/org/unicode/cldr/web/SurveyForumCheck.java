...
public class SurveyForumCheck {
    ...
    public void run() {
        ...
        try {
            ...
            logger.warning("SurveyForumCheck: loaded " + map.size() + " posts; found " + errorCount + " error(s); time: " + et);
    ...
    private void checkPosts() {
        for (Map.Entry<Integer, TestPostInfo> entry : map.entrySet()) {
            ...
            if ((info.parent == NO_PARENT) != (info.root == NO_PARENT)) {
                fail(id, info, "each post has parent = NO_PARENT if and only if root = NO_PARENT", null);
                ...
                fail(id, info, "if a post has parent = n, then a post with id = n must exist", null);
    ...
    private void traverseToRoot(int origId, final TestPostInfo origInfo) {
        ...
        while (info.parent != NO_PARENT) {
            ...
            if (info.forum != origInfo.forum) {
                fail(origId, origInfo, "posts in same thread have same forum id",
                    "reached id " + id + "; forum " + info.forum);
                return;
            } else if (info.root != NO_PARENT && info.root != origInfo.root) {
                fail(origId, origInfo, "posts in same thread have same root",
                    "reached id " + id + "; root " + info.root);
                return;
            } else if (info.root == NO_PARENT && id != origInfo.root) {
                fail(origId, origInfo, "root post id matches descendants",
                    "reached id " + id + "; root " + info.root);
                return;
            } else if (++count > 1000) {
    ...
    private void fail(int id, TestPostInfo info, String expectation, String details) {
        ++errorCount;
        logger.severe("SurveyForumCheck: FAILED expectation that " + expectation +
            "; id = " + id +
            "; parent = " + subst(info.parent) +
            "; root = " + subst(info.root) +
            "; forum = " + subst(info.forum) +
