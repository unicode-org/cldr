...
public class SurveyMain
  extends HttpServlet
  implements CLDRProgressIndicator, Externalizable {
  ...
  private static UnicodeSet supportedNameSet = new UnicodeSet("[a-zA-Z]")
    .freeze();
  static final int TWELVE_WEEKS = 3600 * 24 * 7 * 12;

  public static final String DEFAULT_CONTENT_LINK =
    "<i><a target='CLDR-ST-DOCS' href='http://cldr.unicode.org/translation/default-content'>default content locale</a></i>";
  ...
  public static final String TRANS_HINT_LANGUAGE_NAME = TRANS_HINT_LOCALE.getDisplayLanguage(
    TRANS_HINT_LOCALE
  ); // Note:
  // Only
  // shows
  ...
  private static String sysmsg(String msg) {
    try {
      ...
      logger.warning(
        "Err " + t.toString() + " while trying to load sysmsg " + msg
      );
  ...
  public static SurveyMain getInstance(HttpServletRequest req) {
    ...
    return (SurveyMain) config
      .getServletContext()
      .getAttribute(SurveyMain.class.getName());
  }
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
     ...
     */
    if (
      request.getParameter("udump") != null &&
      request.getParameter("udump").equals(vap)
    ) { // XML.
  ...
  private void createNewAndLogin(
    ...
    if (real.isEmpty() || real.equals("REALNAME")) {
      ...
      String randomEmail =
        UserRegistry.makePassword(null) +
        "@" +
      ...
      if (autoProceed) {
        ctx.print(
          "You should be logged in shortly, otherwise click this link:"
        );
      } else {
   ...
   * @throws IOException
   */
  private boolean respondToBogusRequest(
    HttpServletRequest request,
    HttpServletResponse response
  ) throws IOException {
  ...
  private void showOfflinePage(
    ...
    try {
      ...
      SurveyLog.logException(
        logger,
        e,
        "SurveyMain.showOfflinePage calling SurveyTool.includeJavaScript"
      );
    ...
    out.print(
      "<div id='st_err'><!-- for ajax errs --></div><span id='progress'>"
    );
  ...
  private boolean ensureStartup(
    ...
    if (!isSetup) {
      ...
      if (isGET) {
        ...
        String pi = "";
        if (
          request.getPathInfo() != null && request.getPathInfo().length() > 0
        ) {
      ...
      try {
        ...
        SurveyLog.logException(
          logger,
          e,
          "SurveyMain.ensureStartup calling SurveyTool.includeJavaScript"
        );
      }
      if (isUnofficial()) {
      ...
      if (sandbox == null) {
        try {
          sandbox =
            new SandboxLocales(
              CLDRCacheDir
                .getInstance(CLDRCacheDir.CacheType.sandbox)
                .getEmptyDir()
            );
        } catch (IOException e) {
      ...
      if (!tmpHome.isDirectory()) {
        if (!tmpHome.mkdir()) {
          throw new InternalError(
            "Couldn't create " + tmpHome.getAbsolutePath()
          );
        }
      }
  ...
  private static String getFileBaseSeed() {
    ...
    if (fileBaseSeed == null) throw new NullPointerException(
      "fileBaseSeed==NULL"
    );
    return fileBaseSeed;
  }
  ...
  private void doSql(WebContext ctx) {
    ...
    if (isBusted != null) { // This may or may
      ...
      // SQL?
      ctx.println(
        "<h4>ST not currently started, attempting to make SQL available</h4>"
      );
      ctx.println("<pre>");
      specialMessage =
        "<b>SurveyTool is in an administrative mode- please log off.</b>";
      try {
        doStartupDB();
      } catch (Throwable t) {
    ...
    if (q.length() == 0) {
      ...
      ctx.println(
        "<a href='" + ctx.base() + "?sql=" + vap + "'>[List of Tables]</a>"
      );
    }
    ctx.println("<form method=POST action='" + ctx.base() + "'>");
    ctx.println("<input type=hidden name=sql value='" + vap + "'>");
    ctx.println(
      "SQL: <input class='inputbox' name=q size=80 cols=80 value=\"" +
      q +
      "\"><br>"
    );
    ctx.println(
    ...
    if (q.length() > 0) {
      ...
      try {
            ...
            for (i = 1; i <= cc; i++) {
              ...
              if (v != null) {
                ...
                if (tblsel == true) {
                  ...
                  ctx.println("<form method=POST action='" + ctx.base() + "'>");
                  ctx.println(
                    "<input type=hidden name=sql value='" + vap + "'>"
                  );
                  ctx.println(
        ...
        try {
          ...
          SurveyLog.logException(logger, se, ctx);
          String complaint =
            "in s.closing: SQL err: " + DBUtils.unchainSqlException(se);
  ...
  public void printHeader(WebContext ctx, String title) {
    ...
    try {
      ...
      SurveyLog.logException(
        logger,
        e,
        "SurveyMain.printHeader calling SurveyTool.includeJavaScript"
      );
    }
  ...
  private String getTopBox() {
    ...
    if (isBusted != null) {
      ...
      String stack = SurveyForum
        .HTMLSafe(isBustedStack)
        .replaceAll("\t", "&nbsp;&nbsp;&nbsp;")
        .replaceAll("\n", "<br>");
  ...
  public static String getCurrev(boolean asLink) {
    ...
    for (Iterator<String> i = currev.keys(); i.hasNext();) {
      ...
      output.append(' ');
      if (asLink) {
        final String friendly = HASH_PATTERN
          .matcher(k)
          .replaceFirst("$1")
          .toLowerCase();
  ...
  public static String getGuestsAndUsers() {
    ...
    if (load > 0.0) {
      ...
      out.append(
        "/<span title='Total System Load' style='background-color: #ff"
      );
  ...
  private void setLocale(WebContext ctx) {
    ...
    if (locale != null && (locale.length() > 0)) {
      ...
      if (getLocalesSet().contains(l)) {
        CLDRLocale theDefaultContent = getSupplementalDataInfo()
          .getBaseFromDefaultContent(l);
   ...
   */
  private void printUserTableWithHelp(
    WebContext ctx,
    String helpLink,
    String helpName
  ) {
    ...
    try {
      ...
      synchronized (reg) {
        ...
        while (rs.next()) {
          ...
          ctx.println(
            "\t<user id=\"" + theirId + "\" email=\"" + theirEmail + "\">"
          );
          ctx.println(
            "\t\t<level n=\"" +
            theirLevel +
          ...
          final LocaleSet locs = LocaleNormalizer.setFromStringQuietly(
            theirLocales,
            null
          );
  ...
  private void doSession(WebContext ctx) throws IOException, SurveyException {
    ...
    if (ctx.session == null) {
      ...
      ctx.println(
        "<p><img src='stop.png' width='16'>" + ctx.getSessionMessage() + "</p>"
      );
      ctx.println(
        "<hr><a href='" +
      ...
      try {
        ...
        if (xpath != null) {
          ...
          if (ph == null) {
            ...
            ph.getSurveyToolStatus() == SurveyToolStatus.HIDE ||
        ...
        SurveyLog.logException(
          logger,
          t,
          "Exception processing StringID " + strid + " - " + whyBad,
          ctx
        );
      }
    }
      ...
      if (doWhat.equals("options")) {
        ...
        try {
          ...
          throw new RuntimeException(
            ioe.toString() + " while redirecting to logout"
          );
        }
        return;
      }
    ...
    if ((ctx.getLocale() != null) && (!shortHeader(ctx))) {
      ...
      if (aliasTarget != null) {
         ...
         */
        CLDRLocale dcParent = getSupplementalDataInfo()
          .getBaseFromDefaultContent(aliasTarget);
        if (dcParent == null) {
  ...
  private void printRecentLocales(WebContext baseContext, WebContext ctx) {
    ...
    if (e.hasMoreElements()) {
      ...
      for (; e.hasMoreElements();) {
        ...
        if (
          (ctx.getLocale() != null) && (ctx.getLocale().toString().equals(k))
        ) {
          continue;
        }
        if (!shownHeader) {
  ...
  public synchronized LocaleTree getLocaleTree() {
    if (localeTree == null) {
      ...
      if (inFiles == null) {
        ...
        throw new RuntimeException(
          "Can't load CLDR data files from " + fileBase
        );
      }
  ...
  private CLDRFormatter setDefaultCLDRLocaleFormatter() {
    CLDRFormatter defaultFormatter = new CLDRLocale.CLDRFormatter(
      getEnglishFile(),
      FormatBehavior.replace
    );
   ...
   */
  public synchronized Collection<CLDRLocale> getRelatedLocs(
    CLDRLocale topLocale
  ) {
    ...
    if (cachedSet == null) {
      ...
      for (CLDRLocale atopLocale : lt.getTopCLDRLocales()) { // add each of the top locales that has the same "highest nonroot parent"
        if (atopLocale.getHighestNonrootParent() == topLocale) {
          final Collection<CLDRLocale> topLocales = lt
            .getSubLocales(atopLocale)
            .values();
          if (topLocales != null) {
  ...
  public String getLocaleLink(WebContext ctx, CLDRLocale locale, String n) {
    ...
    boolean isDefaultContent = getSupplementalDataInfo()
      .isDefaultContent(locale);
    ...
    boolean canModify =
      !isDefaultContent &&
      UserRegistry.userCanModifyLocale(ctx.session.user, locale);
    if (canModify) {
      rv = rv + (modifyThing(ctx));
      int odisp = 0;
      if (
        (
          SurveyMain.phase() == Phase.VETTING ||
  ...
  void notifyUser(WebContext ctx, String theirEmail, String pass) {
    ...
    if (ctx != null) {
      ...
      fromId = null;
      whySent =
        "Your CLDR vetting account information is being sent to you\n\n";
    }
    String body =
      whySent +
      "To access it, visit: \n<" +
  ...
  private void printLocaleTreeMenu(WebContext ctx, String which) {
    ...
    boolean canModify = UserRegistry.userCanModifyLocale(
      subCtx.session.user,
      subCtx.getLocale()
    );
  ...
  public synchronized CLDRFile getTranslationHintsFile() {
    if (gTranslationHintsFile == null) {
      try {
        CLDRFile file = getDiskFactory()
          .make(TRANS_HINT_LOCALE.toString(), true);
        ...
        busted(
          "Could not load translation hints locale " + TRANS_HINT_LOCALE,
          t
        );
      }
    }
  ...
  public synchronized ExampleGenerator getTranslationHintsExample() {
    if (gTranslationHintsExample == null) {
      ...
      gTranslationHintsExample =
        new ExampleGenerator(
          translationHintsFile,
          translationHintsFile,
          fileBase + "/../supplemental/"
        );
    }
    /*
     ...
     */
    gTranslationHintsExample.setVerboseErrors(
      twidBool("ExampleGenerator.setVerboseErrors")
    );
    return gTranslationHintsExample;
  }

  public synchronized WebContext.HTMLDirection getHTMLDirectionFor(
    CLDRLocale locale
  ) {
  ...
  public CheckCLDR createCheck() {
    CheckCLDR checkCldr;
    checkCldr =
      CheckCLDR.getCheckAll(getSTFactory(), "(?!.*(CheckCoverage).*).*");
  ...
  private synchronized void checkAllLocales() {
    ...
    CLDRProgressTask progress = openProgress(
      "Parse locales from XML",
      locales.size()
    );
    try {
      File xmlCacheDir = CLDRCacheDir
        .getInstance(CLDRCacheDir.CacheType.xmlCache)
        .getEmptyDir();
      File xmlCache = new File(xmlCacheDir, XML_CACHE_PROPERTIES);
      File xmlCacheBack = new File(
        xmlCacheDir,
        XML_CACHE_PROPERTIES + ".backup"
      );
      ...
      for (File f : getInFiles()) {
        ...
        try {
          ...
          if (useCache && oldHash != null && oldHash.equals(fileHash)) {
            ...
            Node directionalityItem = LDMLUtilities.getNode(
              d,
              "//ldml/layout/orientation/characterOrder"
            );
            if (directionalityItem != null) {
              direction = LDMLUtilities.getNodeValue(directionalityItem);
              if (direction != null && direction.length() > 0) {} else {
                direction = null;
              }
            }

            Node[] aliasItems = LDMLUtilities.getNodeListAsArray(
              d,
              "//ldml/alias"
            );
            ...
            if ((aliasItems == null) || (aliasItems.length == 0)) {
              ...
              aliasTo =
                LDMLUtilities.getAttributeValue(aliasItems[0], "source");
            }
          }
          ...
          logger.warning(
            "isLocaleAliased: Failed load/validate on: " +
            loc +
            " - " +
            t.toString()
          );
      ...
      if (useCache) {
        try {
          ...
          xmlCachePropsNew.store(
            os,
            "YOU MAY DELETE THIS CACHE. Cache updated at " + new Date()
          );
  ...
  public Set<String> getMetazones(String subclass) {
    ...
    for (String zone : supplementalDataInfo.getAllMetazones()) {
      if (
        subclass.equals(
          supplementalDataInfo.getMetazoneToContinentMap().get(zone)
        )
      ) {
        subSet.add(zone);
      }
  ...
  private void verifySupplementalData() {
    try {
      getSupplementalDataInfo()
        .getBaseFromDefaultContent(CLDRLocale.getInstance("mt_MT"));
    } catch (InternalError ie) {
      logger.warning(
        "can't do SupplementalData.defaultContentToParent() - " + ie
      );
      ie.printStackTrace();
  ...
  private static String getMaintMessage(
    final File maintFile,
    HttpServletRequest request
  ) {
    if (!maintFile.exists() && request != null) {
      try {
    ...
    if (maintFile.exists()) {
      final String maintMessage =
        "SurveyTool is in setup mode. <br><b>Administrator</b>: Please open the file <a href='file://" +
        maintFile.getAbsolutePath() +
        "'>" +
        maintFile.getAbsolutePath() +
        "</a>" +
   ...
   */
  public static synchronized void writeHelperFile(
    HttpServletRequest request,
    File maintFile
  ) throws IOException {
    CLDRConfigImpl
  ...
  public synchronized File getVetdir() {
    if (_vetdir == null) {
      CLDRConfig survprops = CLDRConfig.getInstance();
      vetdata =
        survprops.getProperty(
          "CLDR_VET_DATA",
          SurveyMain.getSurveyHome() + "/vetdata"
        ); // dir
      // for
      // vetted
      // data
   ...
   * @throws IOException
   */
  public synchronized File getDataFile(String kind, CLDRLocale loc)
    throws IOException {
  ...
  private static synchronized void loadLocalesSet() {
    ...
    String onlyLocales = CLDRConfig
      .getInstance()
      .getProperty("CLDR_ONLY_LOCALES", null);
    ...
    for (int i = 0; i < nrInFiles; i++) {
      ...
      if (dot != -1) {
        ...
        if (t == Type.scratch) {
          ...
          t == Type.readonly || (onlySet != null && !onlySet.contains(locale))
        ) {
          ro.add(l); // readonly
        } else {
          w.add(l); // writeable
  ...
  public static void markBusted(Throwable t) {
    markBusted(
      t.toString(),
      t,
      StackTracker.stackToString(t.getStackTrace(), 0)
    );
  }
  ...
  private static void busted(String what, Throwable t, String stack) {
    if (t != null) {
      SurveyLog.logException(
        ...
        t,
        what
        /* , ignore stack - fetched from exception */
      );
    }
    logger.warning(
      "SurveyTool " +
      SurveyMain.getCurrevCldrApps() +
      " busted: " +
      what +
      " ( after " +
      pages +
  ...
  public static void markBusted(String what, Throwable t, String stack) {
    ...
    if (!isBusted()) { // Keep original failure message.
      ...
      SurveyLog.warnOnce(
        logger,
        "[was already busted, not overriding old message.]"
      );
    }
  }
  ...
  private static String getShortened(
    String shortStr,
    String longStr,
    String warnHash
  ) {
    return (
      ("<span id='h_ww" + warnHash + "'>" + shortStr + "... ") +
      (
        "<a href='javascript:show(\"ww" +
        warnHash +
        "\")'>" +
        SHORT_A +
        "</a></span>"
  ...
  private boolean readWarnings() {
    try {
      BufferedReader in = FileUtilities.openUTF8Reader(
        cldrHome,
        "surveyInfo.txt"
      );
      String line;
      ...
      t.printStackTrace();
      busted(
        "Error: trying to read xpath warnings file.  " +
        cldrHome +
        "/surveyInfo.txt"
      );
      return true;
  ...
  public static String getVotesAfterString() {
    return CLDRConfig
      .getInstance()
      .getProperty(
        SurveyMain.CLDR_NEWVERSION_AFTER,
        SurveyMain.NEWVERSION_EPOCH
      );
  }
  ...
  public CLDRFile getEnglishFile() {
    if (gEnglishFile == null) synchronized (this) {
      CLDRFile english = getDiskFactory()
        .make(ULocale.ENGLISH.getBaseName(), true);
  ...
  public JSONObject statusJSON(HttpServletRequest request)
