...
public class SurveyProgressManager implements CLDRProgressIndicator {
  ...
  private static final Logger logger = SurveyLog.forClass(
    SurveyProgressManager.class
  );
  private Deque<SurveyProgressTask> tasks = new LinkedList<>();

  private class SurveyProgressTask
    implements CLDRProgressIndicator.CLDRProgressTask {
    ...
    public void update(String what) {
      ...
      subTaskTime = System.currentTimeMillis();
      logger.fine(
        "Progress (" + progressWhat + ") on " + what + " @" + SurveyMain.uptime
      );
    }
  ...
  public String getProgress() {
    ...
    Set<SurveyProgressTask> orderedTasks = new TreeSet<>(
  ...
  public static StringBuffer appendProgressBar(
    StringBuffer buf,
    double cur,
    double max
  ) {
