...
public class UserList {
  ...
  public UserList(
    ...
    canShowLocked =
      UserRegistry.userIsExactlyManager(me) || UserRegistry.userIsTC(me);
  ...
  public void getJson(SurveyJSONWrapper r)
    throws JSONException, SurveyException, IOException {
    if (!isValid) {
  ...
  private void listUsers(SurveyJSONWrapper r) throws JSONException {
    ...
    try {
      ...
      synchronized (reg) {
        ...
        r.put(
          "canGetEmailList",
          (!isJustMe) && UserRegistry.userCanGetEmailList(me)
        );
      }
  ...
  private void handleOneRow(ResultSet rs, JSONArray shownUsers)
    ...
    if (
      !showLocked && level >= UserRegistry.LOCKED && just == null
      /* if only one user, show regardless of lock state. */
    ) {
      return;
    }
   ...
   * @throws JSONException
   */
  private void handleOneUser(JSONArray shownUsers, UserSettings u)
    throws JSONException {
  ...
  private void handleActions(String action, UserSettings u) {
    if (getParam(LIST_ACTION_SETLOCALES + u.tag).length() > 0) {
      ...
      action == null || action.length() == 0 || action.equals(LIST_ACTION_NONE)
    ) {
      return;
      ...
      (UserRegistry.userCanDeleteUser(me, u.user.id, u.user.userlevel)) &&
      ...
      (UserRegistry.userCanModifyUser(me, u.user.id, u.user.userlevel)) &&
      ...
      u.ua.put(
        action,
        "<i>You can't change that setting on your own account.</i>"
      );
    } else {
  ...
  private void changeUserInfo(InfoType type, String action, UserSettings u) {
    ...
    if (s0.equals(s1) && s0.length() > 0) {
      String s =
        "<h4>Change " + what + " to <tt class='codebox'>" + s0 + "</tt></h4>";
      s +=
        "<div class='fnotebox'>" +
  ...
  private void changePassword(
    ...
    if (s0.equals(s1) && s0.length() > 0) {
      ...
      s +=
        " USE OF A RANDOM PASSWORD (as suggested) IS STRONGLY RECOMMENDED.</em></p>";
      s +=
        "<label><b>New " +
        what +
      ...
      s += "<br /><br />";
      s +=
        "Suggested random password: <tt>" +
        UserRegistry.makePassword(u.user.email) +
        "</tt> )";
      u.ua.put(action, s);
    }
  }

  private void putShownUser(JSONArray shownUsers, UserSettings u)
    throws JSONException {
    User user = u.user;
    String active = (u.session == null)
      ? ""
      : SurveyMain.timeDiff(u.session.getLastBrowserCallMillisSinceEpoch());
    String seen = (user.last_connect == null)
      ? ""
      : SurveyMain.timeDiff(user.last_connect.getTime());
    boolean havePermToChange = me.isAdminFor(user);
    boolean userCanDeleteUser = UserRegistry.userCanDeleteUser(
      me,
      user.id,
      user.userlevel
    );
  ...
  private class UserActions implements JSONString {
    ...
    public void put(String key, String value) {
      if (key == null || key.isEmpty()) {
        System.out.println(
          "Warning: invalid input to UserActions.put; value = " + value
        );
        return;
      }
      if (map == null) {
  ...
  private class EmailInfo {
    ...
    public EmailInfo(SurveyJSONWrapper r) {
      ...
      if (UserRegistry.userCanEmailUsers(me)) {
        if (
          getParam(LIST_MAILUSER_CONFIRM).equals(LIST_MAILUSER_CONFIRM_CODE)
        ) {
    ...
    public void putStatus(int n) {
      ...
      if (getParam(LIST_MAILUSER).length() == 0) {
        ...
        if (sendWhat.length() > 0) {
          if (!didConfirmMail) {
            if (
              !getParam(LIST_MAILUSER_CONFIRM)
                .equals(LIST_MAILUSER_CONFIRM_CODE) &&
