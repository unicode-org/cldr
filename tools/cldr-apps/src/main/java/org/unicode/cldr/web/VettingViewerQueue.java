...
public class VettingViewerQueue {
  ...
  private class Task implements Runnable {
     ...
     * @author srl295
     */
    private final class CLDRProgressCallback
      extends VettingViewer.ProgressCallback {
      ...
      private String setRemStr(long now) {
        ...
        String remStr =
          ElapsedTimer.elapsedTime(now, now + rem) + " " + "remaining";
        /*
    ...
    public void run() {
       ...
       * Reference: https://unicode-org.atlassian.net/browse/CLDR-14925
       */
      final CLDRProgressTask progress = CookieSession.sm.openProgress(
        "vv: Priority Items Summary",
        maxn + 100
      );
    ...
    private void processCriticalWork(final CLDRProgressTask progress) {
      ...
      last = start;
      n = 0;
      vv.setProgressCallback(
        new CLDRProgressCallback(progress, Thread.currentThread())
      );
      ...
      if (myThread.isAlive()) {
        aBuffer.append(
          "<hr/>Processing time: " + ElapsedTimer.elapsedTime(start)
        );
   ...
   */
  private static final String SUM_MESSAGE_COMPLETE = "Stopped on completion";
  private static final String SUM_MESSAGE_STOPPED_ON_REQUEST =
    "Stopped on request";
  private static final String SUM_MESSAGE_PROGRESS = "In Progress";
  private static final String SUM_MESSAGE_STOPPED_STUCK =
    "Stopped (refresh if stuck)";
  private static final String SUM_MESSAGE_NOT_LOADING =
    "Not loading. Click the button to load.";
   ...
   * @throws JSONException
   */
  public void putTaskStatus(JSONObject debugStatus, Task t)
    throws JSONException {
