...
public class WebContext implements Cloneable, Appendable {
   ...
   */
  public WebContext(HttpServletRequest irq, HttpServletResponse irs)
    throws IOException {
  ...
  protected void setRequestResponse(
    HttpServletRequest irq,
    HttpServletResponse irs
  ) throws IOException {
    request = irq;
    response = irs;
  ...
  int prefCodesPerPage() {
    if (codesPerPage == -1) {
      codesPerPage =
        prefInt(SurveyMain.PREF_CODES_PER_PAGE, SurveyMain.CODES_PER_PAGE);
  ...
  public String vurl(
    ...
    return WebContext
      .appendContextVurl(sb, loc, page, strid, queryAppend)
      .toString();
  }
  ...
  public void redirectToVurl(String vurl) {
    println(
      "<a class='vredirect' href='" + vurl + "'>Redirecting to " + vurl + "</a>"
    );
    println(
  ...
  void redirect(String where) {
    try {
      ...
      throw new RuntimeException(
        ioe.toString() + " while redirecting to " + where
      );
    }
  }
  ...
  public void showCoverageLevel() {
    ...
    print(
      "Recommended level: <tt class='codebox'>" +
      recLevel.toString() +
      "</tt><br>"
    );
    print("<ul><li>To change your default coverage level, see ");
    SurveyMain.printMenu(
      this,
      "",
      "options",
      "My Options",
  ...
  public String getEffectiveCoverageLevel(String locale) {
    ...
    if (
      (level == null) ||
      (level.equals(COVLEV_RECOMMENDED)) ||
  ...
  public String getRecommendedCoverageLevel() {
    ...
    if ((myOrg == null) || !isCoverageOrganization(myOrg)) {
      ...
      if (loc != null) {
        Level lev = StandardCodes
          .make()
          .getLocaleCoverageLevel(myOrg, loc.toString());
   ...
   */
  public static final Level COVLEVEL_DEFAULT_RECOMMENDED =
    org.unicode.cldr.util.Level.MODERN;
  public static final String COVLEV_DEFAULT_RECOMMENDED_STRING = COVLEVEL_DEFAULT_RECOMMENDED
    .name()
    .toLowerCase();
  ...
  static String[] getLocaleCoverageOrganizations() {
    Set<Organization> set = StandardCodes
      .make()
      .getLocaleCoverageOrganizations();
  ...
  private static final Map<String, DataSection> dataSectionCache = CACHE_DATA_SECTION
  ...
  static final HelpMessages surveyToolHelpMessages = new HelpMessages(
    "test_help_messages.html"
  );
  ...
  private void printHelpLink(
    ...
    print(
      "<a href=\"" + (SurveyMain.CLDR_HELP_LINK) + what + "\">" + title + "</a>"
    );
    if (parens) {
      println(")");
    }
  ...
  public void includeFragment(String filename) {
    try {
      ...
      this.println("</div>");
      System.err.println(
        "While expanding " + TMPL_PATH + filename + ": " + t.toString()
      );
      t.printStackTrace();
    }
  }
  ...
  public Boolean canModify() {
    if (
      STFactory.isReadOnlyLocale(locale) || SurveyMain.phase() == Phase.READONLY
  ...
  public boolean getCanModify() {
    boolean canModify =
      (UserRegistry.userCanModifyLocale(session.user, getLocale()));
    return canModify;
  }
  ...
  public void setSession() {
    ...
    if (session != null && session.user != null) {
      if (hasField(SurveyMain.QUERY_SAVE_COOKIE)) {
        logger.fine(
          "Remembering user: " + session.user + " for session " + session.id
        );
  ...
  public static void logout(
    ...
    if (session != null) {
      String sessionId = (String) session.getAttribute(
        SurveyMain.SURVEYTOOL_COOKIE_SESSION
      );
  ...
  public static void loginRemember(HttpServletResponse response, User user) {
    addCookie(
      response,
      SurveyMain.QUERY_EMAIL,
      user.email,
      SurveyMain.TWELVE_WEEKS
    );
    addCookie(
      response,
      SurveyMain.QUERY_PASSWORD,
      user.getPassword(),
      SurveyMain.TWELVE_WEEKS
    );
  }
