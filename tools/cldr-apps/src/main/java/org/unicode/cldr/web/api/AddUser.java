...
public class AddUser {
  ...
  private AddUserResponse doAddNewUser(
    AddUserRequest request,
    ...
    u.setPassword(
      UserRegistry.makePassword(u.email + u.org + session.user.email)
    );
    if (
      request.level < 0 || !reg.canSetUserLevel(session.user, u, request.level)
    ) {
