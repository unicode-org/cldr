...
public class Auth {
  ...
  public Response login(
    ...
    if (request.isEmpty()) {
      ...
      final String myEmail = WebContext.getCookieValue(
        hreq,
        SurveyMain.QUERY_EMAIL
      );
      final String myPassword = WebContext.getCookieValue(
        hreq,
    ...
    try {
      ...
      if (!request.isEmpty()) {
        UserRegistry.User user = CookieSession.sm.reg.get(
          request.password,
          request.email,
          userIP
        );
        ...
        final String sessionFromCookie = WebContext.getSessionIdFromCookie(
          hreq
        );
        ...
        if (session == null) {
          ...
          if (
            ...
            return Response
              .status(429, "Too many requests from this IP")
  ...
  public Response lock(
    ...
    String lockResult = CookieSession.sm.reg.lockAccount(
      request.email,
      request.reason,
      WebContext.userIP(hreq)
    );
