...
public class LocaleCompletion {
  ...
  private static final Logger logger = SurveyLog.forClass(
    LocaleCompletion.class
  );
  ...
  static Response getLocaleCompletion(CLDRLocale cldrLocale) {
    ...
    final LocaleCompletionResponse lcr = getLocaleCompletion(
      cldrLocale,
      stFactory
    );
  ...
  static LocaleCompletionResponse getLocaleCompletion(
    final CLDRLocale cldrLocale,
    final STFactory stFactory
  ) {
    final String localeId = cldrLocale.toString(); // normalized
    final Level level = StandardCodes.make().getTargetCoverageLevel(localeId);
    final CheckCLDR.Options options = new CheckCLDR.Options(
      cldrLocale,
    ...
    ElapsedTimer et = new ElapsedTimer(
      "LocaleCompletion:" + options.toString()
    );
    logger.info("Starting LocaleCompletion for " + options.toString());
    final TestResultBundle checkCldr = stFactory.getTestResult(
      cldrLocale,
      options
    );
    ...
    final SupplementalDataInfo sdi = SupplementalDataInfo.getInstance(
      stFactory.getSupplementalDirectory()
    );
    final CoverageLevel2 covLeveller = CoverageLevel2.getInstance(
      sdi,
      localeId
    );
    ...
    for (final String xpath : file) {
      ...
      final boolean statusMissing =
        STFactory.calculateStatus(file, baselineFile, xpath) ==
        VoteResolver.Status.missing;
      ...
      if (hasError) {
        if (
          results.size() == 1 &&
          results.get(0).getSubtype() == Subtype.coverageLevel
        ) {
          lcr.addMissing();
        } else {
          lcr.addError();
        ...
        if (pathLevel.getLevel() < level.getLevel()) {
          final CLDRFile.DraftStatus status = CLDRFile.DraftStatus.forXpath(
            fullPath
          );
          if (
            status == DraftStatus.provisional ||
