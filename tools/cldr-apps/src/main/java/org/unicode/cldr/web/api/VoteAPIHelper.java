...
public class VoteAPIHelper {
    ...
    try {
      ...
      if (args.xpath == null && args.page != null) {
        ...
        if (xp == null) {
          return Response
            .status(404)
            .entity(new STError(ErrorCode.E_BAD_XPATH))
            .build();
        }
        ...
        return new STError(
          ...
          "handleGetRows: need xpath or page, but not both"
        )
          .build();
      }
      final DataSection pageData = DataSection.make(
        pageId,
        null,
        mySession,
        locale,
      ...
      CLDRLocale dcParent = SupplementalDataInfo
        .getInstance()
      ...
      SurveyLog.logException(
        logger,
        t,
        "Trying to load " + args.localeId + " / " + args.xpath
      );
  ...
  private static Candidate[] calculateItems(
    final DataRow r,
    RowResponse.Row row
  ) {
    // Add candidate items
  ...
  private static VoteEntry[] calculateVotes(
    Set<User> votes,
    Map<User, Integer> overrides
  ) {
    ...
    synchronized (mySession) {
      try {
        ...
        if (!r.statusAction.isForbidden()) {
          ...
          if (!r.statusAction.isForbidden()) {
            try {
              final BallotBox<UserRegistry.User> ballotBox = stf.ballotBoxForLocale(
                locale
              );
              if (request.voteLevelChanged == 0) { // treat 0 as null
                request.voteLevelChanged = null;
              }
              ballotBox.voteForValue(
                mySession.user,
                xp,
                val,
  ...
  private static String processValue(
    ...
    String val;
    if (origValue != null) {
      final DisplayAndInputProcessor daip = new DisplayAndInputProcessor(
        locale,
        true
      );
