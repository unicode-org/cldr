...
public class VotingParticipation {
  ...
  public Response getParticipation(
    ...
    for (final Organization o : Organization.values()) {
      ...
      try (
        Connection conn = dbUtils.getAConnection();
        ...
        SurveyLog.logException(
          logger,
          sqe,
          "while checking voting participation for " + o
        );
    ...
    try (Connection conn = dbUtils.getAConnection();) {
      Collection<String> tables = DBUtils.getTables(
        conn,
        Pattern.compile("cldr_vote_value_[0-9]+$").asPredicate()
      ); // skip beta
      for (String table : tables) {
        ...
        try (
          PreparedStatement psParticipation = conn.prepareStatement(
      ...
      SurveyLog.logException(
        logger,
        sqe,
        "while getting versioned participation"
      );
      return new STError(sqe).build();
    }
  ...
  private void appendInterestLocales(
    UserRegistry reg,
    final Set<String> localesWithVetters,
    int theirId
  ) {
    final UserRegistry.User theUser = reg.getInfo(theirId);
    if (
      (theUser.userlevel > UserRegistry.STREET) ||
  ...
  private void appendInterestLocales(
    final Set<String> localesWithVetters,
    final UserRegistry.User theUser
  ) {
  ...
  public static class SurveyParticipation {
    ...
    public void put(int count, String locale, String org) {
      ...
      int allCount =
        count + allParticipationByLocale.computeIfAbsent(locale, ignored -> 0);
  ...
  public static class OrgParticipation {
    ...
    public OrgParticipation(Organization o) {
      ...
      coverageLocales =
        codes.getLocaleCoverageLocales(o).toArray(new String[0]);
    }
