        ...
        for (CLDRLocale loc : SurveyMain.getLocalesSet()) {
            ...
            for (String xp : f) {
                ...
                if (str.contains(q2)) {
                    ...
                    r.matches.add(what.new PathAndValue(xp, str));
                    /*
        ...
        for (String type : sc.getAvailableTypes()) {
            for (String code : sc.getAvailableCodes(type)) {
                ...
                if (code.toLowerCase().equals(q)) {
                    if (what.exact == null) {
                        what.exact = new ArrayList<>();
                    }
                    StandardCodeResult r = what.new StandardCodeResult(type, code);
                    for (String s : v) {
                        r.data.add(what.new StandardCodeResultDataItem(s, true));
                    }
                    what.exact.add(r);
                } else {
                    for (String s : v) {
                        if (s != null && s.toLowerCase().equals(q)) {
                            ...
                            for (String s2 : v) {
                                ...
                                r.data.add(what.new StandardCodeResultDataItem(s2, isWinner));
                            }
                            what.exact.add(r);
                        }
                    }
