...
function showV() {
  ...
  function (
      ...
      this.loadSpecial(
        ...
        function onFailure(err) {
          ...
          params.flipper.flipTo(
            ...
            (loadingChunk = createChunk(msg_fmt, "p", "errCodeMsg"))
          );
          isLoading = false;
        }
      );
    ...
    window.reloadV = function reloadV() {
      ...
      flipper.flipTo(
        pages.loading,
        (loadingChunk = createChunk(cldrText.get("loading"), "i", "loadingMsg"))
      );
      ...
      var shower = function () {
        ...
        if (
          ...
          if (
            ...
            var frag = document.createDocumentFragment();
            frag.appendChild(
              createChunk(cldrText.get("section_help"), "p", "helpContent")
            );
              ...
              if (!json.oldvotes.locale) {
                ...
                if (data.length > 0) {
                  ...
                  for (var k in data) {
                    ...
                    li.appendChild(
                      createChunk("(" + data[k][header.COUNT] + ")")
                    );
                  ...
                  theDiv.appendChild(
                    createChunk(cldrText.get("v_oldvotes_no_old"), "i")
                  ); // TODO fix
                }
              } else {
                ...
                theDiv.appendChild(
                  createChunk(json.oldvotes.localeDisplayName, "h3", "v-title2")
                );
                ...
                if (
                  ...
                  function addOldvotesType(type, jsondata, frag, navChunk) {
                    ...
                    if (
                      ...
                      content.title = title;
                      content.appendChild(
                        createChunk(title, "h2", "v-oldvotes-sub")
                      );
                    }
                  ...
                  if (contestedChunk == null && uncontestedChunk != null) {
                    ...
                    navChunk.appendChild(
                      createChunk(cldrText.get("v_oldvotes_show"))
                    );
                    navChunk.appendChild(
                      createLinkToFn(
                  ...
                  theDiv.appendChild(
                    createChunk(cldrText.get("v_oldvotes_no_old_here"), "i", "")
                  );
                }
              }
            }
          ...
          myLoad(
            ...
            function (json) {
              ...
              if (!verifyJson(json, "mail")) {
                ...
                if (data.length == 0) {
                  listDiv.appendChild(
                    createChunk(cldrText.get("mail_noMail"), "p", "helpContent")
                  );
                } else {
                  for (var ii in data) {
                    var row = data[ii];
                    ...
                    li.onclick = (function (li, row, header) {
                      return function () {
                        ...
                        contentDiv.appendChild(
                          createChunk(row[header.TEXT], "p", "mailContent")
                        );
