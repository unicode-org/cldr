...
function reallyChangeInterestLocales(event) {
  ...
  const xhrArgs = {
    url: getInterestLocalesUrl(interestLocales),
    handleAs: "json",
...
function getInterestLocalesUrl(interestLocales) {
  const p = new URLSearchParams();
  p.append("email", justUser);
  p.append("intlocs", interestLocales);
  // TODO: make this work: /api/intlocs -- use open liberty...
  return cldrStatus.getContextPath() + "/api/intlocs?" + p.toString();
}
