 ...
 * @module search
 */
define("js/special/search.js", ["js/special/SpecialPage.js"], function (
  SpecialPage
) {
  var _super;
  ...
  Page.prototype.show = function show(params) {
    ...
    var showResults = function showResults(searchTerm) {
      ...
      if (results.length == 0) {
        theResult.appendChild(
          createChunk(cldrText.get("searchNoResults", "h3", "searchNoResults"))
        );
      } else {
        for (var i = 0; i < results.length; i++) {
          ...
          if (result.section) {
            ...
            if (result.page) {
              ...
              if (result.strid) {
                codeUrl =
                  "#/" + newLocale + "/" + result.page + "/" + result.strid;
              }
            }
          }
          ...
          if (result.loc) {
            params.exports.appendLocaleLink(
              theLi,
              result.loc,
              locmap.getLocaleInfo(result.loc),
              true
            );
          }
    ...
    var showSearchTerm = function showSearchTerm(searchTerm) {
      if (
        (searchTerm != theResult.last || theResult.loc != newLocale) &&
        searchTerm != null
      ) {
        theResult.last = null;
        theResult.loc = null;
