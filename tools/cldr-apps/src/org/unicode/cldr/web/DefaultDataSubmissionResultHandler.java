...
public class DefaultDataSubmissionResultHandler implements
	...
	public void handleNoPermission(DataRow p, CandidateItem item,
			String what) {
		handleHadError(p);
		ctx.println(" <p class='ferrbox'>Warning: You don't have permission to "+what+" this item: " +"<tt class='codebox'>"+ p.getDisplayName() +"</tt>" + ".</p>");
	}
	...
	public void handleVote(DataRow p, String oldVote, String newVote) {
		if(newVote == null) {
            ...
            ctx.print("<tt class='codebox'>"+ p.getDisplayName() +"</tt>: ");
            ctx.println("<!-- Registering vote for "+p.getXpath()+" - "+p.getLocale()+":" + newVote + " replacing " + oldVote + " --> " + 
	...
	public void handleUnknownChoice(DataRow p, String choice) {
		handleHadError(p);
        ctx.print("<tt class='codebox'>"+ p.getDisplayName() +"</tt>: ");
        ctx.println(ctx.iconHtml("stop","unknown")+"<tt title='"+p.getLocale()+":"+p.getXpath()+"' class='codebox'>" + p.getDisplayName() + "</tt> Note: <i>" + choice + "</i> not supported yet or item not found. <br>");
	}
