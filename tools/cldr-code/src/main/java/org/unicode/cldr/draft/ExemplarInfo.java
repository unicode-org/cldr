...
public class ExemplarInfo {
    ...
    static {
        ...
        // co - Corsican
        languageToExemplars.put(
                "co", new ExemplarInfo("[abc{chj}defg{ghj}hijlmnopqrstuvz]", "[a-z]"));
        // fy - West Frisian
        languageToExemplars.put(
                "fy",
                new ExemplarInfo(
                        "[a  b c d e f g h i y j k l m n o p q r s t u v w x zâ  ê é ô û ú]",
                        "[a-z]"));
        // bho - Bhojpuri
        ...
        languageToExemplars.put(
                "ht",
                new ExemplarInfo("[a{an}b{ch}de{en}èfgijklmnoò{on}{ou}prst{tch}vwyz]", "[a-z]"));
        // jv - Javanese
        languageToExemplars.put(
                "jv",
                new ExemplarInfo(
                        "[a  b c d e é è f g h i j k l m n o p q r s t u v w x y z]", "[a-z]"));
        // la - Latin
        ...
        // su - Sundanese
        languageToExemplars.put(
                "su", new ExemplarInfo("[aeiouépbtdkgcjh{ng}{ny}mnswlry]", "[a-z]"));
        // gn - Guaraní = gug
        languageToExemplars.put(
    ...
    private static final UnicodeMap<String> JapaneseEducationLevels =
            new UnicodeMap<String>()
                    .putAll(
                            new UnicodeSet(
                                    "[一七三-下中九二五人休先入八六円出力十千口右名四土夕大天女子字学小山川左年手文日早月木本村林校森正気水火犬玉王生田男町白百目石空立竹糸耳花草虫見貝赤足車金雨青音]"),
                            "1")
                    .putAll(
                            new UnicodeSet(
    ...
    private static String specialNormalize(String marks, ULocale locale) {
        ...
        marks =
                locale == null
                        ? UCharacter.toLowerCase(marks)
                        : UCharacter.toLowerCase(locale, marks);
    ...
    public static void main(String[] args) {
        ...
        System.out.println(
                ExemplarInfo.make("zh-Hant", missingExemplars).getEducationLevel("\u4e00"));
