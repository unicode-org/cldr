...
/**
 * Immutable class that allows people to fix regex pattern strings to be provide for
 * internationalization support (as per UTS 18 Unicode Regular Expressions). The chief problem with
 * the internationalization of regex engines is that character classes (eg "[\p{alphanum}@#$]") and
 * property support in most regex engines are missing properties, have the wrong values for
 * properties, have out-of-date values for properties, or are missing the syntax to combine
 * properties or character classes (eg to get the assigned characters in the Devanagari block). The
 * reason for supporting more than just Java is so that this can be use in build-time tools for
 * generating fixed regex pattern strings.
 *
 * <p>TODO add options for controlling whether to change \w, \b, etc.
 *
 * <p>TODO be sensitive to COMMENTS
 *
 * <p>TODO add support for (?#)
 *
 * @author markdavis
 */
public class PatternFixer {
     ...
     *
     * <p>The reason for supporting more than just Java is so that this can be use in build-time
     * tools for generating fixed regex pattern strings.
     */
    public enum Target {
        JAVA
    }
    ...
    public static Pattern compile(String regexPattern, int patternOptions) {
        return Pattern.compile(
                new PatternFixer(Target.JAVA).fix(regexPattern, patternOptions), patternOptions);
    }
    ...
    private int parseUnicodeSet(
            String regexPattern,
            UnicodeSetBuilder builder,
