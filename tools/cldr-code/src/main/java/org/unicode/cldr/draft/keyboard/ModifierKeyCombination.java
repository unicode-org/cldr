 ...
 *
 * <p>For example, suppose we have three keys, "A", "B" and "C". Then the boolean expression: {@code
 * A AND !B} means that A must be on, B must be off and C is a don't care.
 *
 * <p>Some keys may have L and R variants (like Control which has a Left-Control and a
 ...
 *
 * <p>For example, suppose we have three keys "A", "B" and "C" and "A" has a left and right variant
 ...
 * </ul>
 *
 * <p>However when printing out the combination, we would exclude the A-Left and A-Right keys
 * because their parent is already included in the ON set. Therefore the output result would be:
 * {@code A+C?}.
 *
 * <p>A slightly different behavior exists for parent modifier keys. When a parent modifier key is
 ...
 *
 * <p>For example, continuing the above example but this time assuming that C has a left and right
...
public final class ModifierKeyCombination implements Comparable<ModifierKeyCombination> {
    ...
    private ModifierKeyCombination(
            ImmutableSet<ModifierKey> onKeys, ImmutableSet<ModifierKey> offKeys) {
     ...
     * both sets are equal.
     *
     * <p>Compares the sets based on the reverse ordering of the natural order imposed by the
