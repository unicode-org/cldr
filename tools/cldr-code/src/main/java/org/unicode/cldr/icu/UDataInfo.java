...
/**
 * This Class is the Java representation of the ICU4C structure UDataInfo which can be found in
 * <I>$icu4c_root</I>/source/common/unicode/udata.h
 *
 * <p>This class is used by LDML2ICUBinaryWriter to store information that must be written in the
 * ICU Binary format.
 *
 * <p>Note that if this data structure ever grows, the getSize() method must be updated.
 *
...
public class UDataInfo {
    ...
    /**
     * Use to signify that this data is in Big Endian form. Currently the only mode supported in
     * Java is Big Endian.
     */
    public static final byte BIGENDIAN = 1;

    /**
     * charsetFamily is equal to this value when the platform is an ASCII based platform. Currently
     * the only mode supported in Java is ASCII This mirrors the ICU4C version in
     * <I>$icu4c_root</I>/source/common/unicode/utypes.h
     */
    public static final byte ASCII_FAMILY = 0;

    /**
     * This is the value for setting sizeofUChar. Currently it is 16 bits (2 bytes). UChar is
     * currently defined in <I>$icu4c_root</I>/source/common/unicode/umachine.h
     */
    public static final byte SIZE_OF_UCHAR = 2;

    /**
     * This field stores the size of this data structure in memory. Add up the size of each part of
     * it.
     */
    public short size;
    ...
    /**
     * This field is used to signify the Endian mode of a system. Choose from the static final int's
     * provided in this class. In Java, there is only one possibility: Big Endian.
     */
    ...
    /** This field stores an identifier for the data format. Array should be of length 4. */
    ...
    public UDataInfo(
        ...
        if (dataFormat.length != 4) {
            throw new IncorrectArrayLengthException(
                    "The byte array 'dataFormat' must be of length 4.");
        }
        if (formatVersion.length != 4) {
            throw new IncorrectArrayLengthException(
                    "The byte array 'formatVersion' must be of length 4.");
        }
        if (dataVersion.length != 4) {
            throw new IncorrectArrayLengthException(
                    "The byte array 'dataVersion' must be of length 4.");
        }
        this.size = size;
    ...
    private static byte[] shortToBytes(short x) {
        ...
        b[0] = (byte) ((x >>> 8) & mask); // shift four bits to the right and fill with zeros, and
        // then bitwise and with
        // the lower byte
        return b;
    }
}
