...
public class CheckMetazones extends CheckCLDR {
    ...
    public CheckCLDR handleCheck(
        ...
        if (value.matches(".*\\p{Nd}.*")) {
            if (!getCldrFileToCheck()
                    .getSourceLocaleID(path, null)
                    .equals(getCldrFileToCheck().getLocaleID())) { // skip
                // if
                // inherited
                // --
                // we
            ...
            result.add(
                    new CheckStatus()
                            ...
                            .setSubtype(Subtype.metazoneContainsDigit) // typically warningType or
                            // errorType
                            .setMessage(
                                    "Metazone name contains digits - translate only the name")); // the message; can be
        ...
        if (isDSTPathForNonDSTMetazone(path)) {
            result.add(
                    new CheckStatus()
                            .setCause(this)
                            .setMainType(CheckStatus.errorType)
                            .setSubtype(Subtype.extraMetazoneString) // typically warningType or
                            // errorType
                            .setMessage(
                                    "Extra metazone string - should only contain standard value for a non-DST metazone"));
        }
        return this;
    }

    /**
     * True if this is a DST path, but a non DST metazone. Such an XPath should not be present in a
     * CLDRFile.
     *
