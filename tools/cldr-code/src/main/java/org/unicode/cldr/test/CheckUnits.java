            ...
            if (!genders.contains(value)) {
                result.add(
                        new CheckStatus()
                                ...
                                .setMessage(
                                        "The gender value for this locale must be one of: {0}",
                                        genders));
            }
        }
                ...
                try {
                    ...
                    result.add(
                            new CheckStatus()
                                    ...
                                    .setMessage(
                                            "Invalid unit pattern, must have min "
                                                    + min
                                                    + " and max "
                                                    + max
                                                    + " distinct placeholders of the form {n}"));
                }
            }
            String idType;
            switch (pathType) {
                case power:
                    {
                        ...
                        if (value != null && "long".contentEquals(width)) {
                            if (DISALLOW_LONG_POWER.containsSome(
                                    fixedValueIfInherited(value, path))) {
                                String unresolvedValue =
                                        getCldrFileToCheck().getUnresolved().getStringValue(path);
                    ...
                    for (String shortUnitId : pathType.sampleComposedShortUnitIds.get(idType)) {
                        ...
                        if (explicitPattern != null) {
                            String composedPattern =
                                    unitId.toString(
                                            cldrFile, width, count, caseVariant, null, false);
                            if (composedPattern != null
                                    && !explicitPattern.equals(composedPattern)) {
                                unitId.toString(
                                        cldrFile,
                                        width,
                                        count,
                                        ...
                                        false); // for debugging
                                final String MESSAGE =
                                        "Mismatched component: «{0}» produces «{1}», but the explicit translation is «{2}»."
                                                + " See "
                                                + CLDRURLS.COMPOUND_UNITS_HELP;
                                result.add(
                                        new CheckStatus()
                                                .setCause(this)
            ...
            if (durationUnitType.contains("h") && !hasHourSymbol) {
                ...
                result.add(
                        new CheckStatus()
                                ...
                                .setMessage(
                                        "The hour indicator should be either h or hh for duration."));
