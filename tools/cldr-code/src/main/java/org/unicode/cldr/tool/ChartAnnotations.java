...
public class ChartAnnotations extends Chart {
    ...
    private static final String MAIN_HEADER =
            "<p>Annotations provide names and keywords for Unicode characters, currently focusing on emoji. "
    ...
    static final UnicodeSet EXTRAS =
            new UnicodeSet()
                    .addAll(
                            Arrays.asList(
                                    "🇪🇺",
    ...
    public void writeSubcharts(Anchors anchors) throws IOException {
        ...
        for (Entry<LanguageGroup, Set<R3<Integer, String, String>>> groupPairs :
            ...
            for (String cp : s.addAllTo(sorted)) {
                tablePrinter.addRow().addCell(cp).addCell(Utility.hex(cp, 4, " "))
                // .addCell(getName(cp))
                ;
            ...
            new Subchart(name + " Annotations", FileUtilities.anchorize(name), tablePrinter)
                    .writeChart(anchors);
        }
    }
    ...
    public static int getRegionalIndicator(int firstCodepoint) {
        return FIRST_REGIONAL <= firstCodepoint && firstCodepoint <= LAST_REGIONAL
                ? firstCodepoint - FIRST_REGIONAL + 'A'
                : -1;
    }
    ...
    private class Subchart extends Chart {
        ...
        public String getExplanation() {
            return MAIN_HEADER
                    ...
                    + "<p>Most short names and keywords that can be constructed with the mechanism in "
                    + LDML_ANNOTATIONS
                    + " are omitted. "
                    + "However, a few are included for comparison: "
                    + Joiner.on(", ").join(EXTRAS.addAllTo(new TreeSet<>()))
                    + ". "
                    + "In this chart, missing items are marked with “"
                    + Annotations.MISSING_MARKER
                    + "”, "
                    + "‘fallback’ constructed items with “"
                    + Annotations.BAD_MARKER
                    + "”, "
                    + "substituted English values with “"
                    + Annotations.ENGLISH_MARKER
                    + "”, and "
                    + "values equal to their parent locale’s values are replaced with "
