...
public class ChartLanguageMatching extends Chart {
    ...
    public void writeContents(FormattedFileWriter pw) throws IOException {
        ...
        for (String type : SDI.getLanguageMatcherKeys()) {
            ...
            for (R4<String, String, Integer, Boolean> row : data) {
                ...
                // All Swiss speakers can read High German -->
    ...
    private String replaceStar(String result) {
        ...
        temp =
                temp.equals(result)
                        ? temp.replace("Xxxx", "any-other-script")
                        : temp.replace("Xxxx", "any-script");
        temp =
                temp.equals(result)
                        ? temp.replace("xxx", "any-other-language")
                        : temp.replace("xxx", "any-language");
        return temp;
    }
}
