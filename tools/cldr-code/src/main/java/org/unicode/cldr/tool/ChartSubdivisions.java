...
public class ChartSubdivisions extends Chart {
    ...
    public void writeContents(FormattedFileWriter pw) throws IOException {
        ...
        Set<String> remainder =
                new HashSet<>(
                        Validity.getInstance()
                                .getStatusToCodes(LstrType.region)
                                .get(Status.regular));
        ...
        for (String container : SDI.getContainersForSubdivisions()) {
            ...
            String region =
                    containerIsRegion
                            ? container
                            : SubdivisionNames.getRegionFromSubdivision(container);
    ...
    private static String getName(String s1) {
        return s1.isEmpty()
                ? ""
                : TransliteratorUtilities.toHTML.transform(CldrUtility.ifNull(EN.get(s1), ""));
    }
}
