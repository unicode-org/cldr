...
public class CompareExemplars {
    ...
    public static void main(String[] args) {
        ...
        for (String s : allExemplars) {
            ...
            if (scriptCode == UScript.INHERITED || scriptCode==UScript.COMMON) { // skip Zyyy, Zinh
                continue;
            }
        ...
        for (Entry<Integer, UnicodeSet> entry : scriptCodeToCharacters.entrySet()) {
            ...
            show(scriptName, "➕CLDR\t➖Allowed", new UnicodeSet(cldr).removeAll(scriptAllowed), charactersToLocales);
            show(scriptName, "➖CLDR\t➕Allowed", new UnicodeSet(scriptAllowed).removeAll(cldr), null);
            show(scriptName, "➕CLDR\t➕Allowed", new UnicodeSet(cldr).retainAll(scriptAllowed), charactersToLocales);
            //show(scriptName, "➖CLDR\t➖Allowed", new UnicodeSet(scriptAllowed).retainAll(cldr), null);
        }
    }

    public static void show(final String scriptName, String title, UnicodeSet allowed, UnicodeRelation<String> charactersToLocales) {
        if (!allowed.isEmpty()) {
    ...
    private static String showLocales(UnicodeSet allowed, UnicodeRelation<String> charactersToLocales) {
        ...
        for (String s : allowed) {
            try {
                ...
                String set = locales.size() > 10 ? "(" + locales.size() + ")"
    ...
    private static void gatherExemplars(
        ...
        for (String locale : factory.getAvailable()) {
            ...
            UnicodeSet aux =
                cldrFile.getExemplarSet(ExemplarType.auxiliary, WinningChoice.WINNING);
