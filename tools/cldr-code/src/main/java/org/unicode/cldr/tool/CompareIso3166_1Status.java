...
public class CompareIso3166_1Status {
    ...
    public static void main(String[] args) {
        ...
        Map<String, Map<LstrField, String>> lstregRegions =
                StandardCodes.getEnumLstreg().get(LstrType.region);
        Map<String, Map<LstrField, String>> lstregRegionsRaw =
                StandardCodes.getLstregEnumRaw().get(LstrType.region);

        Set<String> seen = new HashSet<>();
        M5<CldrStatus, CldrStatus, Iso3166Status, String, Boolean> ordered =
                ChainedMap.of(
        ...
        for (Entry<String, Map<LstrField, String>> entry : lstregRegions.entrySet()) {
            ...
            final Iso3166Status isoStatus2 =
                    CldrUtility.ifNull(isoStatus.get(regionCode), Iso3166Status.out_of_scope);
            ordered.put(
        ...
        for (Entry<CldrStatus, Map<CldrStatus, Map<Iso3166Status, Map<String, Boolean>>>> entry :
                ordered) {
            CldrStatus cldrStatus2 = entry.getKey();
            for (Entry<CldrStatus, Map<Iso3166Status, Map<String, Boolean>>> entry2 :
                    entry.getValue().entrySet()) {
                CldrStatus bcp47Status2 = entry2.getKey();
                for (Entry<Iso3166Status, Map<String, Boolean>> entry3 :
    ...
    private static String setStatus(
            String regionCode,
            final Map<LstrField, String> cldrData,
            Map<String, CldrStatus> cldrStatus,
            boolean showMacro) {
