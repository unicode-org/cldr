...
/**
 * Tool to compare locales, generating tsv in the console. TODO add output filename
 *
 * @author markdavis
 */
public class CompareLocales {
    ...
    private enum MyOptions {
        ...
        // directory(new Params().setHelp("Set the output directory
        // name").setDefault(DEFAULT_DELTA_DIR_NAME).setMatch(".*")),
    ...
    /**
     * Supply list of locales as input args, like en en_001 en_GB Only really useful if there is a
     * shared non-root ancestor. Skips all cases where
     *
     * <ul>
     *   <li>the values are identical in all the locales, and
     *   <li>at least one of the values is not inherited from a different path
     * </ul>
     *
     * @param args
     */
    public static void main(String[] args) {
        ...
        Matcher pathMatcher =
                !MyOptions.pathFilter.option.doesOccur()
                        ? null
                        : Pattern.compile(MyOptions.pathFilter.option.getValue()).matcher("");
        Matcher fileMatcher =
                !MyOptions.fileFilter.option.doesOccur()
                        ? null
                        : Pattern.compile(MyOptions.fileFilter.option.getValue()).matcher("");
        ...
        for (PathHeader ph : sorted) {
            ...
            // locale=da; action=addNew;
            // new_path=//ldml/units/unitLength[@type="long"]/unit[@type="acceleration-g-force"]/unitPattern[@count="one"][@draft="provisional"]; new_value={0} g-kraft
