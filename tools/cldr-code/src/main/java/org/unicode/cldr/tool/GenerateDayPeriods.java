    ...
    public static void main(String[] args) throws IOException {
        try (PrintWriter out =
                FileUtilities.openUTF8Writer(
                        CLDRPaths.GEN_DIRECTORY + "/supplemental", "dayPeriods.xml")) {
            out.println(
            ...
            for (Type type : Type.values()) {
                ...
                for (String locale : SDI.getDayPeriodLocales(type)) {
                    ...
                    Map<DayPeriod, String> localizations =
                            getLocalizations(factory.make(locale, true));
                    for (DayPeriod dayPeriod : DayPeriod.values()) {
                        if ((dayPeriod == DayPeriod.am || dayPeriod == DayPeriod.pm)
                                && !locale.equals("root")) {
                            continue;
                        }
                            ...
                            out.println(
                                    "\t\t\t<dayPeriodRule type=\""
                                            ...
                                            + (span.start == span.end
                                                    ...
                                                    : "\" from=\""
                                                            + format(span.start, false)
                                                            + "\" before=\""
                                                            + format(span.end, true)
                                                            + "\"")
                                            + "/>"
                                            + (localization == null
                                                    ? ""
