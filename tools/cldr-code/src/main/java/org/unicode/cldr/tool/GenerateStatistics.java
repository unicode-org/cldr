...
class GenerateStatistics {
    ...
    public static void generateSize(
            String sourceDir, String logDir, String match, boolean transliterate)
            throws IOException {
        ...
        Set<String> languages = new TreeSet<>(col),
                countries = new TreeSet<>(col),
                draftLanguages = new TreeSet<>(col),
                draftCountries = new TreeSet<>(col);
        Set<Object> nativeLanguages = new TreeSet<>(),
                nativeCountries = new TreeSet<>(),
                draftNativeLanguages = new TreeSet<>(),
        ...
        for (Iterator<String> it = contents.iterator(); it.hasNext(); ) {
            ...
            if (localeID.equals("root")) continue; // skip root
            ...
            if (draft) {
                ...
                addCounts(
                        ...
                        draftLanguages,
                        draftCountries,
                        draftNativeLanguages,
                        draftNativeCountries);
            } else {
                localeCount++;
                addCounts(localeID, false, languages, countries, nativeLanguages, nativeCountries);
            }
            if (false)
                Log.logln(
                        draft
    ...
    private static Set<String> removeSingleLanguagesWhereWeHaveScripts(Set<String> contents) {
        ...
        for (Iterator<String> it = contents.iterator(); it.hasNext(); ) {
            ...
            // if there is a lang_script, then remove everything starting with lang that doesn't
            // have "a" script
            ...
            if (territory.length() != 0
                    && !sc.getGoodAvailableCodes("territory").contains(territory)) {
    ...
    private static String showSet(Set nativeCountries, boolean transliterate, boolean isLanguage) {
        ...
        for (Iterator<String> it = sb.keySet().iterator(); it.hasNext(); ) {
            ...
            for (Iterator<LanguageList> it2 = s.iterator(); it2.hasNext(); ) {
                ...
                if (!localName.equalsIgnoreCase(englishName)) {
                    ...
                    if (transliterate && NON_LATIN.containsSome(localName) && !lang.equals("ja")) {
                        ...
                        try {
                            transName = fixedTitleCase("en", toLatin.transliterate(localName));
                        } catch (RuntimeException e) {
                            System.out.println("\t" + e.getMessage());
                        }
                        if (NON_LATIN.containsSome(transName)) {
                            Log.logln("Can't transliterate " + localName + ": " + transName);
                        } else {
            ...
            if (!needQualifier || qualifierSet.isEmpty()) qualifiers = "";
            else qualifiers = " " + qualifiers; // qualifiers = " (" + qualifiers + ")";

            // fix
            if (englishName.endsWith(", China")) {
                englishName =
                        englishName.substring(0, englishName.length() - ", China".length())
                                + " China";
            }

            result.append(before)
                    .append(TransliteratorUtilities.toHTML.transliterate(englishName + qualifiers))
                    .append(after);
        }
    ...
    private static void addCounts(
            String localeID,
            boolean isDraft,
            Set<String> draftLanguages,
            Set<String> draftCountries,
            Set<Object> draftNativeLanguages,
            Set<Object> draftNativeCountries) {
        ...
        draftNativeLanguages.add(
                new LanguageList(langScript, englishName, fixedTitleCase("en", nativeName)));
        ...
        if (!country.equals("")) {
            ...
            draftNativeCountries.add(
                    new LanguageList(localeID, englishName, fixedTitleCase("en", nativeName)));
        }
    }
        ...
        public boolean isDraft(String localeName) {
            ...
            try {
                ...
                throw new ICUUncheckedIOException(
                        "Failure on " + localeName + ": " + dir + localeName + ".xml", e);
            }
