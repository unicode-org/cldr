...
/**
 * Regenerate the plural files. Related classes are: ShowPlurals.printPlurals(english, null, pw); //
 * for doing the charts PluralRulesFactory.getSamplePatterns(locale2); for the samples (to be moved
 * into data) When you change the plural rules, be sure to look at the minimal pairs in
 * PluralRulesFactory.
 */
public class GeneratedPluralSamples {
    ...
    static SupplementalDataInfo sInfo =
            CLDRConfig.getInstance().getSupplementalDataInfo(); // forward declaration
    ...
    static class Range implements Comparable<Range> {
        ...
        Status getStatus(FixedDecimal ni) {
            ...
            Status status =
                    startValue <= newValue && newValue <= endValue
                            ? Status.inside
                            : endValue + 1 == newValue ? Status.rightBefore : Status.other;
    ...
    static class Ranges {
        ...
        public void trim(int sampleLimit) {
            // limit to a total of sampleLimit ranges, *but* also include have at least one of each
            // fraction length
            for (int i = 0; i < data.length; ++i) {
                if (sampleLimit < 2) {
                    sampleLimit = 2;
    ...
    static class DataSample {
        ...
        public int hashCode() {
            ...
            return count
                    ^ samples.hashCode()
                    ^ Arrays.asList(digitToSample).hashCode()
                    ^ exponentSamples.hashCode();
        }

        public void freeze(String keyword, PluralRules rule) {
            countNoTrailing = noTrailing.size() + exponentSamples.size();
            // System.out.println(sampleType + ", " + keyword + ", " + countNoTrailing + ", " +
            // rule);
        ...
        public boolean computeBoundedWithSize(String keyword, PluralRules rule) {
            ...
            if (keyword.equals("other")) {
                ...
                if (bounded != isKnownBounded) {
                    ...
                    //                        keyword.equals("many") &&
                    // noTrailing.contains(CELTIC_SPECIAL)
                    //                        ? Info.Type.Warning
                    //                                : Info.Type.Error;
                    INFO.add(
                            infoType,
                            sampleType.toString().toLowerCase(Locale.ENGLISH)
    ...
    //                // check to see that the integer values are bounded and that the decimal
    // values are
    //                // once this happens, then the 'and' rule is bounded.
    //
    //                // if the fractional parts must be zero, then this rule is empty for decimals
    // (and thus bounded)
    //                decBounded |= (operand == 'v' || operand == 'w' || operand == 'f' || operand
    // == 't')
    //                        && remainder.equals("is 0");
    //                // if f and t cannot be zero, then this rule is empty for integers (and thus
    // bounded)
    //                intBounded |= (operand == 'f' || operand == 't')
    //                        && (remainder.equals("is 1") || remainder.equals("is not 0")); //
    // should flesh out with parser
    //
    //                if(!atomicRule.contains("mod") && !atomicRule.contains("not")&&
    // !atomicRule.contains("!")) {
    ...
    // HACK because ICU doesn't provide a good way to check for whether a rule depends on the 'e'
    // operand
    ...
    public void add(final PluralRules pluralRules, double n, int v, int e) {
        ...
        System.out.println(
                "{"
                        ...
                        + keyword
                        + ", "
                        + (ni.getVisibleDecimalDigitCount() == 0 ? "integer" : "decimal"));
        add(pluralRules, keyword, ni);
    }

    //    public void add(PluralRules pluralRules, FixedDecimal ni) {
    //        String keyword = pluralRules.select(ni);
    //        System.out.println(ni + " => " + keyword + ", " + (ni.getVisibleDecimalDigitCount() ==
    // 0 ? "integer" : "decimal"));
    //        add(pluralRules, keyword, ni);
    //    }
    ...
    public static String checkForDuplicates(PluralRules pluralRules, FixedDecimal ni) {
        ...
        //            }
        //        }
        //        if (!keywords.contains("other") || keywords.size() > 2) { // should be either
        // {other, x} or {other}
        //            String message = "";
    ...
    public static void main(String[] args) {
        ...
        Matcher localeMatcher =
                !MyOptions.filter.option.doesOccur()
                        ? null
                        : PatternCache.get(MyOptions.filter.option.getValue()).matcher("");
        ...
        for (PluralType type : PluralType.values()) {
            try (TempPrintWriter out =
                ...
                Relation<PluralInfo, String> seenAlready =
                        Relation.of(new TreeMap<PluralInfo, Set<String>>(), TreeSet.class);
                ...
                for (String locale : locales) {
                    ...
                    PluralInfo pluralInfo =
                            testInfo.getSupplementalDataInfo().getPlurals(type, locale);
                    // System.out.println(type + ", " + locale + "=>" + pluralInfo);
                    seenAlready.put(pluralInfo, locale);
                }

                // sort if necessary
                Set<Entry<PluralInfo, Set<String>>> sorted =
                        sortNew
                                ? new LinkedHashSet<>()
                                : new TreeSet<>(
                                        new HackComparator(
                ...
                Relation<GeneratedPluralSamples, PluralInfo> samplesToPlurals =
                        Relation.of(
                                new LinkedHashMap<GeneratedPluralSamples, Set<PluralInfo>>(),
                                LinkedHashSet.class);
                for (Entry<PluralInfo, Set<String>> entry : sorted) {
                    ...
                    for (String keyword : keywords) {
                        ...
                        if (fileFormat) {
                            out.println(
                                    WritePluralRules.formatPluralRule(
                                            keyword, rule, data.toString(), multiline));
                        } else {
                    ...
                    for (Entry<PluralInfo, Set<String>> entry : seenAlready.keyValuesSet()) {
                        ...
                        System.err.println(
                                type
                                        ...
                                        + first
                                        + "\t≣\t"
                                        + Joiner.on(", ").join(remainder));
                    }
                    System.out.println();
                }
                for (Entry<GeneratedPluralSamples, Set<PluralInfo>> entry :
                        samplesToPlurals.keyValuesSet()) {
    ...
    static class HackComparator implements Comparator<Entry<PluralInfo, Set<String>>> {
        ...
        // we get the order of the first items in each of the old rules, and use that order where we
        // can.
        @Override
