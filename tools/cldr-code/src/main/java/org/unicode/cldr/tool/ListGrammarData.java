...
public class ListGrammarData {
    ...
    private enum MyOptions {
        ...
        coverageSkip(
                new Params()
                        .setHelp("filter files by coverage")
                        .setMatch(".*")
                        .setDefault(Level.UNDETERMINED.toString())),
    ...
    public static void main(String[] args) {
        ...
        Matcher coverageSkip =
                Pattern.compile(MyOptions.coverageSkip.option.getValue()).matcher("");
        ...
        for (String localeRaw : factory.getAvailableLanguages()) {
            ...
            Level coverage =
                    StandardCodes.make()
                            .getLocaleCoverageLevel(Organization.valueOf(orgFilter), localeRaw);
            ...
            for (Entry<PathHeader, String> entry : pathHeaderToValue.entrySet()) {
                ...
                String sample =
                        bestMinimalPairSamples.getBestValue(
                                ph.getHeader(), ph.getCode(), shortUnitId);
                String example;
                lines.put(
                        ph.getHeader(),
                        ph.getHeader()
            ...
            if (grammarInfo != null) {
                ...
                if (!genders.isEmpty()) {
                    final Multimap<String, String> genderToUnits =
                            bestMinimalPairSamples.getGenderToUnits();
                    Multimap<String, Integer> genderToUnitCounts = LinkedHashMultimap.create();
                    for (Entry<String, Collection<String>> entry :
                            genderToUnits.asMap().entrySet()) {
                ...
                if (!rawCases.isEmpty()) {
                    Set<String> pluralKeywords =
                            SDI.getPlurals(locale).getPluralRules().getKeywords();
                    final Multimap<Integer, String> uniqueCaseAndCountToUnits =
                            bestMinimalPairSamples.getUniqueCaseAndCountToUnits();
                    final Multimap<String, String> distinctNominativeCaseToUnit =
                            bestMinimalPairSamples.getDistinctNominativeCaseToUnit();

                    Multimap<Integer, Integer> uniqueCaseAndCountToUnitsCounts =
                            LinkedHashMultimap.create();
                    double average = 0;
                    double total = 0;
                    for (Entry<Integer, Collection<String>> entry :
                            uniqueCaseAndCountToUnits.asMap().entrySet()) {
                        uniqueCaseAndCountToUnitsCounts.put(
                                entry.getKey(), entry.getValue().size());
                        average += entry.getKey() * entry.getValue().size();
                        total += entry.getValue().size();
                    }
                    Collection<String> non_distinct =
                            distinctNominativeCaseToUnit.get(
                                    BestMinimalPairSamples.EQUALS_NOMINATIVE);
                    HashSet<String> distinctUnits =
                            new HashSet<>(distinctNominativeCaseToUnit.values());
        ...
        if (!stats.isEmpty()) {
            System.out.println("\n******** Gender/Case Details\n");
            System.out.println(
                    "Type\tLc\tLocale\tCoverage\tCases\tPlurals\tProduct/Gender\tAverage\t%\tCounts\tDetails1\tDistinct\tDetails2");
