...
public class ShowData {
    ...
    private static final UOption[] options = {
        ...
        UOption.create("last", 'l', UOption.REQUIRES_ARG)
                .setDefault(CLDRPaths.LAST_RELEASE_DIRECTORY + "common/main/"),
        ...
        try {
            ...
            cldrFactory =
                    CLDRConfig.getInstance()
                            .getMainAndAnnotationsFactory(); // ignore source directory for now
            english = cldrFactory.make("en", true);
            String lastSourceDir = options[LAST_DIR].value; // Utility.COMMON_DIRECTORY

            Level requiredCoverage =
                    Level.fromString(options[COVERAGE].toString()); // Utility.COMMON_DIRECTORY
            ...
            FileCopier.copy(
                    ShowData.class, "summary-index.css", options[DESTDIR].value, "index.css");
            FileCopier.copy(
                    ShowData.class, "summary-index.html", options[DESTDIR].value, "index.html");
            ...
            Set<String> defaultContents =
                    testInfo.getSupplementalDataInfo().getDefaultContentLocales();
            ...
            for (String locale : locales) {
                ...
                if (locale.startsWith("supplem")
                        || locale.startsWith("character")
                        || locale.equals("root")) {
                    continue;
                }
            ...
            for (Entry<String, Set<String>> group : parentToChildren.keyValuesSet()) {
                ...
                if (file.isNonInheriting()) continue; // skip supplementals
                ...
                CoverageLevel2 coverageLevel =
                        CoverageLevel2.getInstance(testInfo.getSupplementalDataInfo(), locale);
                ...
                getChartTemplate(
                        ...
                        "<script>"
                                ...
                                + "</script>",
                        headerAndFooter,
                        locale.equals("root") ? "Main Charts Index" : null,
                        false);
                ...
                if (true) {
                    pw.println(
                            "<p><b>Aliased/Inherited: </b><a href='"
                                    + locale
                                    + ".html?hide'>Hide</a> <a href='"
                                    + locale
                                    + ".html'>Show </a></p>");
                }
                ...
                Relation<String, String> childValueToLocales =
                        Relation.of(new TreeMap<String, Set<String>>(), TreeSet.class);
                ...
                for (PathHeader prettyPath : prettySet) {
                    ...
                    boolean isInherited =
                            !source.equals(locale); // !CldrUtility.equals(baileyValue, value);
                    if (isInherited) {
                        inheritedCount++;
                    }
                    ...
                    if (!hide) {
                        Relation<String, String> valuesToLocales =
                                pathHeaderToValuesToLocale.get(prettyPath);
                        if (valuesToLocales == null) {
                            pathHeaderToValuesToLocale.put(
                                    prettyPath,
                                    valuesToLocales =
                                            Relation.of(
                    ...
                    pw.println(
                            (hide ? "<tr class='xx'><td" : "<tr><td")
                                    ...
                                    + CldrUtility.getDoubleLinkedText(
                                            Long.toHexString(StringId.getId(path)),
                                            String.valueOf(++count))
                                    + addPart(
                                            oldParts == null ? null : oldParts.getSection(),
                                            prettyPath.getSection())
                                    + addPart(
                                            oldParts == null ? null : oldParts.getPage(),
                                            prettyPath.getPage())
                                    + addPart(
                                            oldParts == null ? null : oldParts.getHeader(),
                                            prettyPath.getHeader())
                                    + addPart(
                    ...
                    if (!childValueToLocales.keyValuesSet().isEmpty()) {
                        ...
                        pw.println(
                                "<td class='info'><a class='rightLink' target='CLDR-ST-DOCS' href='"
                                        + CLDRURLS.CHARTS_URL
                                        + "'>ⓘ</a></td>");
                    }
                    pw.println("</tr>");
                ...
                pw.println(
                        "<tr><td class='a'>Aliased items: </td><td>" + aliasedCount + "</td></tr>");
                pw.println(
                        "<tr><td class='h'>Inherited items:</td><td>"
                                + inheritedCount
                                + "</td></tr>");
                if (skippedCount != 0)
            ...
            if (!TOO_BIG_FOR_GITHUB) {
                ...
                pw.println(
                        "<tr>"
                                ...
                                + "<th>Locales</th>"
                                + "</tr>");
                for (Entry<PathHeader, Relation<String, String>> entry :
                        pathHeaderToValuesToLocale.entrySet()) {
                    PathHeader ph = entry.getKey();
                    Set<Entry<String, Set<String>>> keyValuesSet = entry.getValue().keyValuesSet();
                    String rowspan =
                            keyValuesSet.size() == 1
                                    ? ">"
                                    : " rowSpan='" + keyValuesSet.size() + "'>";
                    pw.append("<tr><td class='g'")
                            .append(rowspan)
                            .append(ph.getSectionId().toString())
                            .append("</td><td class='g'")
                            .append(rowspan)
    ...
    public static void showValue(
            PrintWriter pw, String value, Set<String> locales, boolean isExemplar) {
    ...
    static Set<String> UnicodeScripts =
            Collections.unmodifiableSet(
                    new TreeSet<>(
                            Arrays.asList(
                                    new String[] {
                                        "Arab", "Armn", "Bali", "Beng", "Bopo", "Brai", "Bugi",
                                        "Buhd", "Cans", "Cher", "Copt", "Cprt", "Cyrl", "Deva",
                                        "Dsrt", "Ethi", "Geor", "Glag", "Goth", "Grek", "Gujr",
                                        "Guru", "Hang", "Hani", "Hano", "Hebr", "Hira", "Hrkt",
                                        "Ital", "Kana", "Khar", "Khmr", "Knda", "Laoo", "Latn",
                                        "Limb", "Linb", "Mlym", "Mong", "Mymr", "Nkoo", "Ogam",
                                        "Orya", "Osma", "Phag", "Phnx", "Qaai", "Runr", "Shaw",
                                        "Sinh", "Sylo", "Syrc", "Tagb", "Tale", "Talu", "Taml",
                                        "Telu", "Tfng", "Tglg", "Thaa", "Thai", "Tibt", "Ugar",
                                        "Xpeo", "Xsux", "Yiii"
                                    })));
    ...
    private static Set<String> getScripts(String exemplars, Set<String> results) {
        ...
        for (UnicodeSetIterator it = new UnicodeSetIterator(new UnicodeSet().addAll(exemplars));
                it.next(); ) {
            int cp = it.codepoint;
    ...
    private static void showCollation(
            Factory collationFactory, String locale, ExtractCollationRules collationRules) {
    ...
    private static String showValue(
            boolean showEnglish, String comparisonValue, String mainValue, boolean lastEquals) {
        return !showEnglish
                ? ""
                : comparisonValue == null
                        ? (lastEquals ? "</td><td>=" : "</td><td><i>n/a</i>")
                        : comparisonValue.length() == 0
                                ? "</td><td>&nbsp;"
                                : comparisonValue.equals(mainValue)
                                        ? "</td><td>="
                                        : "</td><td class='e'"
    ...
    private static String getNda(
            Set<String> skipList,
            Map<String, String> nonDistinguishingAttributes,
            CLDRFile file,
            String path,
            String parentFullPath,
            StringBuffer draftRef) {
        draftRef.setLength(0);
        if (parentFullPath != null && !parentFullPath.equals(path)) {
            file.getNonDistinguishingAttributes(
                    parentFullPath, nonDistinguishingAttributes, skipList);
            if (nonDistinguishingAttributes.size() != 0) {
                String parentNda = "";
                for (Iterator<String> it = nonDistinguishingAttributes.keySet().iterator();
                        it.hasNext(); ) {
                    String key = it.next();
    ...
    private static void showChildren(PrintWriter pw, String locale) {
        ...
        for (String sublocale : parentToChildren.get(locale)) {
            ...
            nameToLocale.put(
                    getEnglishLocaleName(sublocale), minimized == null ? sublocale : minimized);
        }
    ...
    public static void getChartTemplate(
            String title,
            String version,
            String header,
