...
public class SubdivisionNames {
    ...
    public static final String SUBDIVISION_PATH_PREFIX =
        ...
        for (String dir : dirs) {
            try {
                XMLFileReader.loadPathValues(
                        CLDRPaths.COMMON_DIRECTORY + dir + "/" + locale + ".xml", data, true);
    ...
    public static String getPathFromCode(String code) {
        // <subdivision type="AD-02">Canillo</subdivision>
        return SUBDIVISION_PATH_PREFIX + "[@type=\"" + code + "\"]";
    }

    public static String getRegionFromSubdivision(String sdCode) {
        return sdCode.compareTo("A") < 0
                ? sdCode.substring(0, 3)
                : sdCode.substring(0, 2).toUpperCase(Locale.ENGLISH);
    }

    public static String getSubregion(String sdCode) {
        return sdCode.compareTo("A") < 0
                ? sdCode.substring(3)
                : sdCode.substring(2).toUpperCase(Locale.ENGLISH);
    }
    ...
    public static void main(String[] args) {
        ...
        for (String locale : annotations.getAvailable()) {
            ...
            /**
             * <subdivision type="gbeng">England</subdivision> <subdivision
             * type="gbsct">Scotland</subdivision> <subdivision type="gbwls">Wales</subdivision>
             */
            System.out.println(
                    locale
                            + " gbeng="
