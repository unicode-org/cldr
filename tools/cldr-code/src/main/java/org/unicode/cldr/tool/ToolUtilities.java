...
/**
 * Utilities for CLDR tools. Not used in Survey Tool. Moved here from CldrUtilities
 *
 * @author srl
 */
public class ToolUtilities {
    ...
    public static void registerExtraTransliterators() {
        ...
        //        String tzadir = CLDRPaths.UTIL_DATA_DIR + File.separatorChar; // work around bad
        ...
        //        // TransliteratorUtilities.registerTransliteratorFromFile(tzadir,
        // "Latin-Ethiopic");
    ...
    public static void generateBat(
        ...
        try {
            ...
            if (!new File(sourceDir, sourceFile).exists()) {
                ...
                if (f.exists()) {
                    if (DEBUG_SHOW_BAT)
                        System.out.println("*Deleting old " + getNormalizedPathString(f));
                    f.delete();
                }
            } else if (!CldrUtility.areFileIdentical(
                    fullSource, fullTarget, failureLines, lineComparer)) {
                PrintWriter bat = FileUtilities.openUTF8Writer(batDir, batName);
                try {
                    bat.println(
                            CLDRPaths.COMPARE_PROGRAM
                                    + " "
                                    + getNormalizedPathString(fullSource)
                                    + " "
                                    + getNormalizedPathString(fullTarget));
                } finally {
                    bat.close();
                }
            } else {
     ...
     * Fetch data from jar
     *
     * @param name a name residing in the org/unicode/cldr/tool/ directory, or loading from a jar
     *     will break.
     */
