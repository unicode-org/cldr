...
public class CaseIterator {
    ...
    // testing stuff
    private static Transliterator toName = Transliterator.getInstance(
        "[:^ascii:] Any-Name"
    );
    private static Transliterator toHex = Transliterator.getInstance(
        "[:^ascii:] Any-Hex"
    );
    private static Transliterator toHex2 = Transliterator.getInstance(
        "[[^\u0021-\u007F]-[,]] Any-Hex"
    );
    ...
    static {
        ...
        for (int i = 0; i <= 0x10FFFF; ++i) {
            ...
            if (
                cat == Character.UNASSIGNED || cat == Character.PRIVATE_USE
            ) continue;
        ...
        if (DUMP) {
            ...
            while (it.hasNext()) {
                ...
                System.out.println(
                    " " +
                    toHex2.transliterate(key) +
                    ": " +
                    toHex2.transliterate(value)
                );
            }
        }
                ...
                if (UTF16.countCodePoint(key) > 2) {
                    ...
                    while (multiIt2.hasNext()) {
                        ...
                        while (true) {
                            ...
                            String[] choices = (String[]) fromCaseFold.get(
                                otherKey
                            );
