...
public class DayPeriodConverter {
    ...
    private static void generateFieldNames() {
        ...
        EnumSet<DayPeriodInfo.DayPeriod> dayPeriodSet = EnumSet.noneOf(
            DayPeriodInfo.DayPeriod.class
        );

        String prefix = getPrefix("stand-alone");

        for (String locale : sdi.getDayPeriodLocales(
            DayPeriodInfo.Type.format
        )) {
            ULocale uLocale = new ULocale(locale);
            DayPeriodInfo dayPeriodInfo = sdi.getDayPeriods(
                DayPeriodInfo.Type.format,
                locale
            );
            ...
            if (!fieldType) {
                ...
                for (String path : cldrFile) {
                    ...
                    if (path.startsWith(prefix)) {
                        ...
                        DayPeriodInfo.DayPeriod period = DayPeriodInfo.DayPeriod.fromString(
            ...
            for (DayPeriodInfo.DayPeriod dayPeriod : dayPeriodSet) {
                if (fieldType) {
                    ...
                    System.out.println(
                        "\t<field type=\"dayPeriod-" + dayPeriod + "\">"
                    );
                    System.out.println(
    ...
    private static void addOldDayPeriod(
        CLDRFile cldrFile,
        DayPeriodInfo.DayPeriod period
    ) {
        String amString = cldrFile.getStringValue(
            getPath("format", "wide", period)
        );
        if (amString != null) {
            ...
            showModLine(
                ...
                period,
                amString,
                !locale.equals("root") && !locale.equals("en")
            );
    ...
    static void generateFormat() {
        SupplementalDataInfo sdi = CLDRConfig
            .getInstance()
            .getSupplementalDataInfo();
        M3<ULocale, Integer, DayPeriodInfo.DayPeriod> allData = ChainedMap.of(
        ...
        for (String locale : sdi.getDayPeriodLocales(
            ...
            DayPeriodInfo data = sdi.getDayPeriods(
                DayPeriodInfo.Type.selection,
                locale
            );
            if (hasMidnight) {
            ...
            if (hasNoon) {
                allData.put(
                    ulocale,
                    12 * DayPeriodInfo.HOUR,
                    DayPeriodInfo.DayPeriod.noon
                );
            }
            int lastTime = -1;
            DayPeriodInfo.DayPeriod lastPeriod = null;
            for (int index = 0; index < data.getPeriodCount(); ++index) {
                R3<Integer, Boolean, DayPeriodInfo.DayPeriod> row = data.getPeriod(
                    index
                );
                ...
                if (!row.get1()) {
                    ...
                    if (time == 12 * DayPeriodInfo.HOUR) {
                        ...
                        lastTime < 12 * DayPeriodInfo.HOUR &&
        ...
        for (Entry<ULocale, Map<Integer, org.unicode.cldr.util.DayPeriodInfo.DayPeriod>> entry : allData) {
            ...
            System.out.println(
                "\t\t<dayPeriodRules locales=\"" + locale + "\">"
            );
            ArrayList<Entry<Integer, DayPeriodInfo.DayPeriod>> list = new ArrayList<>(
                entry.getValue().entrySet()
            );
            for (int i = 0; i < list.size(); ++i) {
                Entry<Integer, org.unicode.cldr.util.DayPeriodInfo.DayPeriod> item = list.get(
                    i
                );
                //System.out.println(item.getKey() + " = " + item.getValue());
                DayPeriodInfo.DayPeriod dayPeriod = item.getValue();
                int start = item.getKey();
                int end = i + 1 == list.size()
                    ? 24 * DayPeriodInfo.HOUR
                    : list.get(i + 1).getKey();
    ...
    private static String getNativeName(
        ...
        DayPeriod otherDayPeriod = DayPeriod.valueOf(
            dayPeriod.toString().toUpperCase(Locale.ENGLISH)
        );
    ...
    private static void check(ULocale locale, DayInfo value) {
        ...
        for (DayPeriod dp : value.data) {
            if (lastDp.compareTo(dp) > 0) {
                if (
                    (
                        lastDp == DayPeriod.NIGHT1 || lastDp == DayPeriod.NIGHT2
                    ) &&
    ...
    private static void check(
        ...
        if (
            value.toNativeName.containsKey(morning2) &&
            !value.toNativeName.containsKey(morning1)
        ) {
            throw new IllegalArgumentException(
