...
public class DayPeriodConverter {
        ...
        public String toString() {
            ...
            for (int i = 0; i < 24; ++i) {
                ...
                if (dayPeriod != lastDayPeriod) {
                    result += ")\n.add(\"" + dayPeriod + "\", \"" + toNativeName.get(dayPeriod) + "\"";
    ...
    static {
        for (String[] x : DayPeriodData.RAW_DATA) {
            ...
            if (old != null && !old.equals(nativeName)) {
                throw new IllegalArgumentException(locale + " inconsistent native name for " + dayPeriod + ", old: «" + old + "», new: «" + nativeName + "»");
            }
            DayPeriod oldDp = data.toDayPeriod.get(nativeName);
            if (oldDp != null && oldDp != dayPeriod) {
                throw new IllegalArgumentException(locale + " inconsistent day periods for name «" + nativeName + "», old: " + oldDp + ", new: " + dayPeriod);
            }
    ...
    private static void showModLine(String locale, DayPeriodInfo.DayPeriod dayPeriod, String name, boolean draft) {
        ...
        System.out.println("locale=" + locale + " ; action=add" + " ; new_value=" + name + " ; new_path=" + path);
    }
    ...
    static String getPath(String context, String width, DayPeriodInfo.DayPeriod dayPeriod) {
        return getPrefix(context) + "/dayPeriodWidth[@type=\"" + width + "\"]" + "/dayPeriod[@type=\"" + dayPeriod + "\"]";
    }
    ...
    static void generateFormat() {
        ...
        M3<ULocale, Integer, DayPeriodInfo.DayPeriod> allData =
                ChainedMap.of(new TreeMap(LanguageGroup.COMPARATOR), new TreeMap(), DayPeriodInfo.DayPeriod.class);
            ...
            for (int i = 0; i < list.size(); ++i) {
                ...
                System.out.println(
                        "\t\t\t<dayPeriodRule type=\""
                                + dayPeriod.toString().toLowerCase(Locale.ENGLISH)
                                + "\" "
                                + (start == end ? "at=\"" + start : startType + "=\"" + start + ":00\" before=\"" + end)
                                + ":00\"/>"
                                + " <!-- "
