...
public class DayPeriodInfo {
    ...
    // the arrays must be in sorted order. First must have start= zero. Last must have end =
    // DAY_LIMIT (and !includesEnd)
    ...
    final Relation<DayPeriod, Span> dayPeriodsToSpans =
            Relation.of(new EnumMap<DayPeriod, Set<Span>>(DayPeriod.class), LinkedHashSet.class);
    ...
    /**
     * For the nth period in the day, returns the start, whether the start is included, and the
     * period ID.
     *
     * @param index
     * @return data
    ...
    public boolean collisionIsError(
        ...
        if ((allowableCollisions.containsKey(dayPeriod1)
        ...
        switch (type1) {
            ...
            case selection:
                {
                    if (fixedOrFlexible.span != null) {
                        if (collisionIsErrorSelection(
                                flexible, fixedOrFlexible.span, sampleError)) {
                            return true;
                        }
                    } else { // flexible
                        for (Span s : dayPeriodsToSpans.get(fixedOrFlexible)) {
    ...
    private boolean collisionIsErrorFormat(
            DayPeriod dayPeriod, Span other, Output<Integer> sampleError) {
