...
public class DayPeriodInfo {
            ...
            for (int i = 1; i < len; ++i) {
                ...
                if (current.start != current.end && last.start != last.end) {
                    if (current.start != last.end) {
                        throw new IllegalArgumentException("Gap or overlapping times:\t" + current + "\t" + last + "\t" + Arrays.asList(locales));
                    }
                }
                    ...
                    for (Span span2 : spanSet) {
                        ...
                        if (start >= end2 && start2 >= end) {
                            throw new IllegalArgumentException(
                                    "Overlapping times for " + dayPeriod + ":\t" + span + "\t" + span2 + "\t" + Arrays.asList(locales));
                        }
                    }
                }
            }
     ...
     * @return
     */
    public boolean collisionIsError(DayPeriodInfo.Type type1, DayPeriod dayPeriod1, Type type2, DayPeriod dayPeriod2, Output<Integer> sampleError) {
