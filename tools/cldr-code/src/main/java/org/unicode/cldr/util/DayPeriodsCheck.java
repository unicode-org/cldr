            ...
            switch (arg) {
                case "groups":
                    ...
                    for (LanguageGroup group : LanguageGroup.values()) {
                        System.out.println(
                            ...
                            group.iso +
                            "\t" +
                            english.getName(group.iso)
                        );
                    }

                    for (LanguageGroup group : LanguageGroup.values()) {
                        for (ULocale loc : LanguageGroup.getLocales(group)) {
                            Level level = sc.getLocaleCoverageLevel(
                                "cldr",
                                loc.toString()
                            );
                            System.out.println(
                                ...
                                loc +
                                "\t" +
                                loc.getDisplayLanguage(ULocale.ENGLISH) +
                                "\t" +
                                level
                            );
                        }
                    ...
                    for (String s : CLDRConfig
                        .getInstance()
                        .getCldrFactory()
                        .getAvailableLanguages()) {
                        ULocale loc = new ULocale(s);
                        loc =
                            loc.getScript().isEmpty()
                                ? loc
                                : new ULocale(loc.getLanguage());
                ...
                case "chart":
                    ...
                    for (ULocale locale : localesToCheck) {
                        ...
                        if (dayPeriods == null) {
                            System.out.println(
                                "No data for locale; not supported"
                            );
                            continue;
                        }
                        System.out.print(
                ...
                case "diff":
                    for (ULocale locale : DayPeriods.getAvailable()) {
                        DayPeriods dayPeriods = DayPeriods.getInstance(locale);
                        DayPeriodsOld dayPeriodsOld = DayPeriodsOld.getInstance(
                            locale
                        );
                        System.out.println(
                            locale.getDisplayName(ULocale.ENGLISH)
                        ...
                        for (int i = 0; i < 24 * HOUR; i += HOUR) {
                            DayPeriod dayPeriod = dayPeriods.get(i);
                            DayPeriodsOld.DayPeriod dayPeriodOld = dayPeriodsOld.get(
                                i
                            );
                            String sampleOld = dayPeriodsOld.getSample(
                                dayPeriodOld
                            );
    ...
    private static void show(
        ...
        if (html) {
            System.out.println(
                ...
                dayPeriod +
                "</td><td>" +
                dayPeriods.getSample(dayPeriod) +
                "</td></tr>"
            );
        } else {
            System.out.println(
