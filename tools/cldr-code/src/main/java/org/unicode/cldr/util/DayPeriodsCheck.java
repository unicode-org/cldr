...
public class DayPeriodsCheck {
    ...
    static final Comparator<Enum<?>> ENUM_COMPARATOR =
            new Comparator<Enum<?>>() {
                @Override
        ...
        for (String arg : args) {
            ...
            switch (arg) {
                case "groups":
                    ...
                    for (String s :
                            CLDRConfig.getInstance().getCldrFactory().getAvailableLanguages()) {
    ...
    private static void show(
            ULocale locale, int start, int limit, DayPeriods dayPeriods, DayPeriod dayPeriod) {
        if (html) {
            System.out.println(
                    "<tr><td>"
                            + format(start)
