...
public class DictionaryStringByteConverter extends StringByteConverter {
  ...
  public DictionaryStringByteConverter(
    ...
    int mBytesPerChar = 0;
    for (
      Iterator<Entry<CharSequence, String>> m = dictionary.getMapping();
      m.hasNext();
    ) {
      Entry<CharSequence, String> entry = m.next();
      // System.out.println("** " + key + "\t\t" + value);
      int bytesPerChar =
        entry.getValue().length() * byteMaker.getMaxBytesPerChar(); // all bytes are generated
      // from last char
  ...
  public int toBytes(
    ...
    while (buffer.length() != 0) {
      ...
      main:while (true) {
        ...
        switch (tempStatus) {
          ...
          case PARTIAL:
            ...
            if (
              stopOnFinalPartial && matcher.getMatchEnd() == buffer.length()
            ) {
      ...
      switch (status) {
        ...
        default:
          bytePosition =
            byteMaker.toBytes(buffer.charAt(0), output, bytePosition);
          buffer.delete(0, 1);
          break;
      }
    }
  ...
  public Appendable fromBytes(
    ...
    if (backMatcher == null) {
      ...
      backMatcher =
        new StateDictionaryBuilder<CharSequence>().make(back).getMatcher();
    }
