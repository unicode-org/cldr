...
public class DictionaryStringByteConverter extends StringByteConverter {
    ...
    public DictionaryStringByteConverter(
            Dictionary<String> dictionary, StringByteConverter byteMaker) {
        super();
        ...
        for (Iterator<Entry<CharSequence, String>> m = dictionary.getMapping(); m.hasNext(); ) {
            ...
            int bytesPerChar =
                    entry.getValue().length()
                            * byteMaker.getMaxBytesPerChar(); // all bytes are generated
            // from last char
    ...
    public Appendable fromBytes(byte[] input, int byteStart, int byteLength, Appendable result) {
        ...
        if (backMatcher == null) {
            Map<CharSequence, CharSequence> back =
                    new TreeMap<>(Dictionary.CHAR_SEQUENCE_COMPARATOR);
            for (Iterator<Entry<CharSequence, String>> m = dictionary.getMapping(); m.hasNext(); ) {
                Entry<CharSequence, String> entry = m.next();
