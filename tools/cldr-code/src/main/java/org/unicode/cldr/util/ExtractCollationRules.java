...
public class ExtractCollationRules {
  ...
  public ExtractCollationRules set(CLDRFile file) {
    ...
    for (
      Iterator it = file.iterator("//ldml/collations", file.getComparator());
      it.hasNext();
    ) {
  ...
  static final String quote(String s) {
    if (needsQuoting == null) {
       ...
       */
      needsQuoting =
        new UnicodeSet("[[:whitespace:][:c:][:z:][:ascii:]-[a-zA-Z0-9]]"); //
    ...
    for (int i = 0; i < s.length(); i += UTF16.getCharCount(cp)) {
      ...
      if (!needsQuoting.contains(cp)) {
        ...
        if (cp == '\'') {
          ...
          if (!needsUnicodeForm.contains(cp)) quoteOperandBuffer.append(
            UTF16.valueOf(cp)
          ); // cp != 0x2028
          else if (cp > 0xFFFF) {
