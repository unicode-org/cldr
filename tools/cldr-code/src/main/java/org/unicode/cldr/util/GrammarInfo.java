...
public class GrammarInfo implements Freezable<GrammarInfo> {
    ...
    public enum GrammaticalFeature {
        ...
        public String getDefault(Collection<String> values) {
            return this == grammaticalGender && values != null && !values.contains("neuter")
                    ? "masculine"
                    : defaultValue;
        }
    ...
    public void add(
            GrammaticalTarget target, GrammaticalFeature feature, GrammaticalScope usage, Collection<String> valueSet) {
        Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>> featureToUsageToValues =
                targetToFeatureToUsageToValues.get(target);
        if (featureToUsageToValues == null) {
            targetToFeatureToUsageToValues.put(target, featureToUsageToValues = new TreeMap<>());
    ...
    public GrammarInfo freeze() {
        if (!frozen) {
            Map<GrammaticalTarget, Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>>> temp =
                    CldrUtility.protectCollection(targetToFeatureToUsageToValues);
    ...
    public void forEach(Handler4<GrammaticalTarget, GrammaticalFeature, GrammaticalScope, String> handler) {
        for (Entry<GrammaticalTarget, Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>>> entry1 :
                targetToFeatureToUsageToValues.entrySet()) {
            GrammaticalTarget target = entry1.getKey();
            final Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>> featureToUsageToValues =
                    entry1.getValue();
    ...
    public static String getGrammaticalInfoAttributes(
            GrammarInfo grammarInfo, UnitPathType pathType, String plural, String gender, String caseVariant) {
        ...
        if (grammarInfo != null && gender != null && pathType.features.contains(GrammaticalFeature.grammaticalGender)) {
            Collection<String> genders =
                    grammarInfo.get(
                            GrammaticalTarget.nominal, GrammaticalFeature.grammaticalGender, GrammaticalScope.units);
    ...
    public static final ImmutableMultimap<String, PluralInfo.Count> NON_COMPUTABLE_PLURALS =
    ...
    public static final Set<String> SEED_LOCALES =
            ImmutableSet.of(
                    "pl", "ru", "da", "de", "nb", "sv", "hi", "id", "es", "fr", "it", "nl", "pt", "en", "ja", "th",
                    "vi", "zh", "zh_TW", "ko", "yue");

    /** TODO: change this to be data-file driven */
    public static final Set<String> SPECIAL_TRANSLATION_UNITS =
            ImmutableSet.of(
                    // new in v38
                    "mass-grain",
                    ...
                    "volume-teaspoon");
    // compounds
    // "kilogram-per-cubic-meter", "kilometer-per-liter", "concentr-gram-per-mole", "speed-mile-per-second",
    // "volumetricflow-cubic-foot-per-second",
