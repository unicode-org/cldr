...
public class GrammarInfo implements Freezable<GrammarInfo> {
    ...
    public enum CaseValues {
        ...
        public static Comparator<String> COMPARATOR = EnumComparator.create(
            CaseValues.class
        );
    }
    ...
    public enum GrammaticalFeature {
        ...
        public static final Pattern PATH_HAS_FEATURE = Pattern.compile(
            "\\[@(count|case|gender|definiteness)="
        );
        ...
        public String getDefault(
            Collection<String> featureValuesFromGrammaticalInfo
        ) {
            return (
        ...
        static final Map<String, GrammaticalFeature> shortNameToEnum = ImmutableMap.copyOf(
            Arrays
                .asList(GrammaticalFeature.values())
                .stream()
    ...
    public void add(
        ...
        Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>> featureToUsageToValues = targetToFeatureToUsageToValues.get(
            target
        );
        if (featureToUsageToValues == null) {
            targetToFeatureToUsageToValues.put(
                target,
                featureToUsageToValues = new TreeMap<>()
            );
        }
        if (feature != null) {
            Map<GrammaticalScope, Set<String>> usageToValues = featureToUsageToValues.get(
                feature
            );
            if (usageToValues == null) {
                featureToUsageToValues.put(
                    feature,
                    usageToValues = new TreeMap<>()
                );
            }
    ...
    public void add(
        ...
        for (String targetString : SupplementalDataInfo.split_space.split(
            targetsRaw
        )) {
            ...
            if (featureRaw == null) {
                ...
                final GrammaticalFeature feature = GrammaticalFeature.valueOf(
                    featureRaw
                );

                List<String> usages = usagesRaw == null
                    ? Collections.singletonList(
                        GrammaticalScope.general.toString()
                    )
                    : SupplementalDataInfo.split_space.splitToList(usagesRaw);

                List<String> values = valuesRaw == null
    ...
    public GrammarInfo freeze() {
        if (!frozen) {
            Map<GrammaticalTarget, Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>>> temp = CldrUtility.protectCollection(
                targetToFeatureToUsageToValues
            );
    ...
    public void forEach(
        ...
        for (Entry<GrammaticalTarget, Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>>> entry1 : targetToFeatureToUsageToValues.entrySet()) {
            ...
            } else for (Entry<GrammaticalFeature, Map<GrammaticalScope, Set<String>>> entry2 : featureToUsageToValues.entrySet()) {
                GrammaticalFeature feature = entry2.getKey();
                for (Entry<GrammaticalScope, Set<String>> entry3 : entry2
                    .getValue()
                    .entrySet()) {
    ...
    public void forEach3(
        Handler3<GrammaticalTarget, GrammaticalFeature, GrammaticalScope, Collection<String>> handler
    ) {
        for (Entry<GrammaticalTarget, Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>>> entry1 : targetToFeatureToUsageToValues.entrySet()) {
            GrammaticalTarget target = entry1.getKey();
            final Map<GrammaticalFeature, Map<GrammaticalScope, Set<String>>> featureToUsageToValues = entry1.getValue();
    ...
    public static String getGrammaticalInfoAttributes(
        ...
        if (pathType.features.contains(GrammaticalFeature.grammaticalNumber)) { // count is special
            grammaticalAttributes +=
                "[@count=\"" + (plural == null ? "other" : plural) + "\"]";
        }
        if (
            grammarInfo != null &&
            gender != null &&
            ...
            if (
                !gender.equals(
                    GrammaticalFeature.grammaticalGender.getDefault(genders)
                )
            ) {
                ...
                switch (desiredCase) {
                    ...
                    case "accusative":
                        switch (gender) {
                            ...
                            case "personal":
                                sourceCase.value =
                                    isNominative ? "vocative" : "genitive";
                                break;
                        }
                        break;
                }
    ...
    static class GrammarLocales {
        ...
        static final Set<String> data = ImmutableSortedSet.copyOf(
            ImmutableSet
                .<String>builder()
                .addAll(
                    CLDRConfig
    ...
    static class UnitsToAddGrammar {
        ...
        static {
            ...
            final UnitConverter converter = config
                .getSupplementalDataInfo()
                .getUnitConverter();
            Set<String> missing = new TreeSet<>();
            Set<String> _data = new TreeSet<>();
            for (String path : With.in(
                config
                    .getRoot()
