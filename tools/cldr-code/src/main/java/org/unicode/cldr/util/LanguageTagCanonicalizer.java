...
public class LanguageTagCanonicalizer implements StringTransform {
  ...
  public synchronized String transform(String locale, OutputOption oo) {
    ...
    copyFields2(
      ...
      getReplacement(
        LanguageTagField.language,
        LanguageTagField.language.get(ltp1),
        locale
      )
    );
    copyFields2(
      ...
      getReplacement(
        LanguageTagField.script,
        LanguageTagField.script.get(ltp1),
        locale
      )
    );
    copyFields2(
      ...
      getReplacement(
        LanguageTagField.region,
        LanguageTagField.region.get(ltp1),
        locale
      )
    );
  ...
  private String getReplacement(
    ...
    if (list.size() != 0) {
      ...
      if (list.size() > 1 && tagField == LanguageTagField.region) {
        LanguageTagParser x = new LanguageTagParser()
          .set(languageTag)
          .setRegion("");
  ...
  private void copyFields2(LanguageTagField mainField, String otherField) {
    ...
    ltp2.set(
      mainField == LanguageTagField.language ? otherField : "und-" + otherField
    );
    if (
      mainField == LanguageTagField.language || ltp1.getLanguage().length() == 0
    ) {
      ltp1.setLanguage(ltp2.getLanguage());
    }
    if (
      mainField == LanguageTagField.script || ltp1.getScript().length() == 0
    ) {
      ltp1.setScript(ltp2.getScript());
    }
    if (
      mainField == LanguageTagField.region || ltp1.getRegion().length() == 0
    ) {
