...
public class LanguageTagParser {
     ...
     * @return Returns each of the language-script tags in the collection.
     */
    public static Set<String> getLanguageAndScript(
    ...
    private Map<String, List<String>> localeExtensions = new TreeMap<>(
        EXTENSION_ORDER
    );
    ...
    private static final UnicodeSet ALPHANUM = new UnicodeSet("[0-9a-zA-Z]")
        .freeze();
    private static final UnicodeSet EXTENSION_VALUE = new UnicodeSet(
        "[0-9a-zA-Z/_]"
    )
        .freeze();
    private static final UnicodeSet X = new UnicodeSet("[xX]").freeze();
    private static final UnicodeSet ALPHA_MINUS_X = new UnicodeSet(ALPHA)
        .removeAll(X)
        .freeze();
    private static StandardCodes standardCodes = StandardCodes.make();
    private static final Set<String> legacyCodes = standardCodes.getAvailableCodes(
        "legacy"
    );
    private static final String separator = "-_"; // '-' alone for 3066bis language tags
    private static final UnicodeSet SEPARATORS = new UnicodeSet()
        .addAll(separator)
        .freeze();
    private static final Splitter SPLIT_BAR = Splitter.on(
        CharMatcher.anyOf(separator)
    );
    ...
    public LanguageTagParser set(String languageTag) {
        ...
        if (atPosition >= 0) {
            final String extensionsString = languageTag
                .substring(atPosition + 1)
                .toLowerCase(Locale.ROOT);
            for (String keyValue : SPLIT_COLON.split(extensionsString)) {
                final Iterator<String> keyValuePair = SPLIT_EQUAL
                    .split(keyValue)
                    .iterator();
                final String key = keyValuePair.next();
                final String value = keyValuePair.next();
                if (
                    keyValuePair.hasNext() ||
                    !ALPHANUM.containsAll(key) ||
                    !EXTENSION_VALUE.containsAll(value)
                ) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                "Illegal language tag: " + languageTag,
                e1
            );
        }
        ...
        try { // The try block is to catch the out-of-tokens case. Easier than checking each time.
            ...
            if (subtag.length() == 4 && ALPHA.containsAll(subtag)) {
                script = subtag;
                script =
                    script.substring(0, 1).toUpperCase(Locale.ROOT) +
                    script.substring(1);
                subtag = getSubtag(st); // prepare for next
            }
            ...
            if (
                subtag.length() == 2 &&
                ALPHA.containsAll(subtag) ||
                subtag.length() == 3 &&
                DIGIT.containsAll(subtag)
            ) {
    ...
    private boolean isValidVariant(String subtag) {
        return (
            subtag != null &&
            ALPHANUM.containsAll(subtag) &&
            (
                subtag.length() > 4 ||
                subtag.length() == 4 &&
                DIGIT.contains(subtag.charAt(0))
            )
        );
    }
    ...
    public Status getStatus(Set<String> errors) {
        ...
        Map<String, Map<String, String>> languageInfo = StandardCodes
            .getLStreg()
            .get("language");
        ...
        String minimized = LikelySubtags.minimize(
            tag,
    ...
    private boolean validates(String subtag, String type) {
        return (
            subtag.length() == 0 ||
            standardCodes.getAvailableCodes(type).contains(subtag)
        );
    }
    ...
    private String getExtension(
        ...
        if (!st.hasMoreElements()) {
            throwError(
                subtag,
                "Private Use / Extension requires subsequent subtag"
            );
        }
        ...
        try {
            ...
            if (takesSubkeys) {
                ...
                if (!haveContents) {
                    throw new IllegalArgumentException(
                        "extension must not be empty: " + base
                    );
                }
            } else {
                if (result.isEmpty()) {
    ...
    private String getSubtag(StringTokenizer st) {
        ...
        if (!ALPHANUM.containsAll(result)) {
            throwError(
                result,
                "Illegal characters (" +
                new UnicodeSet().addAll(result).removeAll(ALPHANUM) +
                ")"
            );
        }
        return result;
    ...
    public enum OutputOption {
        ...
        public Map<String, String> convert(
            Map<String, List<String>> mapToList
        ) {
    ...
    public String toString(OutputOption oo) {
        ...
        if (this.script.length() != 0) result
            .append(oo.separator)
            .append(script);
        if (this.region.length() != 0) result
            .append(oo.separator)
            .append(region);
        ...
        if (this.localeExtensions.size() != 0) {
            if (oo == OutputOption.BCP47) {
                ...
                for (Entry<String, List<String>> extension : localeExtensions.entrySet()) {
                    ...
                    String value = oo.joiner.join(extension.getValue());
                    result
                        .append(key.toUpperCase(Locale.ROOT))
                        .append('=')
    ...
    public static Set<Fields> LANGUAGE_SCRIPT = Collections.unmodifiableSet(
        EnumSet.of(Fields.LANGUAGE, Fields.SCRIPT)
    );
    public static Set<Fields> LANGUAGE_REGION = Collections.unmodifiableSet(
        EnumSet.of(Fields.LANGUAGE, Fields.REGION)
    );
    public static Set<Fields> LANGUAGE_SCRIPT_REGION = Collections.unmodifiableSet(
        EnumSet.of(Fields.LANGUAGE, Fields.SCRIPT, Fields.REGION)
    );

    public String toString(Set<Fields> selection) {
        String result = language;
        if (selection.contains(Fields.SCRIPT) && script.length() != 0) result +=
            "_" + script;
        if (selection.contains(Fields.REGION) && region.length() != 0) result +=
            "_" + region;
    ...
    public LanguageTagParser setLocaleExtensions(
        Map<String, String> localeExtensions
    ) {
        this.localeExtensions =
            expandMap(localeExtensions, 1, Integer.MAX_VALUE);
        return this;
    }
    ...
    static final Pattern EXTENSION_PATTERN = PatternCache.get(
        "([0-9a-zA-Z]{2,8}(-[0-9a-zA-Z]{2,8})*)?"
    );
    ...
    private Map<String, List<String>> expandMap(
        Map<String, String> newLocaleExtensions,
        int minLength,
        int maxLength
    ) {
        ...
        for (Entry<String, String> entry : newLocaleExtensions.entrySet()) {
            result.put(
                entry.getKey(),
                split(entry.getValue(), minLength, maxLength)
            );
        }
        return result.build();
    }
    ...
    private List<String> split(String value, int minLength, int maxLength) {
        ...
        for (String s : values) {
            if (s.length() < minLength || s.length() > maxLength) {
                throw new IllegalArgumentException(
                    "Illegal subtag length for: " + s
                );
            }
            if (!ALPHANUM.containsAll(s)) {
                throw new IllegalArgumentException(
                    "Illegal locale character in: " + s
                );
            }
        }
        return values;
        ...
        if (match == null && format != Format.structure) {
            ...
            if (haveTLang) {
                result
                    ...
                    .append('t')
                    .append(format.separator)
                    .append(Joiner.on(format.separator).join(tLang));
            }
            if (haveUSpecial) {
                result2
                    .append(format.separator)
                    .append('u')
                    .append(format.separator)
                    .append(Joiner.on(format.separator).join(uSpecial));
            }
