  ...
  public LanguageTagParser set(String languageTag) {
    ...
    try { // The try block is to catch the out-of-tokens case. Easier than checking each time.
      ...
      if (subtag.length() == 4 && ALPHA.containsAll(subtag)) {
        script = subtag;
        script =
          script.substring(0, 1).toUpperCase(Locale.ROOT) + script.substring(1);
        subtag = getSubtag(st); // prepare for next
      }
  ...
  private String getExtension(
    ...
    try {
      while (st.hasMoreElements()) {
        ...
        if (
          takesSubkeys && subtag.length() == 2 && (!firstT || isTKey(subtag))
    ...
    if (this.variants.size() != 0) {
      for (String variant : variants) {
        result
          .append(oo.separator)
          .append(
            oo != OutputOption.ICU ? variant : variant.toUpperCase(Locale.ROOT)
          );
      }
    }
