...
public class PathChecker {
    ...
    private static Map<DtdType, Map<String, Map<String, Map<String, ValueStatus>>>> seenEAV =
            new ConcurrentHashMap<>();

    /**
     * Returns true if the path is ok. The detailed errors (if any) are set only the first time the
     * path is seen!
     */
    ...
    public boolean checkPath(
            XPathParts parts, Map<Row.R3<String, String, String>, ValueStatus> errors) {
    ...
    private boolean checkAttribute(
        ...
        Map<String, Map<String, Map<String, ValueStatus>>> elementToAttrToAttrValueToStatus =
                seenEAV.get(dtdData.dtdType);
        if (elementToAttrToAttrValueToStatus == null) {
            Map<String, Map<String, Map<String, ValueStatus>>> subAlready =
                    seenEAV.putIfAbsent(
                            dtdData.dtdType,
        ...
        Map<String, Map<String, ValueStatus>> attrToAttrValueToStatus =
                elementToAttrToAttrValueToStatus.get(element);
        if (attrToAttrValueToStatus == null) {
            Map<String, Map<String, ValueStatus>> subAlready =
                    elementToAttrToAttrValueToStatus.putIfAbsent(
                            element, attrToAttrValueToStatus = new ConcurrentHashMap<>());
            if (subAlready != null) {
        ...
        if (attrValueToStatus == null) {
            Map<String, ValueStatus> setAlready =
                    attrToAttrValueToStatus.putIfAbsent(
                            attribute, attrValueToStatus = new ConcurrentHashMap<>());
            if (setAlready != null) {
        ...
        if (valueStatus == null) {
            ...
            if (valueStatus != ValueStatus.valid) {
                ...
                // https://cldr.unicode.org/development/cldr-development-site/testattributevalues)
