...
public abstract class Pick {
  ...
  private static class Repeat extends ItemPick {
    ...
    private Repeat(int minCount, int maxCount, int[] itemWeights, Pick item) {
      super(item);
      weightedIndex =
        new WeightedIndex(minCount).add(maxCount - minCount + 1, itemWeights);
    }
  ...
  static class Morph extends ItemPick {
    ...
    private static final int COPY_NEW = 0, COPY_BOTH = 1, COPY_LAST = 3, SKIP =
      4, LEAST_SKIP = 4;
      ...
      if (lastValue != null) {
        ...
        while (
          ...
          if (c == COPY_LAST || c == COPY_BOTH || c == SKIP) {
            lastIndex =
              getChar(lastValue, lastIndex, mergeBuffer, c < LEAST_SKIP);
          }
        }
