...
public abstract class Pick {
            ...
            while (true) {
                try {
                    ...
                    for (Pick pick : e.target.stack.elementSet()) {
                        System.out.println(
                            pick.name + ": " + e.target.stack.count(pick)
                        );
                    }
                    int debug = 0;
                }
            ...
            for (int i = index;;) {
                try {
                    ...
                    if (i >= weightedIndex.weights.length) {
                        i -=
                            weightedIndex.weights.length -
                            weightedIndex.minCount;
                    }
                }
            }
        }
        ...
        private Repeat(
            ...
            weightedIndex =
                new WeightedIndex(minCount)
                    .add(maxCount - minCount + 1, itemWeights);
        }
    ...
    private static class CodePoint extends FinalPick {
        ...
        protected void addTo(Target target) {
            target.append(
                source.charAt(pick(target.random, 0, source.size() - 1))
            );
        }
    ...
    static class Morph extends ItemPick {
        ...
        private Target addBuffer = Target.make(
            this,
            null,
        ...
        private static final int COPY_NEW = 0, COPY_BOTH = 1, COPY_LAST =
            3, SKIP = 4, LEAST_SKIP = 4;
