...
public final class PreferredAndAllowedHour implements Comparable<PreferredAndAllowedHour> {
     ...
     * character IDs in the given collection. Note that the iteration order of the allowed styles is
     * retained.
     *
     * <p>This constructor is limiting, since some styles are identified by two character strings,
     * which cannot be referenced via this constructor.
     */
     ...
     * retained, since in some situations it is necessary that the preferred style should also be
     * the first allowed style. The list of allowed style IDs must not contain duplicates.
     */
    ...
    private PreferredAndAllowedHour(String preferredStyle, Stream<String> allowedStyles) {
        ...
        checkArgument(allowed.stream().distinct().count() == allowed.size(), "Allowed (%s) must not contain duplicates", allowed);
        ...
        checkArgument(allowed.contains(preferred), "Allowed (%s) must contain preferred (%s)", allowed, preferred);
    }
    ...
    public String toString(Collection<String> regions) {
        ...
        return "<hours preferred=\"" + preferred + "\" allowed=\"" + withSpaces.join(allowed) + "\" regions=\"" + withSpaces.join(regions) + "\"/>";
    }
