...
public final class Rational implements Comparable<Rational> {
    ...
    public static class RationalParser implements Freezable<RationalParser> {
        ...
        public RationalParser addConstant(
            String id,
            String value,
            String status
        ) {
        ...
        public Rational parse(String input) {
            ...
            List<String> comps = slashSplitter.splitToList(
                input.replace(",", "")
            ); // allow commas anywhere
            try {
                switch (comps.size()) {
                    case 1:
                    ...
                    case 2:
                        return process(comps.get(0))
                            .divide(process(comps.get(1)));
                    default:
                        throw new IllegalArgumentException(
                            "too many slashes in " + input
                        );
                }
            } catch (Exception e) {
        ...
        static final UnicodeSet ALLOWED_CHARS = new UnicodeSet("[-A-Za-z0-9_]")
            .add(REPTEND_MARKER)
            .freeze();
        ...
        private Rational process2(String input) {
            ...
            if (firstChar == '-' || (firstChar >= '0' && firstChar <= '9')) {
                ...
                if (!ALLOWED_CHARS.containsAll(input)) {
                    throw new IllegalArgumentException(
                        "Bad characters in: " + input
                    );
                }
                Rational result = constants.get(input);
                if (result == null) {
                    throw new IllegalArgumentException(
                        "Constant not defined: " + input
                    );
                }
                return result;
            }
    ...
    public Rational subtract(Rational other) {
        ...
        return new Rational(
            numerator
                ...
                .divide(gcd_den)
                .subtract(
                    other.numerator.multiply(denominator).divide(gcd_den)
                ),
            denominator.multiply(other.denominator).divide(gcd_den)
        );
    }
    ...
    public Rational multiply(Rational other) {
        ...
        BigInteger smallNum = isZero
            ? numerator
            : numerator.divide(gcd_num_oden);
        BigInteger smallODen = isZero
            ? other.denominator
            : other.denominator.divide(gcd_num_oden);

        BigInteger gcd_den_onum = denominator.gcd(other.numerator);
        isZero = gcd_den_onum.equals(BigInteger.ZERO);
        BigInteger smallONum = isZero
            ? other.numerator
            : other.numerator.divide(gcd_den_onum);
        BigInteger smallDen = isZero
            ? denominator
            : denominator.divide(gcd_den_onum);

        return new Rational(
            smallNum.multiply(smallONum),
            smallDen.multiply(smallODen)
        );
    }
    ...
    public static Rational of(BigDecimal bigDecimal) {
        ...
        if (scale == 0) {
            ...
            return new Rational(
                unscaled,
                BigDecimal.ONE.movePointRight(scale).toBigInteger()
            );
        } else {
            return new Rational(
                unscaled.multiply(
                    BigDecimal.ONE.movePointLeft(scale).toBigInteger()
                ),
                BigInteger.ONE
            );
        }
    ...
    public String toString(FormatStyle style) {
        ...
        Output<BigDecimal> newNumerator = new Output<>(
            new BigDecimal(numerator)
        );
        final BigInteger newDenominator = minimalDenominator(
            newNumerator,
            denominator
        );
    ...
    static final LocalizedNumberFormatter nf = NumberFormatter
        .with()
        .precision(Precision.unlimited())
        .locale(Locale.ENGLISH);
    static final LocalizedNumberFormatter snf = NumberFormatter
        .with()
        .precision(Precision.unlimited())
    ...
    public boolean equals(Rational that) {
        return (
            numerator.equals(that.numerator) &&
    ...
    public static BigInteger minimalDenominator(
        Output<BigDecimal> outNumerator,
    ...
    public static class ContinuedFraction {
        ...
        public ContinuedFraction(Rational source) {
            ...
            while (true) {
                ...
                Rational remainder = source.subtract(
                    Rational.of(floor, BigInteger.ONE)
                );
                _sequence.add(floor);
        ...
        public ContinuedFraction(long... items) {
            ...
            for (long item : items) {
                if (count != 0 && item < 0) {
                    throw new IllegalArgumentException(
                        "Only first item can be negative"
                    );
                }
    ...
    public String getIntPowerOfTen() {
        ...
        if (numerator.compareTo(BigInteger.ZERO) < 0) {
            throw new IllegalArgumentException(
                "Prefix label must be positive: " + this
            );
        }
        if (
            !numerator.equals(BigInteger.ONE) &&
            !denominator.equals(BigInteger.ONE)
        ) {
            throw new IllegalArgumentException(
                "Prefix label must be power of 10: " + this
            );
        }
        BigInteger value;
        int sign;
