...
public final class Rational implements Comparable<Rational> {
    ...
    private Rational process2(String input) {
      ...
      if (firstChar == '-' || (firstChar >= '0' && firstChar <= '9')) {
        ...
        Rational den = Rational.of(
          BigDecimal.ONE.scaleByPowerOfTen(rlen).subtract(BigDecimal.ONE)
        ); // could optimize
  ...
  public double doubleValue() {
    if (
      denominator.equals(BigInteger.ZERO) && numerator.equals(BigInteger.ZERO)
    ) {
      return Double.NaN;
    }
    return new BigDecimal(numerator)
  ...
  public String toString(FormatStyle style) {
    switch (style) {
      case plain:
        return (
          numerator +
          (denominator.equals(BigInteger.ONE) ? "" : " / " + denominator)
        );
    }
  ...
  public boolean equals(Rational that) {
    return (
      numerator.equals(that.numerator) && denominator.equals(that.denominator)
    );
  }
  ...
  public String getIntPowerOfTen() {
    ...
    if (
      !numerator.equals(BigInteger.ONE) && !denominator.equals(BigInteger.ONE)
    ) {
      throw new IllegalArgumentException(
