...
public class SpecialLocales {
    ...
    public static String getCommentRaw(CLDRLocale l) {
        return getInstance()
            .getCommentInternal(l)
            .replaceAll("@@", "@" + l.getBaseName());
    }
    ...
    private SpecialLocales() {
        ...
        for (final LocaleTransform lt : CLDRFileTransformer.LocaleTransform.values()) {
            if (
                lt.getPolicyIfExisting() !=
                CLDRFileTransformer.PolicyIfExisting.DISCARD
            ) {
                continue;
            }
            // Add each of these as if they were in SpecialLocales.txt
            CLDRLocale inputLocale = CLDRLocale.getInstance(
                lt.getInputLocale()
            );
            CLDRLocale outputLocale = CLDRLocale.getInstance(
                lt.getOutputLocale()
            );
            ...
            comments.put(
                outputLocale,
                "@" +
                outputLocale.getBaseName() +
                " is generated from @" +
                inputLocale.getBaseName() +
                " via transliteration, and so @@ may not be edited directly. Edit @" +
                inputLocale.getBaseName() +
                " to make changes."
            );
        }
        ...
        for (final DataFileRow r : DataFileRow.ROWS) {
            ...
            try {
                ...
                throw new IllegalArgumentException(
                    "Invalid CLDRLocale in SpecialLocales.txt:" + r.id
                );
            }
            ...
            if (r.includeSubLocs) {
                if (r.type == Type.scratch) {
                    throw new IllegalArgumentException(
                        "Scratch locales cannot include sublocales: " + l
                    );
                }
            ...
            if (DEBUG) {
                System.out.println(
                    SpecialLocales.class.getSimpleName() +
                    ": locale " +
                    l +
                    ", includejSublocs=" +
                    r.includeSubLocs +
                ...
                while (true) {
                    ...
                    if (includeSublocs) {
                        id =
                            id.substring(
                                0,
                                id.length() - INCLUDE_SUBLOCALES.length()
                            );
                    }
                    ...
                    try {
                        ...
                        throw new IllegalArgumentException(
                            "Invalid SpecialLocales.Type in SpecialLocales.txt:" +
                            ln +
                            ": " +
                            line
                        );
