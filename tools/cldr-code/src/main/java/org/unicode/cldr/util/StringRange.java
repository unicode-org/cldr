...
public class StringRange {
    ...
    public static void compact(
        ...
        if (!moreCompact) {
            ...
            for (Entry<Integer, Set<Ranges>> entry : lengthToArrays.keyValuesSet()) {
                LinkedList<Ranges> compacted = compact(
                    entry.getKey(),
                    entry.getValue()
                );
    ...
    private static LinkedList<Ranges> compact(
        int size,
        Set<Ranges> inputRanges
    ) {
    ...
    static Collection<String> expand(
        String start,
        String end,
