...
public class TestUtilities {
  ...
  private static void testNames() {
    ...
    for (String test : tests) {
      System.out.println(
        test + "\t" + english.getName(test) + "\t" + french.getName(test)
      );
    }
  }
    ...
    while (true) {
      ...
      if (fixedID == null) {
        if (!timezones.contains(originalTzid)) {
          System.out.println(
            numeric + "\t" + originalTzid + "\tStrange ID: " + fixedID
          );
        }
  ...
  private static void writeMetaData() throws IOException {
    ...
    for (
      Iterator<String> it = element_attribute_valueSet.keySet().iterator();
      it.hasNext();
    ) {
  ...
  public static void testBreakIterator(String text) {
    ...
    if (choice.equals("Word")) b =
      ...
      choice.equals("Line")
    ) b = (RuleBasedBreakIterator) BreakIterator.getLineInstance(); else if (
      choice.equals("Sentence")
    ) b = (RuleBasedBreakIterator) BreakIterator.getSentenceInstance(); else b =
    ...
    while (decimalEscapes.find(start)) {
      ...
      result2.append(
        text.substring(start, decimalEscapes.start()) + UTF16.valueOf(code)
      );
  ...
  private static void checkLanguages() {
    ...
    if (langHack.containsAll(available)) System.out.println("All ok"); else {
  ...
  private static void printCountries() throws IOException {
    ...
    StandardCodes sc = StandardCodes.make();
    for (
      Iterator<String> it = sc.getGoodAvailableCodes("language").iterator();
      it.hasNext();
    ) {
      String code = it.next();
    ...
    out.println("****");
    for (
      Iterator<String> it = sc.getGoodAvailableCodes("territory").iterator();
      it.hasNext();
    ) {
      String code = it.next();
