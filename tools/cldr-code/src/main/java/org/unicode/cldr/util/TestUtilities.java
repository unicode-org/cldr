...
public class TestUtilities {
    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            System.out.println(
                State.a + ", " + State.b + ", " + State.c + ", " + State.cc
            );
            ...
            long foo = UniversalTimeScale.from(
                new Date().getTime(),
                UniversalTimeScale.JAVA_TIME
            );
            System.out.println(
                "Current Universal Time: " + Long.toString(foo, 16)
            );
            System.out.println(
                "LVT_Syllable count: " +
                new UnicodeSet("[:Hangul_Syllable_Type=LVT_Syllable:]").size()
            );
            System.out.println(
                "LV_Syllable count: " +
                new UnicodeSet("[:Hangul_Syllable_Type=LV_Syllable:]").size()
            );
            System.out.println(
                "AC00 value: " +
    ...
    private static void testNames() {
        Factory mainCldrFactory = Factory.make(
            CLDRPaths.COMMON_DIRECTORY + "main" + File.separator,
            ".*"
        );
    ...
    private static void testExampleGenerator() throws IOException {
        ...
        ExampleGenerator englishExampleGenerator = new ExampleGenerator(
            english,
            english,
        ...
        Relation<String, String> message_paths = Relation.of(
        ...
        for (String path : english) {
            ...
            result.setLength(0);
            String examples = englishExampleGenerator.getExampleHtml(
                path,
                value
            );
            if (examples != null) {
        ...
        for (String message : message_paths.keySet()) {
            ...
            out.println("<p>");
            out.println(
                CldrUtility.join(paths, "<br>" + CldrUtility.LINE_SEPARATOR)
            );
    ...
    private static void checkNumericTimezone() throws IOException {
        ...
        while (true) {
            ...
            if (fixedID == null) {
                ...
                System.out.println(
                    "Replacing " + originalTzid + " with " + fixedID
                );
            }
    ...
    private static void checkTranslit() {
        ...
        for (
            ...
            out.println(
                com.ibm.icu.impl.Utility.hex(s, 4) +
                " (" +
                literal +
                ") " +
    ...
    private static void checkTranslit(String string) {
        ...
        if (!reverse.equals(string)) System.out.println(
            string +
            "\t=>\t" +
            html +
            "\t=>\t" +
            reverse +
            (!reverse.equals(string) ? " FAIL" : "")
        );
        String htmlAscii = TransliteratorUtilities.toHTMLAscii.transliterate(
            string
        );
        String reverseAscii = TransliteratorUtilities.fromHTML.transliterate(
            htmlAscii
        );
        if (!reverseAscii.equals(string)) System.out.println(
            string +
            "\t=>\t" +
            htmlAscii +
            "\t=>\t" +
    ...
    private static void writeMetaData() throws IOException {
        CLDRFile meta = SimpleFactory
            .makeFile("metaData")
            .setNonInheriting(true);
        ...
        String[] serialElements = new String[] {
            "variable",
            "comment",
            "tRule",
            // collation
        ...
        GenerateAttributeList attributes = new GenerateAttributeList(
            cldrFactory
        );
        Map<String, Map<String, Set<String>[]>> element_attribute_valueSet = attributes.getElement_attribute_valueSet();
        for (
            Iterator<String> it = element_attribute_valueSet
                .keySet()
                .iterator();
            it.hasNext();
        ) {
            String element = it.next();
            Map<String, Set<String>[]> attribute_valueSet = element_attribute_valueSet.get(
                element
            );
            int size = attribute_valueSet.size();
            if (size == 0) continue;
            for (
                Iterator<String> it2 = attribute_valueSet.keySet().iterator();
                it2.hasNext();
            ) {
        ...
        String[] dayValueOrder = new String[] {
            "sun",
            "mon",
            "tue",
            "wed",
        ...
        meta.add(
            prefix +
            "valid/attributeValues[@order=\"given\"][@attributes=\"type\"][@elements=\"" +
            "day" +
            "\"]",
        ...
        meta.add(
            prefix +
            "valid/attributeValues[@order=\"given\"][@attributes=\"" +
            "day" +
            "\"][@elements=\"" +
            "firstDay weekendEnd weekendStart" +
            "\"]",
            String.join(" ", dayValueOrder)
        );
        ...
        meta.add(
            ...
            String.join(" ", widths) +
            "\"]",
            String.join(" ", widthOrder)
        );

        String[] formatLengths = {
            "dateFormatLength",
            "timeFormatLength",
            "dateTimeFormatLength",
            "decimalFormatLength",
        ...
        meta.add(
            prefix +
            "valid/attributeValues[@order=\"given\"][@attributes=\"type\"][@elements=\"" +
            String.join(" ", formatLengths) +
            "\"]",
            String.join(" ", lengthOrder)
        );

        String[] dateFieldOrder = new String[] {
            "era",
            "year",
            "month",
            "week",
        ...
        meta.add(
            prefix +
            "valid/attributeValues[@order=\"given\"][@attributes=\"type\"][@elements=\"field\"]",
        ...
        xfr.read(
            CLDRPaths.GEN_DIRECTORY + "meta/metaData.xml",
    ...
    public static void testBreakIterator(String text) {
        ...
        if (choice.equals("Word")) b =
            (RuleBasedBreakIterator) BreakIterator.getWordInstance(); else if (
            choice.equals("Line")
        ) b =
            (RuleBasedBreakIterator) BreakIterator.getLineInstance(); else if (
            choice.equals("Sentence")
        ) b =
            (RuleBasedBreakIterator) BreakIterator.getSentenceInstance(); else b =
            (RuleBasedBreakIterator) BreakIterator.getCharacterInstance();

        Matcher decimalEscapes = PatternCache
            .get("&#(x?)([0-9]+);")
            .matcher(text);
        ...
        while (decimalEscapes.find(start)) {
            ...
            result2.append(
                text.substring(start, decimalEscapes.start()) +
                UTF16.valueOf(code)
            );
    ...
    private static void checkStandardCodes() {
        ...
        for (Iterator<String> it = m.keySet().iterator(); it.hasNext();) {
            ...
            for (
                Iterator<String> it2 = subtagData.keySet().iterator();
                it2.hasNext();
            ) {
                String tag = it2.next();
        ...
        for (Iterator<String> it = m.keySet().iterator(); it.hasNext();) {
            ...
            for (
                Iterator<String> it2 = oldAndNotNew.iterator();
                it2.hasNext();
            ) {
                String tag = it2.next();
                List<String> sdata = sc.getFullData(oldType, tag);
    ...
    private static void checkLanguages() {
        ...
        if (langHack.containsAll(available)) System.out.println(
            "All ok"
        ); else {
    ...
    private static void printCountries() throws IOException {
        ...
        PrintWriter out = FileUtilities.openUTF8Writer(
            ...
            "country_language_names.txt"
        );
        StandardCodes sc = StandardCodes.make();
        for (
            Iterator<String> it = sc
                .getGoodAvailableCodes("language")
                .iterator();
            it.hasNext();
        ) {
            String code = it.next();
            out.println(
                code + "\t" + english.getName(CLDRFile.LANGUAGE_NAME, code)
            );
        }
        out.println("****");
        for (
            ...
            String code = it.next();
            out.println(
                code + "\t" + english.getName(CLDRFile.TERRITORY_NAME, code)
            );
        }
        out.println("****");
        for (
            Iterator<String> it = sc.getGoodAvailableCodes("script").iterator();
            it.hasNext();
        ) {
            String code = it.next();
            out.println(
                code + "\t" + english.getName(CLDRFile.SCRIPT_NAME, code)
            );
        }
        out.close();
    }
    ...
    private static void printZoneSamples() throws Exception {
        ...
        for (int i = 0; i < locales.length; ++i) {
            ...
            TimezoneFormatter tzf = new TimezoneFormatter(
                mainCldrFactory,
                locale,
                false
            )
            ...
            for (int j = 0; j < zones.length; ++j) {
                ...
                for (int k = 0; k < fields.length; ++k) {
                    ...
                    for (int m = 1; m < fields[k].length; ++m) {
                        ...
                        String formatted = tzf.getFormattedZone(
                            zone,
                            field,
                            datetime.getTime(),
                            false
                        ...
                        String parsed = tzf.parse(
                            formatted,
                            parsePosition,
                            offsetMillis
                        );
                        if (parsed == null) parsed = "FAILED PARSE"; else if (
                            parsed.length() == 0
    ...
    private static String format(long offsetMillis) {
        ...
        return (
            sign +
            String.valueOf(offsetMillis / 60) +
            ":" +
