...
public class TestUtilities {
    ...
    public static void main(String[] args) throws Exception {
        try {
            ...
            System.out.println(
                "AC00 value: " + UCharacter.getIntPropertyValue('\uAC00', UProperty.HANGUL_SYLLABLE_TYPE)
            );
            // checkTranslit();
            // writeMetaData();
            // testXMLFileReader();
    ...
    private static void checkTranslit(String string) {
        ...
        if (!reverseAscii.equals(string)) System.out.println(
            string + "\t=>\t" + htmlAscii + "\t=>\t" + reverseAscii + (!reverseAscii.equals(string) ? " FAIL" : "")
        );
    }
    ...
    private static void writeMetaData() throws IOException {
        ...
        meta.add(
            prefix + "valid/attributeValues[@order=\"given\"][@attributes=\"type\"][@elements=\"" + "day" + "\"]",
        ...
        meta.add(
            prefix + "valid/attributeValues[@order=\"given\"][@attributes=\"type\"][@elements=\"field\"]",
    ...
    public static void testBreakIterator(String text) {
        ...
        if (choice.equals("Word")) b = (RuleBasedBreakIterator) BreakIterator.getWordInstance(); else if (
            choice.equals("Line")
        ) b = (RuleBasedBreakIterator) BreakIterator.getLineInstance(); else if (choice.equals("Sentence")) b =
    ...
    private static void checkStandardCodes() {
        ...
        for (Iterator<String> it = m.keySet().iterator(); it.hasNext();) {
            ...
            for (Iterator<String> it2 = subtagData.keySet().iterator(); it2.hasNext();) {
                ...
                if (!description.equals(newDescription)) {
                    System.out.println(
                        type + "\t" + tag + "\tDescriptions differ: {" + description + "} ### {" + newDescription + "}"
                    );
                }
                if (deprecated != newDeprecated) {
                    System.out.println(
                        type + "\t" + tag + "\tDeprecated differs: {" + deprecated + "} ### {" + newDeprecated + "}"
                    );
                }
            }
        }
        ...
        for (Iterator<String> it = m.keySet().iterator(); it.hasNext();) {
            ...
            System.out.println(
                "\t\t\t<variable id=\"$" + oldType + "\" type=\"list\">" + allCodeString + "</variable>"
            );
        }
    }
    ...
    private static void checkLanguages() {
        ...
        if (langHack.containsAll(available)) System.out.println("All ok"); else {
            ...
            for (Iterator<String> it = available.iterator(); it.hasNext();) {
                String item = it.next();
                System.out.println(
                    "{\"" + item + "\", \"XXX\"},/t//" + ULocale.getDisplayLanguage(item, ULocale.ENGLISH)
                );
            }
        }
    }
            ...
            for (int j = 0; j < zones.length; ++j) {
                ...
                for (int k = 0; k < fields.length; ++k) {
                    ...
                    for (int m = 1; m < fields[k].length; ++m) {
                        ...
                        if (parsed == null) parsed = "FAILED PARSE"; else if (parsed.length() == 0) parsed =
                            format(offsetMillis[0]);
                        out.println(
                            "{\"" +
                            locale +
                            "\",\t\"" +
    ...
    private static String format(long offsetMillis) {
        ...
        return (
            sign + String.valueOf(offsetMillis / 60) + ":" + String.valueOf(100 + (offsetMillis % 60)).substring(1, 3)
        );
    }
