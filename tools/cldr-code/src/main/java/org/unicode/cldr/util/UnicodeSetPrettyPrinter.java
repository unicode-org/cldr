...
public class UnicodeSetPrettyPrinter {
    ...
    private static final UnicodeSet PATTERN_WHITESPACE =
            new UnicodeSet("[[:Cn:][:Default_Ignorable_Code_Point:][:patternwhitespace:]]").freeze();
    ...
    public String format(UnicodeSet uset) {
        ...
        //                throw new IllegalStateException("Failure to round-trip in pretty-print " + uset + " => " +
        // sresult + Utility.LINE_SEPARATOR + " source-result: " + new UnicodeSet(uset).removeAll(doubleCheck) +
        // Utility.LINE_SEPARATOR + " result-source: " + new UnicodeSet(doubleCheck).removeAll(uset));
        //            }
        //        } catch (RuntimeException e) {
        //            throw (RuntimeException) new IllegalStateException("Failure to round-trip in pretty-print " +
        // uset).initCause(e);
        //        }
        return sresult;
    }
