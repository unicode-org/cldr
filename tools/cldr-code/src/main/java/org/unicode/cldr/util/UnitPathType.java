...
public enum UnitPathType {
    ...
    unit(
            "kilometer",
            EnumSet.of(
                    GrammarInfo.GrammaticalFeature.grammaticalNumber, GrammarInfo.GrammaticalFeature.grammaticalCase),
            null),
    perUnit("minute", null, null),
    times(
            null,
            null,
            ImmutableMultimap.<String, String>builder().put("", "newton-meter").put("", "kilowatt-hour").build()),
    per(
            null,
            null,
            ImmutableMultimap.<String, String>builder().put("", "meter-per-second").put("", "mile-per-gallon").build()),
    prefix(
            null,
            null,
    ...
    private UnitPathType(
            String sampleType,
            Set<GrammarInfo.GrammaticalFeature> features,
            ImmutableMultimap<String, String> sampleComposedLongUnits) {
    ...
    public String getTranslationPath(
        ...
        switch (pathType) {
            ...
            case prefix:
                longUnitId =
                        CLDRConfig.getInstance().getSupplementalDataInfo().getUnitConverter().getLongId(shortUnitId);
                return pathPrefix + "compoundUnit[@type=\"" + longUnitId + "\"]/unitPrefixPattern";
            case power:
                longUnitId = uc.getLongId(shortUnitId);
                grammarInfo1 =
                        CLDRConfig.getInstance().getSupplementalDataInfo().getGrammarInfo(resolvedFile.getLocaleID());
                grammaticalAttributes =
                        GrammarInfo.getGrammaticalInfoAttributes(
                                grammarInfo1, pathType, pluralCategory, genderVariant, caseVariant);
                return pathPrefix
                        + "compoundUnit[@type=\""
                        + longUnitId
    ...
    public String getTrans(
        ...
        String path =
                pathType.getTranslationPath(
                        resolvedFile, width, shortUnitId, pluralCategory, caseVariant, genderVariant);
