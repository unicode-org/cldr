...
public class UnitPreferences implements Freezable<UnitPreferences> {
    ...
    private static final Splitter SPLIT_SPACE = Splitter
        .on(' ')
        .trimResults()
        .omitEmptyStrings();
    ...
    public void add(
        ...
        if (usageToRegionsToInfo == null) {
            quantityToUsageToRegionsToInfo.put(
                quantity,
                usageToRegionsToInfo = new TreeMap<>()
            );
        }
        Multimap<Set<String>, UnitPreference> regionsToInfo = usageToRegionsToInfo.get(
            usage
        );
        if (regionsToInfo == null) {
            usageToRegionsToInfo.put(
                usage,
                regionsToInfo = LinkedHashMultimap.create()
            );
        }
        Rational newGeq = geq == null || geq.isEmpty()
            ? Rational.ONE
            : Rational.of(geq);
        final UnitPreference newUnitPref = new UnitPreference(
            newGeq,
            unit,
            skeleton
        );
        ...
        regionsToInfo.put(
            ImmutableSet.copyOf(
                new TreeSet<>(SPLIT_SPACE.splitToList(regions))
            ),
            newUnitPref
        );
    }
    ...
    public String toString() {
        ...
        for (Entry<String, Map<String, Multimap<Set<String>, UnitPreference>>> entry1 : quantityToUsageToRegionsToInfo.entrySet()) {
            ...
            for (Entry<String, Multimap<Set<String>, UnitPreference>> entry2 : entry1
                .getValue()
                .entrySet()) {
                String usage = entry2.getKey();
                for (Entry<Set<String>, Collection<UnitPreference>> entry : entry2
                    .getValue()
                    .asMap()
                    .entrySet()) {
        ...
        for (Entry<String, Map<String, Multimap<Set<String>, UnitPreference>>> entry1 : quantityToUsageToRegionsToInfo.entrySet()) {
            ...
            for (Entry<String, Multimap<Set<String>, UnitPreference>> entry2 : entry1
                ...
                for (Entry<Set<String>, Collection<UnitPreference>> entry : entry2
                    ...
                    Set<String> regions = entry.getKey();
                    for (UnitPreference up : entry.getValue()) {
                        String unit = SPLIT_AND
                            .split(up.unit)
                            .iterator()
                            .next(); // first unit
                        quantity = converter.getQuantityFromUnit(unit, false);
                        String baseUnit = converter.getBaseUnitFromQuantity(
                            quantity
                        );
                        Rational geq = converter.parseRational(
                            String.valueOf(up.geq)
                        );
                        Rational value = converter.convert(
                            geq,
                            unit,
                            baseUnit,
                            false
                        ...
                        UnitPreference up2 = new UnitPreference(
                            value,
                            up.unit,
                            up.skeleton
                        );
