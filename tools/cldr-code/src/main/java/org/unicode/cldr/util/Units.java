...
public class Units {
    ...
    private static final UnicodeSet WHITESPACE = new UnicodeSet(
        ...
        .freeze();
    public static Pattern NO_SPACE_PREFIX = Pattern.compile(
        "\\}" +
    ...
    public static String combinePattern(
        ...
        boolean lowercaseUnitIfNoSpaceInCompound
    ) {
        // meterFormat of the form {0} meters or {0} Meter
        // compoundPattern is of the form Z{0} or Zetta{0}

        // extract the unit
        String modUnit = (String) SPACE_SPANNER.trim(
            unitFormat.replace("{0}", "")
        );
        Object[] parameters = { modUnit };

        String modFormat = unitFormat.replace(
            modUnit,
            MessageFormat.format(compoundPattern, parameters)
        );
        ...
        // hack to fix casing
        if (
            lowercaseUnitIfNoSpaceInCompound &&
            NO_SPACE_PREFIX.matcher(compoundPattern).find()
        ) {
            modFormat =
                modFormat.replace(modUnit, modUnit.toLowerCase(Locale.ENGLISH));
        }

        return modFormat;
    }

    static final UnicodeSetSpanner SPACE_SPANNER = new UnicodeSetSpanner(
        WHITESPACE
    );
    ...
    static {
        Set<String> VALID_UNITS = Validity
            .getInstance()
