...
public class Unlocode {
    ...
    public static class LocodeData
        implements Mergeable<LocodeData>, Comparable<LocodeData> {
        ...
        public LocodeData merge(LocodeData other) {
            ...
            throw new IllegalArgumentException(
                "Can't merge " + this + " with " + other
            );
        }
    }

    static Map<String, LocodeData> locodeToData = new HashMap<>();
    static Relation<String, LocodeData> nameToLocodeData = Relation.of(
        new HashMap<String, Set<LocodeData>>(),
        HashSet.class
    );
    static Map<String, Iso3166_2Data> iso3166_2Data = new HashMap<>();
    static Relation<String, String> ERRORS = Relation.of(
    ...
    static {
        ...
        try {
            ...
            // load exceptions
            try {
                BufferedReader br = FileReaders.openFile(
                    CldrUtility.class,
    ...
    public static void loadIso() throws IOException {
        ...
        while (true) {
            ...
            String locode =
                list[SubdivisionFields.Code_3166_2.ordinal()].trim();
            if (locode.endsWith("*")) {
    ...
    public static void load(int file) throws IOException {
        ...
        M3<String, String, Boolean> nameToAlternate = ChainedMap.of(
        ...
        while (true) {
            ...
            LocodeData locodeData = new LocodeData(
                locode,
                name,
                subdivision,
                latN,
    ...
    public static <K, V extends Mergeable<V>> void putCheckingDuplicate(
        ...
        if (old != null && !old.equals(value)) {
            try {
                ...
                ERRORS.put(
                    key.toString(),
                    "Can't merge records: " + key + "\t" + e.getMessage()
                );
            }
        } else {
            map.put(key, value);
    ...
    public static void main(String[] args) throws IOException {
        Relation<String, LocodeData> countryNameToCities = Relation.of(
            new TreeMap<String, Set<LocodeData>>(),
            TreeSet.class
        );
        Set<String> errors = new TreeSet<>();
        loadCitiesCapitals(countryNameToCities, errors);
        loadCitiesOver1M(countryNameToCities, errors);
        SupplementalDataInfo supp = ToolConfig
            .getToolInstance()
        ...
        for (Entry<String, LocodeData> entry : countryNameToCities.keyValueSet()) {
            ...
            LinkedHashSet<String> remainingNames = new LinkedHashSet<>(
                item.names
            );
    ...
    public static void showLocodes(String title, Set<String> already) {
        ...
        for (String locode : already) {
            ...
            String countryName = ULocale.getDisplayCountry(
                "und-" + upperLocode.substring(0, 2),
                ULocale.ENGLISH
            );
    ...
    public static int loadCitiesOver1M(
        ...
        BufferedReader br = FileReaders.openFile(
            CldrUtility.class,
            "data/external/Cities-Over1M.txt"
        );
        main:while (true) {
            String line = br.readLine();
            if (line == null) {
            ...
            if (bracket > 0) {
                try {
                    subdivision =
                        cityName.substring(bracket + 1, cityName.indexOf(']'));
    ...
    public static int loadCitiesCapitals(
        ...
        BufferedReader br = FileReaders.openFile(
            CldrUtility.class,
            "data/external/Cities-CountryCapitals.txt"
        );
        while (true) {
            String line = br.readLine();
            if (line == null) {
    ...
    static final Set<String> noncountries = new HashSet<>(
        Arrays.asList(
    ...
    static final Transform<String, String> REMOVE_ACCENTS = Transliterator.getInstance(
        "nfd;[:mn:]remove"
    );
    ...
    static void add(
        ...
        String countryCode = CountryCodeConverter.getCodeFromName(
            countryName,
            false
        );
        if (countryCode == null) {
        ...
        countryName =
            ULocale.getDisplayCountry("und-" + countryCode, ULocale.ENGLISH);
        Set<LocodeData> locodeDatas = nameToLocodeData.get(cityName);
        if (locodeDatas == null) {
