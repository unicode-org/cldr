...
/**
 * Small helper class; given a file with a specified syntax, extract variables and paths, with
...
public class VariableAndPathParser<E, F> {
    ...
    public static interface ExtractablePath<E, F> {
        /**
         * Given a DOM Node, extract the information contained therein, and return it as a Key-Value
         * pair
         *
    ...
    public Map<String, String> getVariables(String varsXPath) {
        ...
        if (variableExtractor != null) {
            try {
                xpExpParser.iterateThroughNodeSet(
                        varsXPath,
                        new NodeHandlingInterface() {

                            @Override
                            public void handle(Node aNode) {
                                Map.Entry<String, String> curVar =
                                        variableExtractor.extractVariable(aNode);
                                if (curVar != null) {
