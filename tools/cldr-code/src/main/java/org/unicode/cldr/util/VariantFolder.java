...
public class VariantFolder {
    ...
    public Set<String> getClosure(String source) {
        ...
        for (int i = 0; i < stringLength; ++i) {
            combos[0][i] = alternateFetcher.getAlternates(source.substring(i, i + 1), new HashSet<String>());
        }
        for (int level = 1; level < stringLength; ++level) {
            ...
            for (int start = 0; start < stringLength - level; ++start) {
                ...
                current.addAll(alternateFetcher.getAlternates(source.substring(start, limit), new HashSet<String>()));
                // then we add the cross product of shorter strings
                for (int breakPoint = start + 1; breakPoint < limit; ++breakPoint) {
                    addCrossProduct(combos[breakPoint - start - 1][start], combos[limit - breakPoint - 1][breakPoint], current);
                }
            }
        }
