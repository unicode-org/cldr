...
public class VariantFolder {
    ...
    public static SetMaker mySetMaker =
            new SetMaker() {
                Comparator c = new UTF16.StringComparator(true, false, 0);
                Comparator bestIsLowest =
                        new Comparator() {
                            @Override
                            public int compare(Object o1, Object o2) {
                                ...
                                final boolean canonical1 =
                                        Normalizer.isNormalized(s1, Normalizer.COMPOSE, 0);
                                final boolean canonical2 =
                                        Normalizer.isNormalized(s2, Normalizer.COMPOSE, 0);
    ...
    public interface AlternateFetcher {
        /**
         * The input string MUST be in the output set. Note that the results must be valid even if
         * input string might not be on even code point boundaries. For example, if the input is
         * "XabY" where X and Y are have surrogates, and the alternates are by case, then the
         * results have to be {"XabY", "XAbY", "XaBY", "XABY"}.
         *
    ...
    /**
     * The class is designed to be immutable, at least as far as Java allows. That is, if the
     * alternateFetcher is, then it will be.
     *
    ...
    public Set<String> getClosure(String source) {
        ...
        for (int i = 0; i < stringLength; ++i) {
            combos[0][i] =
                    alternateFetcher.getAlternates(
                            source.substring(i, i + 1), new HashSet<String>());
        }
        for (int level = 1; level < stringLength; ++level) {
            ...
            for (int start = 0; start < stringLength - level; ++start) {
                ...
                current.addAll(
                        alternateFetcher.getAlternates(
                                source.substring(start, limit), new HashSet<String>()));
    ...
    private void addCrossProduct(
            Collection<String> source1, Collection<String> source2, Collection<String> output) {
