...
public class VerifyCompactNumbers {
    ...
    enum MyOptions {
        ...
        MyOptions(
            ...
            option =
                myOptions.add(this, argumentPattern, defaultArgument, helpText);
        }
    }

    // later, look at DateTimeFormats to set up as an HTML table

    public static final Set<String> USES_GROUPS_OF_4 = new HashSet<>(
        Arrays.asList("ko", "ja", "zh", "zh_Hant")
    );
    ...
    public static void main(String[] args) throws IOException {
        ...
        Set<String> availableLanguages = new TreeSet<>(
            factory2.getAvailableLanguages()
        );
        if (Pattern.matches(filter, "pt_PT")) {
            availableLanguages.add("pt_PT");
        }

        PrintWriter plainText = FileUtilities.openUTF8Writer(
            DIR,
            "compactTestFile.txt"
        );
        ...
        for (String locale : availableLanguages) {
            ...
            Level level = StandardCodes
                .make()
                .getLocaleCoverageLevel(organization, locale);
            ...
            String title =
                "Verify Number Formats: " + englishCldrFile.getName(locale);
            out.println(
            ...
            CLDRFile cldrFile = factory2.make(
                locale,
                true,
        ...
        try {
            ...
            if (showCurrency) {
                tablePrinter1
                    ...
                    .setCellAttributes(
                        "class='dtf-s'"
                    )//                    .setHeaderAttributes("class='dtf-th'") //                    .addColumn("Compact-Short<br>+Unit")
                //                    .setCellAttributes("class='dtf-s'")
            ...
            ICUServiceBuilder builder = new ICUServiceBuilder()
                .setCldrFile(cldrFile);
            NumberFormat nf = builder.getNumberFormat(1);

            // nf.setMaximumFractionDigits(0);
            SupplementalDataInfo sdi = CLDR_CONFIG.getSupplementalDataInfo();
            String[] debugOriginals = null;
            CompactDecimalFormat cdf = BuildIcuCompactDecimalFormat.build(
                cldrFile,
                debugCreationErrors,
                debugOriginals,
                CompactStyle.SHORT,
            ...
            CompactDecimalFormat cdfs = BuildIcuCompactDecimalFormat.build(
                cldrFile,
                debugCreationErrors,
                debugOriginals,
                CompactStyle.LONG,
            ...
            Set<Double> allSamples = collectSamplesAndSetFormats(
                currencyCode,
                locale,
                sdi,
                cdf,
            ...
            try {
                for (double source : allSamples) {
                    ...
                    String compactFormattedNumber = cdf == null
                        ? "n/a"
                        : cdf.format(source);
                    String compactLongFormattedNumber = cdfs == null
                        ? "n/a"
                        : cdfs.format(source);
                    String compactCurrFormattedNumber = !showCurrency ||
                        cdfs == null
                        ? "n/a"
                        : cdfCurr.format(source);
                    ...
                    if (showCurrency) {
                        tablePrinter1.addCell(
                            compactCurrFormattedNumber
                        )//                             .addCell(cdfsCurr.format(source)) //                            .addCell(cdfU.format(source))
            ...
            ShowPlurals showPlurals = new ShowPlurals(
    ...
    private static String surveyUrl = CLDR_CONFIG.getProperty(
        "CLDR_SURVEY_URL",
        "http://st.unicode.org/cldr-apps/survey"
    );

    private static void showErrors(Set<String> errors, Appendable out)
        throws IOException {
        if (errors.size() != 0) {
    ...
    private static Set<Double> collectItems(
