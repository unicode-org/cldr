...
public class XMLNormalizingLoader {
  ...
  private static LoadingCache<XMLSourceCacheKey, XMLSource> cache = CacheBuilder
    .newBuilder()
    ...
    public XMLSourceCacheKey(
      ...
      if (dirs == null || dirs.isEmpty()) {
        throw new ICUUncheckedIOException(
          "Attempt to create a XMLSourceCacheKey with a null directory, please supply a non-null one."
        );
      }
      ...
      for (File dir : dirs) {
        if (!dir.canRead()) {
          throw new ICUUncheckedIOException(
            "The directory specified, " + dir.getPath() + ", cannot be read"
          );
        }
        _dirs.add(dir);
      }
      ...
      this.hashCode =
        Objects.hash(this.localeId, this.dirs, this.minimalDraftStatus);
    }
  ...
  public static XMLSource getFrozenInstance(
    ...
    XMLSourceCacheKey key = new XMLSourceCacheKey(
      localeId,
      dirs,
      minimalDraftStatus
    );
  ...
  private static XMLSource makeXMLSource(XMLSourceCacheKey key) {
    ...
    for (File dir : key.dirs) {
      dirList.clear();
      dirList.add(dir);
      XMLSourceCacheKey singleKey = new XMLSourceCacheKey(
        key.localeId,
        dirList,
        key.minimalDraftStatus
      );
  ...
  public static XMLSource loadXMLFile(
    File f,
    String localeId,
    DraftStatus minimalDraftStatus
  ) {
    ...
    try (InputStream fis = new FileInputStream(f);) {
      ...
      XMLNormalizingHandler XML_HANDLER = new XMLNormalizingHandler(
        source,
        minimalDraftStatus
      );
      ...
      source.setNonInheriting(
        XML_HANDLER.supplementalStatus == SupplementalStatus.NOT_SUPPLEMENTAL
      );
  ...
  private static class XMLNormalizingHandler implements AllHandler {
    ...
    private SupplementalStatus supplementalStatus =
      SupplementalStatus.NEVER_SET;
    private static final int MAX_DEPTH = 30; // just make deep enough to handle any CLDR file.
    ...
    private static final Set<String> CHANGED_TYPES = new HashSet<>(
      Arrays.asList(
        new String[] {
    ...
    private static final Pattern DRAFT_PATTERN = PatternCache.get(
      "\\[@draft=\"([^\"]*)\"\\]"
    );
    private static final Pattern WHITESPACE_WITH_LF = PatternCache.get(
      "\\s*\\u000a\\s*"
    );
    private Matcher draftMatcher = DRAFT_PATTERN.matcher("");
    private Matcher whitespaceWithLf = WHITESPACE_WITH_LF.matcher("");
    private static final UnicodeSet CONTROLS = new UnicodeSet("[:cc:]")
      .freeze();
    ...
    private void push(String qName, Attributes attributes) {
      ...
      if (attributes.getLength() > 0) {
        ...
        for (int i = 0; i < attributes.getLength(); ++i) {
          ...
          if (attribute.equals("cldrVersion") && (qName.equals("version"))) {
            ((SimpleXMLSource) source).setDtdVersionInfo(
      ...
      Log.logln(
        LOG_PROGRESS,
        "currentFullXPath\t" + currentFullXPathSb.toString()
      );
    }
    ...
    private void putAndFixDeprecatedAttribute(
      ...
      if (attribute.equals("draft")) {
        ...
        if (
          CHANGED_TYPES.contains(element) &&
          supplementalStatus != SupplementalStatus.NOT_SUPPLEMENTAL
        ) { // measurementSystem for example did not
    ...
    private void warnOnOverride(String former, String formerPath) {
      ...
      System.out.println(
        ...
        CldrUtility.LINE_SEPARATOR +
        "\tnew fullpath: " +
        currentFullXPathSb.toString()
      );
      System.err.println(
        new XMLSource.SourceLocation(documentLocator) + "Location of error"
      );
      overrideCount += 1;
    }
    ...
    public void startElement(
      ...
      try {
        if (supplementalStatus == SupplementalStatus.NEVER_SET) { // set by first element
          ...
          supplementalStatus =
            source.getXMLNormalizingDtdType() == DtdType.ldml
              ? SupplementalStatus.IS_SUMPPLEMENTAL
