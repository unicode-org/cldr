...
public class XMLNormalizingLoader {
    ...
    private static LoadingCache<XMLSourceCacheKey, XMLSource> cache =
            CacheBuilder.newBuilder()
                    .maximumSize(CACHE_LIMIT)
                    .softValues() // will garbage-collected in LRU manner in response to memory
                    // demand
                    .build(
    ...
    private static XMLSource makeXMLSource(XMLSourceCacheKey key) {
        ...
        // if contains more than one file, make XMLSource from each file and then combine them to a
        // combined XMLSource,
        ...
        for (File dir : key.dirs) {
            dirList.clear();
            dirList.add(dir);
            XMLSourceCacheKey singleKey =
                    new XMLSourceCacheKey(key.localeId, dirList, key.minimalDraftStatus);
    ...
    public static XMLSource loadXMLFile(File f, String localeId, DraftStatus minimalDraftStatus) {
        ...
        try (InputStream fis = new FileInputStream(f); ) {
            ...
            XMLNormalizingHandler XML_HANDLER =
                    new XMLNormalizingHandler(source, minimalDraftStatus);
    ...
    private static class XMLNormalizingHandler implements AllHandler {
        ...
        // orderedCounter, orderedString, and level logically form a single class that allows adding
        // elements, but never removed.
        ...
        private static final Set<String> CHANGED_TYPES =
                new HashSet<>(
                        Arrays.asList(
                                new String[] {
        ...
        private void putAndFixDeprecatedAttribute(String element, String attribute, String value) {
            if (attribute.equals("draft")) {
                ...
                if (CHANGED_TYPES.contains(element)
                        && supplementalStatus
                                != SupplementalStatus
                                        .NOT_SUPPLEMENTAL) { // measurementSystem for example did
                    // not
        ...
        private void warnOnOverride(String former, String formerPath) {
            ...
            System.err.println(new XMLSource.SourceLocation(documentLocator) + "Location of error");
            overrideCount += 1;
        }
