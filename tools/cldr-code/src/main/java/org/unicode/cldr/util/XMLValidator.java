...
public class XMLValidator {
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (args.length == 0) {
      System.out.println(
        "No files specified. Validation failed. Use --help for help."
      );
      return;
    }
    ...
    int failCount = new XMLValidator(quiet, parseonly, justCheckBom)
      .check(toCheck);
    if (failCount != 0) {
      System.err.println(
        "# FAIL: " +
  ...
  private static void addDirectory(File f, List<File> toCheck)
    throws IOException {
        ...
        public boolean accept(File arg0, String arg1) {
          ...
          if (n.isDirectory()) {
            try {
              ...
              e.printStackTrace();
              System.err.println(
                "Error " + e.toString() + " parsing " + arg0.getPath()
              );
            }
            return false;
    ...
    try {
      ...
      try {
        ...
        throw new RuntimeException(
          filename + ": XMLComparator not HTML parser!"
        );
      } catch (Exception e) {
        if (filename != null) {
