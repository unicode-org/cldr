...
public class ZoneFormats {
    ...
    public String formatGMT(TimeZone currentZone) {
        ...
        SimpleDateFormat dateFormat = icuServiceBuilder.getDateFormat(
            "gregorian",
            hourFormatPlusMinus[tzOffset >= 0 ? 0 : 1]
        );
        String hoursMinutes = dateFormat.format(
    ...
    public String getExemplarCity(String timezoneString) {
        String exemplarCity = cldrFile.getWinningValue(
    ...
    public String getMetazoneName(
        String metazone,
        ZoneFormats.Length length,
        ZoneFormats.Type typeIn
    ) {
        ZoneFormats.Type type = typeIn == Type.genericOrStandard
            ? Type.generic
            : typeIn;
        String name = cldrFile.getWinningValue(
