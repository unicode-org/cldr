...
public class ZoneInflections implements Comparable<ZoneInflections> {
    ...
    // to
    private static final int currentYear = Calendar
        .getInstance()
        .get(Calendar.YEAR);
    ...
    public ZoneInflections(TimeZone zone) {
        ...
        inflectionPoints.add(
            new InflectionPoint(endDate, zone.getOffset(endDate))
        );
        ...
        inflectionPoints.add(
            new InflectionPoint(startDate, zone.getOffset(startDate))
        );
    }
    ...
    public int compareTo(ZoneInflections other, OutputLong mostRecentDateTime) {
        ...
        if (minLength < that.inflectionPoints.size()) minLength =
            that.inflectionPoints.size();
        for (int i = 0; i < minLength; ++i) {
            InflectionPoint ip1 = get(i);
            InflectionPoint ip2 = that.get(i);
            if (
                ip1.offset == ip2.offset && ip1.utcDateTime == ip2.utcDateTime
            ) continue;
            if (ip1.offset != ip2.offset) {
                // back up a bit
                mostRecentDateTime.value =
                    Math.max(ip1.utcDateTime, ip2.utcDateTime) - EPSILON;
    ...
    public static class InflectionPoint implements Comparable<InflectionPoint> {
        ...
        public String toString() {
            return (
                ICUServiceBuilder.isoDateFormat(new Date(utcDateTime)) +
                ";" +
                formatHours(offset)
            );
        }
