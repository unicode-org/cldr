...
public class BagFormatter {
    ...
    private void addMatching(String src, String pattern, String substitute, StringBuffer result) {
        ...
        while (true) {
            ...
            first = false;
            if (s1.equals(s2)) result.append(substitute); else result.append(s1);
        }
    }
    ...
    private class MyVisitor extends Visitor {
        ...
        public void toOutput(String s) {
            ...
            output.print(s);
            if (isHtml) output.println("</p>"); else output.print(lineSeparator);
        }
        ...
        public void doAt(Object c, PrintWriter out) {
            ...
            if (DEBUG) System.out.println(
                "Tabber: " +
                tabber.process("200C..200D\t; White_Space\t #\tCf\t [2]\t ZERO WIDTH NON-JOINER..ZERO WIDTH JOINER")
            );
            doAt(c);
        }
        ...
        protected void doSimpleAt(Object o) {
            if (o instanceof Map.Entry) {
                ...
                String label = getLabelSource(true) == UnicodeLabel.NULL
                    ? ""
                    : getLabelSource(true).getValue(thing, ",", true);
                if (label.length() != 0) label = " " + label;
                toTable(hex(thing) + value + commentSeparator + label + insertLiteral(thing) + "\t" + getName(thing));
                counter++;
            }
        }
        ...
        private void showLine(int start, int end) {
            ...
            if (labelSize > 0) {
                ...
                throw new IllegalArgumentException(
                    "maxwidth bogus " + label + ", " + getLabelSource(true).getMaxWidth(shortLabel)
                );
            }

            String count = "";
            if (mergeRanges && showCount) {
                if (end == start) count = "\t"; else count = "\t [" + nf.format(end - start + 1) + "]";
            }
    ...
    private class RangeFinder {
        ...
        boolean next() {
            ...
            if (DEBUG && 0x3FFD < limit && limit < 0x9FD6) {
                System.out.println(
                    Utility.hex(limit) + ", Label: " + label + ", Value: " + value + ", Break: " + breaker
                );
            }
            limit++;
    ...
    public static final Transliterator hex = Transliterator.getInstance("[^\\u0009\\u0020-\\u007E\\u00A0-\\u00FF] hex");
