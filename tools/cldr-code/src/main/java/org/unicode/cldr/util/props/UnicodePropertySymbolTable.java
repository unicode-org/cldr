...
public class UnicodePropertySymbolTable extends UnicodeSet.XSymbolTable {
    ...
    public boolean applyPropertyAlias0(
        ...
        if (canonicalName.equals("General_Category")) {
            String[] parts = GC_REMAP.get(
                UnicodeProperty.toSkeleton(propertyValue)
            );
            if (parts != null) {
        ...
        if (
            ...
            String fixedRegex = unicodeRegex.transform(
        ...
        if (
            ...
            String otherPropName = propertyValue
                .substring(1, propertyValue.length() - 1)
                .trim();
