...
2. Next is a bullet entry `-` with a regex in backticks, matching a set of xpaths.
    Each regex gets `^//ldml/` inserted at the beginning automatically.
    The bracket in `[@` is interpreted literally rather than as regex; that is, a backslash is inserted automatically.
...
3. The final section is the markdown body, continuing up to but not including the next line beginning with `#`.
   ...
   Also note, if you want a literal `{0}` in the Markdown (and who wouldn’t?), go ahead, use `{0}`, `{1}` etc.
