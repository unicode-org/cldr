...
public class CheckLanguageCodeConverter {
    public static void main(String[] args) {
        ...
        for (Entry<String, String> codeName :
                LanguageCodeConverter.getLanguageNameToCode().entrySet()) {
        ...
        Map<String, R2<List<String>, String>> languageAliases =
                supplementalDataInfo.getLocaleAliasInfo().get("language");
        ...
        for (Entry<String, R2<List<String>, String>> languageAlias : languageAliases.entrySet()) {
            ...
            if (LanguageCodeConverter.getExceptionCodes().contains(goodCode)) {
                ...
                String googleCode = LanguageCodeConverter.toGoogleLocaleId(goodCode);
                addLine(lines, badCode, goodCode, googleCode, cldrName);
            }
        }
        for (Entry<String, String> entry : LanguageCodeConverter.GOOGLE_CLDR.entrySet()) {
            String googleCode = entry.getKey();
            String goodCode = LanguageCodeConverter.toHyphenLocale(entry.getValue());
            String cldrName = getName(english, goodCode);
        ...
        for (Entry<String, R2<List<String>, String>> languageAlias : languageAliases.entrySet()) {
            ...
            if ("macrolanguage".equals(reason)) {
                macroToEncompassed.put(
                        languageAlias.getValue().get0().get(0), languageAlias.getKey());
            }
        }
        ...
        for (String code : LanguageCodeConverter.getExceptionCodes()) {
            ...
            System.out.println(
                    LanguageCodeConverter.toHyphenLocale(code)
                            + "\t"
                            + getName(english, code)
                            + "\t"
                            + script
                            + encompassed);
        }
    ...
    private static class LanguageLine extends R5<String, String, String, String, String> {
        public LanguageLine(String a, String b, String c, String d) {
            super(
                    LanguageCodeConverter.toHyphenLocale(a),
                    b,
                    LanguageCodeConverter.toUnderbarLocale(a),
                    c,
                    d);
        }
    ...
    public static void addLine(
        ...
        lines.add(
                new LanguageLine(
                        goodCode,
                        LanguageCodeConverter.toUnderbarLocale(goodCode),
                        googleCode,
                        cldrName));
        lines.add(
                new LanguageLine(
                        goodCode,
                        LanguageCodeConverter.toUnderbarLocale(badCode),
                        googleCode,
                        cldrName));
        lines.add(
                new LanguageLine(
                        goodCode,
                        LanguageCodeConverter.toUnderbarLocale(googleCode),
                        googleCode,
                        cldrName));
        lines.add(
                new LanguageLine(
                        goodCode,
                        LanguageCodeConverter.toHyphenLocale(goodCode),
                        googleCode,
                        cldrName));
        lines.add(
                new LanguageLine(
                        goodCode,
                        LanguageCodeConverter.toHyphenLocale(badCode),
                        googleCode,
                        cldrName));
        lines.add(
                new LanguageLine(
                        goodCode,
                        LanguageCodeConverter.toHyphenLocale(googleCode),
                        googleCode,
                        cldrName));
    }
