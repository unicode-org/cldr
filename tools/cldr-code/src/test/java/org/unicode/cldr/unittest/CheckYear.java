...
public class CheckYear {
    ...
    static Map<String, Relation<String, String>> stock2skeleton2locales =
            new LinkedHashMap<String, Relation<String, String>>();

    static {
        for (String stock : STOCK) {
            stock2skeleton2locales.put(
                    "date-" + stock,
    ...
    static class LocaleInfo {
        ...
        Relation<Category, String> category2base =
                Relation.of(new EnumMap<Category, Set<String>>(Category.class), TreeSet.class);
        // collisions between baseSkeletons
        Map<String, Relation<String, Row.R2<String, String>>> base2BasePatterns2Info =
                new TreeMap<String, Relation<String, Row.R2<String, String>>>();
        ...
        public void recordStock(String localeId, String stock, String dateTimePattern) {
            String skeleton = dtp.getSkeleton(dateTimePattern);
            String base = getBaseSkeleton(skeleton);
            stock2skeleton2locales
                    .get("date-" + stock)
                    .put(skeleton.replace("yyyy", "y"), localeId);
        ...
        public void recordBase(String base, String skeleton, String dateTimePattern) {
            ...
            Relation<String, Row.R2<String, String>> basePatterns2Info =
                    base2BasePatterns2Info.get(coreBase);
            if (basePatterns2Info == null) {
                base2BasePatterns2Info.put(
                        coreBase,
                        basePatterns2Info =
                                Relation.of(
                                        new TreeMap<String, Set<Row.R2<String, String>>>(),
                                        TreeSet.class));
            }
            // adjust the pattern to correspond to the base fields
            // String coreSkeleton = getCoreSkeleton(skeleton);
            String minimizedPattern =
                    replaceFieldTypes(dateTimePattern, coreBase, !coreBase.equals(base));
        ...
        private void recordYearStuff(String skeleton, String dateTimePattern) {
            ...
            if (dateTimePattern.contains("MMM") || dateTimePattern.contains("LLL")) {
                // nothing
            } else if (dateTimePattern.contains("M") || dateTimePattern.contains("L")) {
                monthNumeric = true;
            }
            if (isDigit4) {
                if (monthNumeric) {
        ...
        public String replaceFieldTypes(
                String dateTimePattern, String skeleton, boolean isInterval) {
            if (!isInterval) {
        ...
        public String replaceFieldPartsCompletely(String dateTimePattern, String skeleton) {
            String minimizedPattern = dtp.replaceFieldTypes(dateTimePattern, skeleton);
        ...
        private String getCorePattern(String intervalPattern) {
            ...
            if (!normalized.equals(intervalPattern)) {
                System.out.println("Not normalized: " + intervalPattern + "\t" + normalized);
            }
            if (endFirstPart < 0 || startSecondPart < 0) {
                ...
                if (DEBUG)
                    System.out.println(
                            normalized.substring(0, endFirstPart)
                                    + "$$"
    ...
    public static void main(String[] args) throws IOException {
        ...
        writeConflictingPatterns(sorted, false, "conflicting-patterns-other.txt");
    }

    public static void gatherInfo(Factory factory, String calendarID, Map<String, String> sorted)
            throws IOException {
    ...
    public static void writeYearWidths(Map<String, String> sorted, boolean modern, String filename)
            throws IOException {
        PrintWriter out =
                FileUtilities.openUTF8Writer(CLDRPaths.GEN_DIRECTORY + "datecheck/", filename);
        out.println("Name\tid\t" + Joiner.on("\t").join(Category.values()));
        for (Entry<String, String> entry : sorted.entrySet()) {
            String localeId = entry.getValue();
    ...
    public static void writeConflictingStockItems(boolean modern, String filename)
            throws IOException {
        PrintWriter out;
        System.out.println("\nMismatched Stock items\n");
        out = FileUtilities.openUTF8Writer(CLDRPaths.GEN_DIRECTORY + "datecheck/", filename);
        out.println("Stock\tSkeleton\tLocales");
        for (Entry<String, Relation<String, String>> stockAndSkeleton2locales :
                stock2skeleton2locales.entrySet()) {
            String stock = stockAndSkeleton2locales.getKey();
            for (Entry<String, Set<String>> entry2 :
                    stockAndSkeleton2locales.getValue().keyValuesSet()) {
    ...
    public static void writeConflictingPatterns(
            Map<String, String> sorted, boolean modern, String filename) throws IOException {
        PrintWriter out;
        out = FileUtilities.openUTF8Writer(CLDRPaths.GEN_DIRECTORY + "datecheck/", filename);
        out.println(
                "Language\tId\tMin. Skeleton\tMin Pat1\tskeleton → pattern\tMin Pat2\tskeleton → pattern\tMin Pat3\tskeleton → pattern");
        for (Entry<String, String> entry : sorted.entrySet()) {
            String localeId = entry.getValue();
            if (modern != getPriority(localeId)) {
                continue;
            ...
            for (Entry<String, Relation<String, R2<String, String>>> baseAndBasePatterns2Info :
                    localeInfo.base2BasePatterns2Info.entrySet()) {
                String base = baseAndBasePatterns2Info.getKey();
                Relation<String, R2<String, String>> basePatterns2Info =
                        baseAndBasePatterns2Info.getValue();
                ...
                for (Entry<String, Set<R2<String, String>>> basePatternsAndInfo :
                        basePatterns2Info.keyValuesSet()) {
                    String basePattern = basePatternsAndInfo.getKey();
                    s.append("\t‹" + basePattern + "›:\t\"");
                    boolean first = true;
                    for (R2<String, String> info : basePatternsAndInfo.getValue()) {
                        if (first) {
                            first = false;
                        } else {
                            s.append(";\n");
    ...
    public static boolean getPriority(String localeId) {
        return STANDARD_CODES.getLocaleCoverageLevel(Organization.google.toString(), localeId)
                        == Level.MODERN
                || STANDARD_CODES.getLocaleCoverageLevel(Organization.apple.toString(), localeId)
                        == Level.MODERN
                || STANDARD_CODES.getLocaleCoverageLevel(Organization.ibm.toString(), localeId)
                        == Level.MODERN;
    }
}
