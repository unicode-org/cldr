...
public class LocaleMatcherTest extends TestFmwk {
    ...
    private LocaleMatcher newLocaleMatcher(
            LocalePriorityList lpl, LanguageMatcherData data, double d) {
    ...
    private void assertCloser(String a, String closer, String further) {
        ...
        assertEquals(
                "test " + a + " is closer to " + closer + " than to " + further,
                new ULocale(closer),
                matcher.getBestMatch(a));
    ...
    public void testOverrideData() {
        ...
        LanguageMatcherData localeMatcherData =
        ...
        final LocaleMatcher matcher =
                newLocaleMatcher(
                        LocalePriorityList.add(ULocale.ENGLISH)
                                .add(ULocale.FRENCH)
                                .add(ULocale.UK)
                                .build(),
                        localeMatcherData,
    ...
    public void testRegionalSpecials() {
        ...
        assertEquals(
                "es_MX in {en, en_GB, es, es_419}",
                new ULocale("es_419"),
                matcher.getBestMatch("es_MX"));
        assertEquals(
                "en_AU in {en, en_GB, es, es_419}",
                new ULocale("en_GB"),
                matcher.getBestMatch("en_AU"));
        assertEquals(
                "es_ES in {en, en_GB, es, es_419}",
                new ULocale("es"),
                matcher.getBestMatch("es_ES"));
    }
    ...
    public void testMatch_exact() {
        ...
        assertEquals(1.0, LocaleMatcher.match(ENGLISH_CANADA, ENGLISH_CANADA));
    }
    ...
    public void testMatch_none() {
        ...
        double match = LocaleMatcher.match(new ULocale("ar_MK"), ENGLISH_CANADA);
    ...
    public void testMatch_matchOnMazimized() {
        ...
        assertTrue(
                "und_TW should be closer to zh_Hant ("
                        + matchZhHant
                        + ") than to zh ("
                        + matchZh
                        + ")",
                matchZh < matchZhHant);
        double matchEnHantTw = LocaleMatcher.match(new ULocale("en_Hant_TW"), zhHant);
        assertTrue(
                "zh_Hant should be closer to und_TW ("
                        + matchZhHant
                        + ") than to en_Hant_TW ("
                        + matchEnHantTw
                        + ")",
    ...
    public void testGetBestMatchForList_matchOnMaximized() {
        ...
        assertEquals(
                "Match for ja_Jpan_JP (maximized already)",
                "ja",
                matcher.getBestMatch("ja_Jpan_JP, en-AU").toString());
        if (true) return;
        // ja_JP matches ja on likely subtags, and it's listed first, thus it
        // wins over
        // thus it wins over the second preference en_GB.
        assertEquals(
                "Match for ja_JP, with likely region subtag",
                "ja",
                matcher.getBestMatch("ja_JP, en_US").toString());
        // Check that if the preference is maximized already, it works as well.
        assertEquals(
                "Match for ja_Jpan_JP (maximized already)",
                "ja",
                matcher.getBestMatch("ja_Jpan_JP, en_US").toString());
    }
    ...
    public void testUndefined() {
        ...
        // Note that as of ICU 64, "und" is normalized to ""
    ...
    public void testGetBestMatch_googlePseudoLocales() {
        ...
        final LocaleMatcher matcher = newLocaleMatcher("fr, pt");
    ...
    private void showDistance(LocaleMatcher matcher, String desired, String supported) {
        ...
        double distance =
                matcher.match(
                        desired2,
                        ULocale.addLikelySubtags(desired2),
                        supported2,
                        ULocale.addLikelySubtags(supported2));
    ...
    private void check2(Set<ULocale> sorted) {
        ...
        LocaleMatcher matcher =
                newLocaleMatcher(
                        LocalePriorityList.add(sorted.toArray(new ULocale[sorted.size()])).build());
    ...
    public void testGetBestMatchForList_matchOnMaximized2() {
        ...
        assertEquals(
                "Match for ja-JP, with likely region subtag",
                "ja",
                matcher.getBestMatch("ja-JP, en-GB").toString());
        // Check that if the preference is maximized already, it works as well.
        assertEquals(
                "Match for ja-Jpan-JP (maximized already)",
                "ja",
                matcher.getBestMatch("ja-Jpan-JP, en-GB").toString());
    }
    ...
    public void testGetBestMatchForPortuguese() {
        ...
        // Brazillian user who prefers South American Spanish over European Portuguese as a
        // fallback.
        // The asymmetry between this case and above is because it's "pt_PT" that's missing between
        // the
    ...
    public void testPerf() {
        ...
        final LocaleMatcher matcherLong =
                newLocaleMatcher(
    ...
    private long timeLocaleMatcher(
            String title,
            String desired,
            LocaleMatcher matcher,
            boolean showmessage,
        ...
        if (showmessage)
            logln(
                    title
                            ...
                            + " nanos, "
                            + (comparisonTime > 0
                                    ? (delta * 100 / comparisonTime - 100) + "% longer"
                                    : ""));
        return delta;
    }
}
