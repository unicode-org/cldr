...
public class TestCheckAltOnly extends TestFmwk {
    ...
    private enum TestSublLocaleMode {
        PARENT_PRESENT,
        PARENT_ABSENT,
    }
     ...
     *
     * The values (Gɛ̀ɛ́sì vs Gẹ̀ẹ́sì) are slightly different, as in the actual data;
     * that difference is probably not important for this test. What is important is
     * whether the non-alt path (with value "↑↑↑" in the actual data) is present in
     * the parent locale.
     */
    public void testSubLocale() {
        reallyTestSubLocale(TestSublLocaleMode.PARENT_PRESENT);
        reallyTestSubLocale(TestSublLocaleMode.PARENT_ABSENT);
    }

    private void reallyTestSubLocale(TestSublLocaleMode mode) {
        final String subLocale = "yo_BJ";
        final String parLocale = "yo";
        final String altPath = "//ldml/localeDisplayNames/languages/language[@type=\"en_US\"][@alt=\"short\"]";
        final String nonAltPath = "//ldml/localeDisplayNames/languages/language[@type=\"en_US\"]";
        ...
        if (TestSublLocaleMode.PARENT_PRESENT.equals(mode)) {
