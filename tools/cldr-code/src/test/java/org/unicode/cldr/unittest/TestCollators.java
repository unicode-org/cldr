...
public class TestCollators extends TestFmwk {
    ...
    public void TestAccessByCldrConfig() {
        ...
        for (String test : testSequence) {
            if (last != null && test != null) {
                ...
                assertEquals(
                        last + " < " + test + " — " + Utility.hex(last) + " < " + Utility.hex(test),
                        -1,
                        comp);
            }
            last = test;
    ...
    static class CollatorSource {
        ...
        static M3<String, String, String> localeToTypeToPath =
                ChainedMap.of(
        ...
        static final Pattern TYPE =
                PatternCache.get("//ldml/collations/collation\\[@type=\"([^\"]+)\"\\].*/cr");

        public static Set<String> getAvailableTypes(String locale) {
            CLDRFile cldrFile = cldrFactory.make(locale, false); // don't need resolved
            ...
            for (String path : cldrFile) {
                if (m.reset(path).matches()) {
                    ...
                    if (newOne) {
                        localeToTypeToPath.put(
                                locale, type,
                                path); // hack to get around the fact that some attributes should be
                        // nondistinguishing
                    }
                }
            }
