...
public class TestDisplayAndInputProcessor extends TestFmwk {
    ...
    /** "bad" zawgyi (misencoded) */
    public static final String zawgyi_z_mi =
            "ေမာင္းရီ (နယူးဇီလန္ကၽြန္းရွိ ပင္ရင္းတိုင္းရင္းသားလူမ်ိဳး)";

    /** corrected zawgyi */
    public static final String zawgyi_u_mi =
            "မောင်းရီ (နယူးဇီလန်ကျွန်းရှိ ပင်ရင်းတိုင်းရင်းသားလူမျိုး)";

    public void TestMyanmarZawgyi() {
        // Check that the Zawgyi detector and Zawgyi->Unicode converter ignore zawgyi
        DisplayAndInputProcessor daip = new DisplayAndInputProcessor(info.getCLDRFile("my", false));

        String converted_z_mi = daip.processInput("", zawgyi_z_mi, null);
        assertEquals("Zawgyi isn't converted in DAIP", zawgyi_z_mi, converted_z_mi);

        String converted_u_mi = daip.processInput("", zawgyi_u_mi, null);
        assertEquals(
                "Already-Unicode is left alone converted in DAIP", zawgyi_u_mi, converted_u_mi);

        // Note: Zawgyi converter is no longer in DAIP. See {@link TestCheckForExemplars} for more
        // tests.
    }
