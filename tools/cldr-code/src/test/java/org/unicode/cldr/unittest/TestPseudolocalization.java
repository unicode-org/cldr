...
public class TestPseudolocalization extends TestFmwk {
    ...
    public void testConverter() {
        ...
        assertEquals(
                "Language name is not pseudolocalized",
                "[Ɓéļåŕûšîåñ one two]",
                result.getStringValue(
                        "//ldml/localeDisplayNames/languages/language[@type=\"be\"]"));

        assertEquals(
                "Numeric placeholders should not be pseudolocalized",
                "[Ļåñĝûåĝé∶ {0} one two]",
                result.getStringValue(
                        "//ldml/localeDisplayNames/codePatterns/codePattern[@type=\"language\"]"));

        assertEquals(
                "Exemplar characters are not properly merged",
        ...
        assertEquals(
                "Date and time placeholders should only be bracketed",
                "[h:mm:ss a]",
                result.getStringValue(
        ...
        assertEquals(
                "Only literal part of a placeholder should be accented",
                "[{1} 'åţ' {0} 'one']",
                result.getStringValue(
                        "//ldml/dates/calendars/calendar[@type=\"gregorian\"]/dateTimeFormats/"
