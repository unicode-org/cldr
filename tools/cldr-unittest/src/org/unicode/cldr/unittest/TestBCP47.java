...
public class TestBCP47 extends TestFmwk {
    ...
    public void checkEnglishTranslations(boolean keysVsTypes) {
        // for (Iterator it =
        // testInfo.getEnglish().iterator("//ldml/localeDisplayNames/types");
        // it.hasNext();) {
        // System.out.println(it.next());
        // }
        Relation<String, String> bcp47key_types = testInfo
            .getSupplementalDataInfo().getBcp47Keys();
        Relation<R2<String, String>, String> bcp47keyType_aliases = testInfo
        ...
        for (String key : bcp47key_types.keySet()) {
            ...
            if (keyTranslation == null) {
                if (keysVsTypes)
                    errln("Translation for " + key + ":\t" + keyTranslation);
                keyTranslation = key;
            } else {
                if (keysVsTypes)
                    logln("OK Translation for " + key + ":\t" + keyTranslation);
            }
            if (!keysVsTypes) {
                for (String subtype : bcp47key_types.get(key)) {
                    setToItems(types, subtype,
                        bcp47keyType_aliases.get(Row.of(key, subtype)));
                    ...
                    if (typeTranslation == null) {
                        errln("Translation for " + key + "=" + subtype + ":\t"
                            + keyTranslation + "=" + subtype);
                    } else {
                        logln("OK Translation for " + key + "=" + subtype
                            + ":\t" + typeTranslation);
                    }
                }
            }
