...
public class TestBcp47Transforms extends TestFmwk {
    ...
    public void TestNames() {
        ...
        Relation<String, String> extensionToKeys = suppData.getBcp47Extension2Keys();
        Set<String> keys = extensionToKeys.getAll("t");
        // extension="t" name="m0"
        Relation<String, String> keyToSubtypes = suppData.getBcp47Keys();
        Map<R2<String, String>, String> descriptions = suppData.getBcp47Descriptions();
        for (String key : keys) {
        ...
        for (String file : Arrays.asList(new File(CLDRTransforms.TRANSFORM_DIR).list())) {
            ...
            getIcuRulesFromXmlFile(CLDRTransforms.TRANSFORM_DIR, file, directionInfo);
            if (directionInfo.getVisibility() == Visibility.internal) continue;
        ...
        for (String source : Collections.list(Transliterator.getAvailableSources())) {
            for (String target : Collections.list(Transliterator.getAvailableTargets(source))) {
                for (String variant : Collections.list(Transliterator.getAvailableVariants(source, target))) {
                    if (variant.isEmpty()) variant = null;
                    String name = source + "-" + target + (variant == null ? "" : "/" + variant);
        ...
        for (Entry<String, Set<R2<Type, String>>> entry : MISSING.keyValuesSet()) {
            System.out.println(entry);
        }
    }
    ...
    private String getStandard0(String source, String target, String variant) {
        ...
        return newTarget + "-t-" + newSource + (newMechanism == null ? "" : "-m0-" + newMechanism);
    }
    ...
    private String getStandard(Type type, String source, String id) {
        ...
        if (type == Type.mechanism) {
            ...
            if (source.equals("bgn") || source.equals("ungegn")) return source;
        ...
        try {
            ...
            if (languages.contains(language)) {
                ...
                if (script.isEmpty() || scripts.contains(script.toLowerCase(Locale.ENGLISH))) {
                    String region = ulocale.getCountry();
                    if (region.isEmpty() || regions.contains(region.toLowerCase(Locale.ENGLISH))) {
    ...
    public String getIcuRulesFromXmlFile(String dir, String cldrFileName, ParsedTransformID directionInfo) {
        final MyHandler myHandler = new CLDRTransforms.MyHandler(cldrFileName, directionInfo);
