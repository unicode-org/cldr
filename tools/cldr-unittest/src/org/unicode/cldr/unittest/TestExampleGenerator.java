...
public class TestExampleGenerator extends TestFmwk {
	...
	static final Set<String> OK_TO_MISS_EXAMPLES = new HashSet<String>(
			Arrays.asList(
	...
	public void TestAllPaths() {
		if (logKnownIssue(
				"Cldrbug:6342",
				"In our tests, make sure that either we have an example for every path, or that the path is on an exception list.")) {
			return;
		}
			...
			if (example == null) {
				...
				if (simplified.contains("null")) {
					...
					if (!seen.contains(javaEscapedStarred)) {
						...
						&& !OK_TO_MISS_BACKGROUND.contains(plainStarred)) {
					if (!seen.contains(javaEscapedStarred)) {
	...
	public void TestCompoundUnit() {
		String[][] tests = { { "LONG", "one", "〖❬1 meter❭ per ❬second❭〗" },
				{ "SHORT", "one", "〖❬1 m❭/❬sec❭〗" },
				{ "NARROW", "one", "〖❬1m❭/❬s❭〗" },
	...
	private void checkCompactExampleFor(String localeID, Count many,
		...
		String path = "//ldml/numbers/decimalFormats[@numberSystem=\"latn\"]/decimalFormatLength[@type=\"long\"]"
				+ "/decimalFormat[@type=\"standard\"]/pattern[@type=\"1000000\"][@count=\""
				+ many + "\"]";
