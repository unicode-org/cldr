...
public class TestIdentity extends TestFmwk {
	...
	public void TestIdentityVsFilename() {
		...
		Factory[] factories = {
				testInfo.getFullCldrFactory(),
				testInfo.getExemplarsFactory(),
				testInfo.getCollationFactory(),
		...
		for (Factory factory : factories) { 
			for (String locale : factory.getAvailable()) {
				...
				String fScript = ltp.getScript().length() > 0 ? ltp.getScript()	: "<missing>";
				String fTerritory = ltp.getRegion().length() > 0 ? ltp.getRegion() : "<missing>";
				...
				if (!fLanguage.equals(iLanguage)) {
					errln("Language code for locale \"" + locale
							+ "\" does not match the identity section."
							+ "\n\tLocated in : " + factory.getSourceDirectoryForLocale(locale).getPath()
							+ "\n\tValue in file name is: " + fLanguage
							+ "\n\tValue in identity section is: " + iLanguage);
				}
				String iScript = ltp.getScript().length() > 0 ? ltp.getScript()	: "<missing>";
				if (!fScript.equals(iScript)) {
					errln("Script code for locale \"" + locale
							+ "\" does not match the identity section."
							+ "\n\tLocated in : " + factory.getSourceDirectoryForLocale(locale).getPath()
							+ "\n\tValue in file name is: " + fScript
							+ "\n\tValue in identity section is: " + iScript);
				}
				String iTerritory = ltp.getRegion().length() > 0 ? ltp.getRegion() : "<missing>";
				if (!fTerritory.equals(iTerritory)) {
					errln("Territory code for locale \"" + locale
							+ "\" does not match the identity section."
							+ "\n\tLocated in : " + factory.getSourceDirectoryForLocale(locale).getPath()
							+ "\n\tValue in file name is: " + fTerritory
							+ "\n\tValue in identity section is: " + iTerritory);
				}
				Set<String> iVariants = new HashSet<String>(ltp.getVariants());
				if (!fVariants.equals(iVariants)) {
					errln("Variants for locale \"" + locale
				...
				if (canonicalLocaleID != null) {
					...
					if ( !fLanguage.equals(canonicalLanguage)) {
						errln("Locale \"" + locale
								+ "\" uses a non-canonical language tag: " + fLanguage
								+ "\n\tLocated in : " + factory.getSourceDirectoryForLocale(locale).getPath()
								+ "\n\tCanonical form would be : " + canonicalLanguage);
					}
				}
			}
		}
