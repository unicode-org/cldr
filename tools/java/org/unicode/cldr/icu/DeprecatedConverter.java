...
public class DeprecatedConverter {
  ...
  if (emptyLocaleList != null && emptyLocaleList.size() > 0) {
    for (int i = 0; i < emptyLocaleList.size(); i++) {
      ...
      DeprecatedConverter.writeSimpleLocaleAlias(writer, loc, loc, null,
    ...
    for (Iterator<String> i = aliasMap.keySet().iterator(); i.hasNext();) {
      ...
      if (!fromFiles.containsKey(toFileName + ".xml")) {
        ...
        if (xpath != null) {
          ...
          DeprecatedConverter.writeSimpleLocaleAlias(writer, from, fromLocaleName,
              toLocaleName, null);
        }
      }
    }
      ...
      for (int i = 0; i < validSubs.length; i++) {
        ...
        if (testSub != null) {
          ...
          if (maybeValidAlias.containsKey(aSub)) {
            ...
            DeprecatedConverter.writeSimpleLocaleAlias(writer, from, from,
                aSub, null);
            maybeValidAlias.remove(aSub);
            generatedAliasFiles.put(from, new File(depDir, from + ".xml"));
          }
          DeprecatedConverter.writeSimpleLocaleAlias(writer, aSub, aSub, null,
...
private static void writeSimpleLocaleAlias(ICUWriter writer, String fileName, String fromLocale,
