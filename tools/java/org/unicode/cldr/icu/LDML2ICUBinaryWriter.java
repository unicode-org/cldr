...
public class LDML2ICUBinaryWriter 
{
	 ...
	 * 
	 * @param resTop The top of the resource tree that you would like written to file. This
	 * 			object should be a ICUResourceWriter.ResourceTable.
	 * @param outDir A string pointing to the path of the output directory.
	 * @param outFile The name of the output file. If filename has an extension other than .res 
	...
	public static void writeBinaryFile(ICUResourceWriter.Resource resTop, String outDir, String outFile)
	{
		...
		try
		{
			...
			//this method goes through the tree and looks for a table named CollationElements or Collations, and adds the 
			...
			while(cur != null)
			{
				switch(getSpecialType(cur))
				{
					...
					case SPECIAL_DEPENDENCY:
						addDependency((ICUResourceWriter.ResourceTable)top, (ICUResourceWriter.ResourceProcess)cur, outDir);
						break; 
	...
	private static void addTransliteration(ICUResourceWriter.ResourceTable parent, ICUResourceWriter.ResourceProcess trans)
	{
		...
		//yes, we're using an address comparison below...because they should both be pointing the the same object when we find it.
		if(current != trans)
		{
		...
		while(currentIndex < data.length())
		{
			...
			else if(!quoted)
			{
				...
				if(curChar == '#')
				{
					...
					while(newData.length() > 0 && (isNewLine(newData.charAt(newData.length()-1)) || isUWhiteSpace(newData.charAt(newData.length()-1))))
					{
						if(newData.length() == 1)
						{
							newData = "";
				...
				else if(curChar == '\\') //OR if its an escape char //((UChar)0x005C) - \
				{
                	...
                	if(curChar == 'u' && (data.length() - currentIndex) >= 4)
                	{
                		...
                		if(!isUWhiteSpace(tempChar) && !isPunctuation(tempChar) && !isControl(tempChar) )
                		{
                			...
                			curChar = tempChar;
                			currentIndex += 4; //the 4 numbers...will add one more for the u, already did one for the slash
                			if(temp.length() > 1)
                    		{
			...
			if(cur.hasKey && (cur instanceof ICUResourceWriter.ResourceString))
			{
				...
				if(strElement.name.equals("Sequence"))
				{
					try
					{
						 ...
						 * CollatorWriter was not committed to the ICU4J trunk, it currently lives in the bdrower subdirectory of icu4j in the IBM local cvs 
						 */						
	 ...
	 * 
	 * <p>In the 'en_PK' locale, you may have a Table resource with the key "Version." The Table contains a string
	 * resource with the key "1.31."</p>
	 * <p>If this were the whole of the locale data, the key string would be an encoded version of this:</p>
	 * 
	 * "Version\01.31\0" 
	 * 
	 * <br><br>In UTF-16 encoding, each character will take 2 bytes. 
	 * <br>keyStringOffset for the table object would be 0.
	 * <br>keyStringOffset for the string resource would be = "Version".length() + 2 = 16
	 * 
	 * 
