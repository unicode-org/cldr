...
public class LDML2ICUConverter extends CLDRConverterTool {
  ...
  private boolean writeLikelySubtags = false;
  private boolean writePlurals = false;
  private boolean writeNumberingSystems = false;
  private boolean writeBinary = false;
  private boolean verbose = false;
  private boolean asciiNumbers;

  private String factoryDir = null;
  /**
  ...
  private Document likelySubtagsDoc = null;
  private Document pluralsDoc = null;
  private Document numberingSystemsDoc = null;
  ...
  private CLDRFile.Factory cldrFactory = null;
  private CLDRFile.Factory specialsFactory = null;
  ...
  private void usage() {
    System.out.println("\nUsage: LDML2ICUConverter [OPTIONS] [FILES]\nLDML2ICUConverter [OPTIONS] -w [DIRECTORY] \n"+
            ...
            "-l or --supplemental-only  read " + supplementalDataFile + " file from the given directory and write appropriate files to destination directory\n"+
            "-t or --likely-only        read " + likelySubtagsFile + " file from the given directory and write appropriate files to destination directory\n"+
            "-r or --plurals-only       read " + pluralsFile + " file from the given directory and write appropriate files to destination directory\n"+
            "-z or --metazone-only      read " + metazoneInfoFile + " file from the given directory and write appropriate files to destination directory\n"+
            "-n or --numbers-only       read " + numberingSystemsFile + " file from the given directory and write appropriate files to destination directory\n"+
  ...
  public void processArgs(String[] args) {
    ...
    if(writeSupplemental==true){
      ...
      ICUResourceWriter.Resource res = parseNumberingSystemsFile(numberingSystemsDoc, numberingSystemsFile);
    ...
    for(Node node=root.getFirstChild(); node!=null; node=node.getNextSibling()){
      ...
      if(name.equals(LDMLConstants.SUPPLEMENTAL_DATA) ){
        ...
        res = parseLikelySubtagsInfo(node, xpath);
              ...
              name.equals(LDMLConstants.GENERATION)){
        continue;
      }
      else{
        printError(file,"Encountered unknown element "+ getXPath(node, xpath).toString());
  ...
  private ICUResourceWriter.Resource parseSupplemental(Node root, String file){
    ...
    for(Node node=root.getFirstChild(); node!=null; node=node.getNextSibling()){
      ...
      if(name.equals(LDMLConstants.SUPPLEMENTAL_DATA) ){
        ...
        //    printWarning(file,"The " + supplementalDataFile + " file is marked draft! Not producing ICU file. ");
        //System.exit(-1);
        // return null;
        //}
        node=node.getFirstChild();
  ...
  private ICUResourceWriter.Resource parsePluralsInfo(Node root, StringBuffer xpath){
    ...
    ICUResourceWriter.ResourceTable localesTable =
      new ICUResourceWriter.ResourceTable();
    localesTable.name = LDMLConstants.LOCALES;

    ICUResourceWriter.ResourceTable ruleSetsTable =
      new ICUResourceWriter.ResourceTable();
  ...
  private ICUResourceWriter.Resource parseBundle(InputLocale loc) {
    ...
    if ( ULocale.getScript(localeID).length() > 0 && ULocale.getCountry(localeID).length() == 0 &&
          ! supplementalDataInfo.getDefaultContentLocales().contains(localeID)) {
      ...
      .hasNext();) {
        ...
        if(!LDMLConstants.TYPE.equals(name2)) {
          printError(loc.locale,"Encountered unknown <" + xpath
                  + "> subelement: " + name2 + " while looking for " + LDMLConstants.TYPE);
          System.exit(-1);
        }
        String key = loc.getAttributeValue(xpath, LDMLConstants.KEY);              
        if(!registeredKeys[i].equals(key)) continue;               
  ...
  private ICUResourceWriter.Resource parseListAlt(InputLocale loc, String originalName, String name, String altValue) {
    ICUResourceWriter.ResourceTable table = new ICUResourceWriter.ResourceTable();
    String rootNodeName = name;
    table.name = (String) keyNameMap.get(rootNodeName);
    ICUResourceWriter.Resource current = null;
    ...
    for (Iterator<String> iter = loc.file.iterator(xpath); iter.hasNext();) {
      ...
      if (name.equals(LDMLConstants.ALIAS)) {
        ...
        if (name.equals(LDMLConstants.SINGLE_COUNTRIES)) {
          values = loc.getBasicAttributeValue(apath,
                  LDMLConstants.LIST).split(" ");
        } else {
          String temp = loc.getBasicAttributeValue(apath,
                  LDMLConstants.CHOICE);
          if (temp == null) {
            temp = loc.getBasicAttributeValue(apath,
                    LDMLConstants.TYPE);
            if (temp == null) {
    ...
    for (Iterator<String> iter = loc.file.iterator(xpath); iter.hasNext();) {
      ...
      if (name.equals(LDMLConstants.SPECIAL)) {
        ...
        newres = new ICUResourceWriter.ResourceString(
                ...
                loc.file.getStringValue(localxpath));
      }
  ...
  private ICUResourceWriter.Resource parseMonthsAndDays(InputLocale loc,
    ...
    // if the whole thing is an alias
    if ((current = getAliasResource(loc, xpath)) != null) {
      //System.err.println("AR: was " + current.name + " but we think it should be " + table.name + " - on " + xpath);
    ...
    for (int jj = 0; jj < stuff.length; jj++) {
      ...
      if (name.equals(LDMLConstants.ALIAS)) {
        res = parseAliasResource(loc, xpath);
        //                System.err.println("AR 2: was " + res.name + " but we think it should be " + table.name + " - on " + xpath);
        res.name = table.name;
        return res;
      } else if (name.equals(LDMLConstants.DEFAULT)) {
        res = getDefaultResource(loc, xpath, name);
  ...
  private ICUResourceWriter.Resource parseWidth(InputLocale loc,
    ...
    if (map.size() > 0) {
      for (int i = 0; i < allThings.size(); i++) {
        ...
        if (res.val == null) {
          printError(loc.locale, "Could not get full " + resName
  ...
  private static final Set<String> NUMBER_SYSTEMS = new HashSet<String>(Arrays.asList("arab arabext armn armnlow beng deva ethi fullwide geor grek greklow gujr guru hans hansfin hant hantfin hebr jpan jpanfin knda khmr laoo latn mlym mong mymr orya roman romanlow taml telu thai tibt".split("\\s+")));
  // TODO: update to get from supplemental data:             <variable id="$numberSystem" type="choice">arab arabext armn armnlow beng deva ethi fullwide geor grek greklow gujr guru hans hansfin hant hantfin hebr jpan jpanfin knda khmr laoo latn mlym mong mymr orya roman romanlow taml telu thai tibt</variable>
  ...
  private ICUResourceWriter.Resource parseUnits(InputLocale loc,
          String xpath, String tableName, String altValue) {
    ICUResourceWriter.ResourceTable unitsTable =
      new ICUResourceWriter.ResourceTable();
    unitsTable.name = tableName;
    ICUResourceWriter.Resource current = null, first = null;

    String origXpath = xpath;
    String names[] = { LDMLConstants.ALIAS, LDMLConstants.UNIT,
    ...
    for (String name : names) {
      ...
      if (name.equals(LDMLConstants.ALIAS)) {
        ...
        res.name = name;
        return res;
      } else if (name.equals(LDMLConstants.UNIT)) {
  ...
  private ICUResourceWriter.Resource parseUnit(InputLocale loc,
          String xpath, String altValue) {
    Map<String, ICUResourceWriter.ResourceTable> tableMap =
      new HashMap<String, ICUResourceWriter.ResourceTable>();
    ...
    for (Iterator<String> iter = loc.file.iterator(xpath); iter.hasNext();) {
      ...
      else if (name.equals(LDMLConstants.UNIT_PATTERN)) {
  ...
  private GroupStatus parseGroupWithFallback(InputLocale loc,
          ...
          ICUResourceWriter.ResourceString res[], boolean preferNativeNumberSymbols) {
  ...
  private String reviseAPath(InputLocale loc, XPathParts xpp, String aPath) {
    ...
    // We have all the paths that match, now. We prefer ones that have an alt value that is a valid number system
    for (String path : paths) {
      String distinguishing = loc.file.getDistinguishingXPath(path, null, false);
      if (distinguishing.contains("@alt")) {
        String alt = xpp.set(distinguishing).getAttributeValue(-1, "alt");
  ...
  private ICUResourceWriter.Resource parseCurrencyPlurals(InputLocale loc,
          String xpath) {
  ...
  private ICUResourceWriter.Resource parseCurrencyFormatPlurals(InputLocale loc,
          String xpath) {
  ...
  public ICUResourceWriter.Resource parseCollations(InputLocale loc, String xpath) {   
    if(loc.notOnDisk) {
      ...
      ICUResourceWriter.Resource first = null;
      if ((first = getAliasResource(loc, xpath)) != null) {
        first.name =  LDMLConstants.COLLATIONS;
        return first;
      }

      // now, look for a default
      if ((first = getDefaultResource(loc, xpath+"/"+LDMLConstants.DEFAULT, LDMLConstants.DEFAULT))!=null) {
        ICUResourceWriter.ResourceTable table = new ICUResourceWriter.ResourceTable();
      ...
      for(String type : collTypes) {
        ICUResourceWriter.Resource res = getAliasResource(loc, xpath+"/"+LDMLConstants.COLLATION+"[@type=\""+type+"\"]");
        if(res != null) {
          first = ICUResourceWriter.Resource.addAfter(first, res);
        } else {
          throw new InternalError("FAIL: locale " + loc.locale + " not on disc, and non-alias collation " + type +" encountered.");
        }
      }
  ...
  private ICUResourceWriter.Resource parseBrkItrData(Node root,
    ...
    .getNextSibling()) {
      ...
      if (name.equals(ICU_BOUNDARIES)) {
        ...
        System.err.println("Encountered @ " + xpath + "  unknown <" + root.getNodeName()
                + "> subelement: " + name);
        System.exit(-1);
      }
      if (res != null) {
    ...
    if (xpath != null) {
      ...
      if (res != null
        ...
        System.err.println("Failed to write out alias bundle " // parse
                // error?
                + fromLocale.toString() + " from " + xpath + " - XML list follows:");
        fakeFile.write(new PrintWriter(System.out));
      }
