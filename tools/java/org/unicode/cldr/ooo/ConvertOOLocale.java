...
public class ConvertOOLocale
{
    ...
    private static final String RES_REFS = "-res_refs"; // resolve all refs fully, otherwise only resolve those for non
                                                        // OO.0 specific element categories
    ...
    private void printUsage()
    {
        ...
        System.err
            .println("  ConvertOOLocalw [-single inFile] | [-bulk inDir]  [-date_time] [-res_refs] [-dest_dir] [-dtd_dir] [-cldr_ver]");
        System.err.println("");
        ...
        System.err.println("  " + DEST_DIR
            + "  : the dir where output is written (it will be created if non existant) (dafault=./main) ");
        System.err.println("  " + DTD_DIR + "   : the location of the dtds (ldml.dtd and ldmlOpenOffice.dtd)");
        System.err
            .println("  "
                + CLDR_VER
                + "  : the CLDR version (default =1.3) this is used to determine the URL of the DTD if -dtd_dir not specified");
        System.err.println("  " + CLDR_ONLY
            + " : only write data that can be mapped to CLDR. Used for importing new locales into CLDR.");
        System.err.println("");
        System.err.println("EXAMPLES:");
        System.err.println("  ConvertOOLocale -single OOFileName.xml");
        System.err
            .println("    will create the LDML file in the \"main\" subfolder (\"main\" will be created if it doesn't exist)");
        System.err.println("  ConvertOOLocale -bulk OODirName  ");
        System.err
            .println("    will create the LDML files in the \"main\" subfolder (\"main\" will be created if it doesn't exist)");
        System.err.println("");
    }
    ...
    private void processSingle()
    {
        ...
        String ns = null;
        if (m_bWriteCLDROnly == false) ns = XMLNamespace.OPEN_OFFICE; // don't write OO.o dtd line if importing new
                                                                      // locale to CLDR
        ...
        // ###### write identity ######
        data.put(LDMLConstants.VERSION + " " + LDMLConstants.NUMBER, "$Revision$"); // this is the CVS version
                                                                                           // not the CLDR version
        // data.put(LDMLConstants.GENERATION, "Generated from ConvertOOLocale"); //comment is superflous and would need
        // to be removed for importing new locales inot CLDR
        ...
        if (reader.m_Language_DefaultName != null)
            data.put(OpenOfficeLDMLConstants.DEFAULT_NAME + " " + reader.m_LangId, reader.m_Language_DefaultName);
        if (reader.m_Country_CountryID != null)
            data.put(LDMLConstants.TERRITORY + " " + LDMLConstants.TYPE, reader.m_Country_CountryID);
        if (reader.m_Country_DefaultName != null)
            data.put(OpenOfficeLDMLConstants.DEFAULT_NAME + " " + reader.m_Country_CountryID,
                reader.m_Country_DefaultName);
        ...
        if (Float.parseFloat(m_cldr_ver) > 1.3999)
        {
            ...
            if (abbrQuarters != null)
                data.put(LDMLConstants.QUARTER_WIDTH + " " + LDMLConstants.ABBREVIATED, abbrQuarters);
        ...
        if (OutlineNumberingLevels != null)
            data.put(OpenOfficeLDMLConstants.OUTLUNE_NUMBERING_LEVELS, OutlineNumberingLevels);
        ...
        if (reader.m_FollowPageWord != null)
            data.put(OpenOfficeLDMLConstants.FOLLOW_PAGE_WORD, reader.m_FollowPageWord);
        ...
        if (reader.m_CollationOptions != null)
            data.put(OpenOfficeLDMLConstants.COLLATION_OPTIONS, reader.m_CollationOptions);
