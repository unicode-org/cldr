...
public class OOComparator
{
    ...
    private class TableInfo // holds optional details to go in talble headers
    {
        ...
        public String url; // url to link to from talbe header like
                           // http://l10n.openoffice.org/nonav/source/browse/*checkout*/l10n/i18npool/source/localedata/data/zu_ZA.xml?rev=SRC680_m151
    ...
    private static final int OPT_UNRESOLVED_REFS = 0x004; // means refs are not resolved in conversion to LDML so don't
                                                          // compare "ref" attributes
    ...
    private Hashtable m_DiffsPerLocale = new Hashtable(); // key = # conflicts, value = # of locales with this number of
                                                          // conflicts
    ...
    private void printUsage()
    {
        ...
        System.err
            .println("  Compares 2 or more locale files or directories of locale files and writes the differences ");
        ...
        System.err.println("  " + OPT_SINGLE_STR
            + "    : specifies 2 or more locales in OpenOffice.org format to be compared");
        System.err.println("  " + OPT_BULK_STR
            + "      : specifies 2 or more dirs containing locales in OpenOffice.org format to be compared");
        ...
        System.err
            .println("  OOComparator -single dir1/it_IT.xml;OOo;http://l10n.openoffice.org/location/it_IT.xml?rev=SRC680_m151 dir2/it_IT.xml dir3/it_IT.xml ....");
        System.err
            .println("    write the differences to it_IT.html adding the header \"OOo\" and link http://l10n.openoffice.org/location/he_IL.xml?rev=SRC680_m151 to the table");
        System.err
            .println("  OOComparator -bulk dir1;OOo;http://l10n.openoffice.org/location/LOCALE?rev=SRC680_m151 dir2;CLDR ....");
                ...
                if (options[k].compareTo(OPT_SINGLE_STR) == 0)
                {
                    ...
                    for (int i = k + 1; i < options.length; i++)
                    {
                        ...
                        else
                        {
                            ...
                            m_TableInfo.add(ti);
                            // System.err.println(Integer.toString(m_XMLfiles.size()-1) + "  " + parts[1] + "  " +
                            // parts[2]);
                        }
                    }
                }
    ...
    private void printHTMLEnd(PrintWriter writer)
    {
        ...
        writer.print("          <p><b>Total Number of locales audited : " + m_TotalNumLocales + "</b></p>" +
            ...
            "           <p><b>Total Number of conflicting locale data across all locales : " + m_DiffCounter + " ("
            + percent + " %)" + "</b></p>" +
            "    </body>\n" +
            "</html>\n");
        writer.flush();
    ...
    private void printTableStart(PrintWriter writer, Vector XMLfiles, Locale locale)
    {
        ...
        String displayName = "<a name=\"" + localeStr + "\" href=\"#" + localeStr + "\">" + localeStr + "</a>" + " ("
            + displayLang + (dispCountry.equals("") == false ? ("_" + dispCountry) : "") + ")";
        ...
        if (XMLfiles.size() < 2)
        {
            printWarning(XMLfiles.size() + " file was specified for comparison for " + locale.toString()
    ...
    private void doBulkComparison(PrintWriter writer)
    {
        ...
        TreeMap locales = new TreeMap(); // TreeMap isorte in ascending order key=locale String , value = LocaleFiles
                                         // inst
        ...
        while (enDirs.hasMoreElements())
        {
            ...
            { // create a LocaleFiles inst for each locale and add to inst all fully qualified filenames belonging to
              // that locale
    ...
    private void compareData(PrintWriter writer, Vector XMLfiles, Locale locale)
    {
        ...
        // LC_INFO
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.LOCALE, OOConstants.LOCALE, readers,
            "m_LocaleInfo");
        cntDiff = compareString(writer, cntDiff, OOConstants.LOCALE, OOConstants.VERSION_DTD, "", readers,
            "m_versionDTD");
        cntDiff = compareString(writer, cntDiff, OOConstants.LOCALE, OOConstants.ALLOW_UPDATE_FROM_CLDR, "", readers,
            "m_allowUpdateFromCLDR");
        ...
        cntDiff = compareString(writer, cntDiff, OOConstants.LANGUAGE, OOConstants.DEFAULT_NAME, "", readers,
            "m_Language_DefaultName");
        cntDiff = compareString(writer, cntDiff, OOConstants.COUNTRY, OOConstants.COUNTRY_ID, "", readers,
            "m_Country_CountryID");
        cntDiff = compareString(writer, cntDiff, OOConstants.COUNTRY, OOConstants.DEFAULT_NAME, "", readers,
            "m_Country_DefaultName");
        cntDiff = compareString(writer, cntDiff, OOConstants.PLATFORM, OOConstants.PLATFORM_ID, "", readers,
            "m_PlatformID");
        cntDiff = compareString(writer, cntDiff, OOConstants.LC_INFO, OOConstants.VARIANT, "", readers, "m_Varient");

        // LC_TYPE
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.LC_CTYPE, OOConstants.SEPARATORS, readers,
            "m_Separators");
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.LC_CTYPE, OOConstants.MARKERS, readers,
            "m_Markers");
        ...
        cntDiff = compareString(writer, cntDiff, OOConstants.LC_CTYPE, OOConstants.MEASUREMENT_SYSTEM, "", readers,
            "m_MeasurementSystem");

        // LC_FORMAT
        cntDiff = compareString(writer, cntDiff, OOConstants.LC_FORMAT, OOConstants.REPLACE_FROM_SMALL, "", readers,
            "m_ReplaceFrom");
        cntDiff = compareString(writer, cntDiff, OOConstants.LC_FORMAT, OOConstants.REPLACE_TO_SMALL, "", readers,
            "m_ReplaceTo");
        cntDiff = compareUniqueElemAtts(writer, cntDiff, OOConstants.FORMAT_ELEMENT, readers, "m_FormatElements");
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.LC_FORMAT, OOConstants.FORMAT_CODE, readers,
            "m_FormatCodes");
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.LC_FORMAT, OOConstants.FORMAT_CODE, readers,
            "m_FormatDefaultNames");

        // LC_COLLATION
        cntDiff = compareUniqueElemAtts(writer, cntDiff, OOConstants.COLLATOR, readers, "m_Collators",
            OOConstants.UNOID);
        cntDiff = compareVectorofStrings(writer, cntDiff, OOConstants.COLLATION_OPTIONS,
            OOConstants.TRANSLITERATION_MODULES, readers, "m_CollationOptions");

        // LC_SEARCH
        cntDiff = compareVectorofStrings(writer, cntDiff, OOConstants.SEARCH_OPTIONS,
            OOConstants.TRANSLITERATION_MODULES, readers, "m_SearchOptions");

        // LC_INDEX
        cntDiff = compareUniqueElemAtts(writer, cntDiff, OOConstants.INDEX_KEY, readers, "m_IndexKeys",
            OOConstants.UNOID);
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.LC_INDEX, OOConstants.INDEX_KEY, readers,
            "m_IndexKeysData");
        cntDiff = compareVectorofStrings(writer, cntDiff, OOConstants.LC_INDEX, OOConstants.UNICODE_SCRIPT, readers,
            "m_UnicodeScript");
        cntDiff = compareVectorofStrings(writer, cntDiff, OOConstants.LC_INDEX, OOConstants.FOLLOW_PAGE_WORD, readers,
            "m_FollowPageWord");

        // LC_CALENDAR
        cntDiff = compareUniqueElemAtts(writer, cntDiff, OOConstants.CALENDAR, readers, "m_Calendars",
            OOConstants.UNOID);
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.START_DAY_OF_WEEK, OOConstants.DAY_ID,
            readers, "m_StartDayOfWeek");
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.CALENDAR,
            OOConstants.MINIMAL_DAYS_IN_FIRST_WEEK, readers, "m_MinDaysInFirstweek");
        cntDiff = compareHashtableOfHashtablesOfStrings(writer, cntDiff, OOConstants.DAYS_OF_WEEK + "."
            + OOConstants.DAY + "." + OOConstants.DAY_ID, OOConstants.DEFAULT_ABBRV_NAME, readers, "m_AbbrDays");
        cntDiff = compareHashtableOfHashtablesOfStrings(writer, cntDiff, OOConstants.DAYS_OF_WEEK + "."
            + OOConstants.DAY + "." + OOConstants.DAY_ID, OOConstants.DEFAULT_FULL_NAME, readers, "m_WideDays");
        cntDiff = compareHashtableOfHashtablesOfStrings(writer, cntDiff, OOConstants.MONTHS_OF_YEAR + "."
            + OOConstants.MONTH + "." + OOConstants.MONTH_ID, OOConstants.DEFAULT_ABBRV_NAME, readers, "m_AbbrMonths");
        cntDiff = compareHashtableOfHashtablesOfStrings(writer, cntDiff, OOConstants.MONTHS_OF_YEAR + "."
            + OOConstants.MONTH + "." + OOConstants.MONTH_ID, OOConstants.DEFAULT_FULL_NAME, readers, "m_WideMonths");
        cntDiff = compareHashtableOfHashtablesOfStrings(writer, cntDiff, OOConstants.ERAS + "." + OOConstants.ERA + "."
            + OOConstants.ERA_ID, OOConstants.DEFAULT_ABBRV_NAME, readers, "m_AbbrEras");
        cntDiff = compareHashtableOfHashtablesOfStrings(writer, cntDiff, OOConstants.ERAS + "." + OOConstants.ERA + "."
            + OOConstants.ERA_ID, OOConstants.DEFAULT_FULL_NAME, readers, "m_WideEras");

        // LC_CURRENCY
        cntDiff = compareCurrencies(writer, cntDiff, OOConstants.CURRENCY, readers);

        cntDiff = compareUniqueElemAtts(writer, cntDiff, OOConstants.TRANSLITERATION, readers, "m_Transliterations",
            OOConstants.UNOID);

        // LC_MISC
        cntDiff = compareString(writer, cntDiff, OOConstants.FORBIDDEN_CHARACTERS,
            OOConstants.FORBIDDEN_LINE_BEGIN_CHARACTERS, "", readers, "m_ForbiddenBeginChar");
        cntDiff = compareString(writer, cntDiff, OOConstants.FORBIDDEN_CHARACTERS,
            OOConstants.FORBIDDEN_LINE_END_CHARACTERS, "", readers, "m_ForbiddenEndChar");
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.LC_MISC, OOConstants.RESERVED_WORDS, readers,
            "m_ReservedWords");

        // LC_NumberingLevel
        cntDiff = compareVectorofHashtables(writer, cntDiff, OOConstants.LC_NUMBERING_LEVEL,
            OOConstants.NUMBERING_LEVEL, readers, "m_NumberingLevels");
        cntDiff = compareVectorofHashtables(writer, cntDiff, OOConstants.OUTLINE_STYLE,
            OOConstants.OUTLUNE_NUMBERING_LEVEL, readers, "m_OutlineNumberingLevels");
    ...
    private int compareString(PrintWriter writer, int cntDiff, String parent, String attribute, String id,
        OOLocaleReader[] readers, String OOLocaleReaderVarName)
    {
    ...
    private int compareVectorofStrings(PrintWriter writer, int cntDiff, String parentName, String elementName,
        OOLocaleReader[] readers, String OOLocaleReaderVarName)
    {
    ...
    private int compareRefs(PrintWriter writer, int cntDiff, OOLocaleReader[] readers, String OOLocaleReaderVarName)
    {
        ...
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.CALENDAR, OOConstants.DAYS_OF_WEEK + " _"
            + OOConstants.REF, daysOfWeek);
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.CALENDAR, OOConstants.MONTHS_OF_YEAR + " _"
            + OOConstants.REF, monthsOfYear);
        cntDiff = compareHashtableofStrings(writer, cntDiff, OOConstants.CALENDAR, OOConstants.ERAS + " _"
            + OOConstants.REF, eras);

        return cntDiff;
    }

    private int compareHashtableOfHashtablesOfStrings(PrintWriter writer, int cntDiff, String parent, String element,
        OOLocaleReader[] readers, String OOLocaleReaderVarName)
    {
    ...
    private int compareHashtableofStrings(PrintWriter writer, int cntDiff, String parent, String element,
        OOLocaleReader[] readers, String OOLocaleReaderVarName)
    {
    ...
    private int compareHashtables(PrintWriter writer, int cntDiff, Vector keys, Hashtable[] hashtables, String parent,
        String element)
    {
    ...
    private int compareUniqueElemAtts(PrintWriter writer, int cntDiff, String parent, OOLocaleReader[] readers,
        String OOLocaleReaderVarName, String uniqueAttName)
    {
        Hashtable[] hashtables = new Hashtable[m_CurrNumFiles];
    ...
    private int compareAttValues(PrintWriter writer, int cntDiff, String parent, Hashtable[] hashtables, Vector keys,
        Vector attributes)
    {
    ...
    private int compareVectorofHashtables(PrintWriter writer, int cntDiff, String parentName, String elementName,
        OOLocaleReader[] readers, String OOLocaleReaderVarName)
    {
    ...
    private int compareCurrencies(PrintWriter writer, int cntDiff, String parent, OOLocaleReader[] readers)
    {
        ...
        for (int i = 0; i < m_CurrNumFiles; i++)
        {
            // m_CurrencyData is a vector of vectors, find out which OO file ahs the most currencies defined, should be
            // the same
            if (readers[i].m_CurrencyData.size() > max)
                max = readers[i].m_CurrencyData.size();
        }

        // inner vector holds data in following order : CurrencyID,symbol,code,name, blank ,default,
        // usedInCompatibleFormatCode, legacyOnly (if defined)
        String[] values = new String[m_CurrNumFiles];
        for (int j = 0; j < max; j++)
    ...
    private void printSummaryPart1(int diffs, int counter, Locale locale)
    {
        ...
        try
        {
            ...
            out.write(Integer.toString(counter) + "  " + localeToString(locale) + "  Conflicts = "
                + Integer.toString(diffs) + "\n");
            out.close();
        } catch (IOException e)
        {
        }
