...
public class supplementalData
{
    ...
    // Hashtable of vectors , key = iso3166 region code, vector[0] = default currency, vector[1] - vecotr[n] =
    // deprecated currencies starting with msot recent
    private Hashtable m_currencuiesInRegion = new Hashtable();

    // key = number of days, value = string of 2 letter iso country codes i.e. <"1", "GB IE US IT"> territory=001 => all
    // otheers
    ...
    // key = metric or US, value = string of 2 letter iso country codes i.e. <"1", "GB IE US IT"> territory=001 => all
    // otheers
    ...
    private void readCurrencies()
    {
        ...
        for (int i = 0; i < nl.getLength(); i++)
        {
            ...
            for (int j = 0; j < nl2.getLength(); j++)
            {
                ...
                String from = LDMLUtilities.getAttributeValue(nl2.item(j), LDMLConstants.FROM); // YYYY-MM-DD or just
                                                                                                // YYYY
