...
public class CheckExemplars extends FactoryCheckCLDR {
    ...
    private static final UnicodeSet SPECIAL_ALLOW = new UnicodeSet("[\\u200E\\u200F\\u200c\\u200d" +
    		...
    		"‎‎‎[:word_break=Katakana:][:word_break=ALetter:][:word_break=MidLetter:] ]" // restore [:word_break=Katakana:][:word_break=ALetter:][:word_break=MidLetter:]
            ).freeze(); // add RLM, LRM [\u200C\u200D]‎
    ...
    .removeAll(AlwaysOK) // this will remove some [:word_break=Katakana:][:word_break=ALetter:][:word_break=MidLetter:] so we restore them in SPECIAL_ALLOW
