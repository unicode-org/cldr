...
public class GenerateDayPeriodChart {
    ...
    static {
        ...
        dayPeriodBuilder.add(DayPeriod.night, 0, false, HOUR*2, false);
        dayPeriodBuilder.add(DayPeriod.morning, HOUR*2, true, HOUR*12, false);
        dayPeriodBuilder.add(DayPeriod.noon, HOUR*12, true, HOUR*12, true);
        dayPeriodBuilder.add(DayPeriod.afternoon, HOUR*12, false, HOUR*18, false);
        dayPeriodBuilder.add(DayPeriod.evening, HOUR*18, true, HOUR*21, false);
        dayPeriodBuilder.add(DayPeriod.night, HOUR*21, true, HOUR*24, false);
    ...
    public static void main(String[] args) {
        ...
        for (String locale : SUP.getDayPeriodLocales()) {
            DayPeriodInfo info = getFixedDayPeriodInfo(locale);
            if (info == null) {
                continue;
            }
        ...
        for (String locale : SC.getLocaleCoverageLocales("google", EnumSet.of(Level.MODERN))) {
            System.out.print(locale + "\t" + ENGLISH.getName(locale));
            DayPeriodInfo dayPeriod = getFixedDayPeriodInfo(locale);
            doRow(dayPeriod);
        }
    ...
    public static DayPeriodInfo getFixedDayPeriodInfo(String locale) {
        DayPeriodInfo result = locale.equals("en") || locale.startsWith("en_") 
            ? ENGLISH_DAY_INFO 
                : SUP.getDayPeriods(locale);
