...
public class TestStringByteConverter {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("el rand utf8/comp: " + totalUtf8Bytes + "/" + totalBytes + " = "
    ...
    private static void testWithLocale(String locale, Type type) throws IOException {
        ...
        System.out.println(locale + "\t" + type + "\tutf8/comp: " + totalUtf8Bytes + "/" + totalBytes + " = "
            + (totalUtf8Bytes / (double) totalBytes));
    }
        ...
        for (int i = 0; i < test.length(); ++i) {
            ...
            for (int j = 0; j < byteCount; ++j) {
                if (bytes2[j] != bytes[lastPosition + j]) {
                    throw new IllegalArgumentException("Fails incremental: <" + Utility.hex(test) + ">, <"
                        + (lastPosition + j) + ">, <" + bytes[lastPosition + j] + ">, <" + bytes2[j] + ">");
                }
            }
    ...
    private static void checkUtf8(String test) throws IOException {
        ...
        for (int i = 0; i < len; ++i) {
            if (output1[i] != output2[i]) {
                ...
                System.out.println(Utility.hex(test) + "\t" + i + "\t" + Utility.hex(output1[i] & 0xFF, 2) + "\t"
    ...
    private static void testInt(int test, boolean unsigned) throws UnsupportedEncodingException {
        byte[] bytes = new byte[1000];
        int[] ioBytePosition = new int[1];
        int len = unsigned ? CompactStringByteConverter.writeUnsignedInt(test, bytes, 0) : CompactStringByteConverter
            .writeInt(test, bytes, 0);
        if (CompactStringByteConverter.DEBUG) {
            System.out.println("\tcm" + (unsigned ? "u" : "s") + ": " + hex(bytes, 0, len, " "));
        }
        ioBytePosition[0] = 0;
        int retest = unsigned ? CompactStringByteConverter.readUnsignedInt(bytes, ioBytePosition)
