 ...
 * hasCharAt instead.
 * <ul>
 * <li>To force the source to grow to the end, use
 * hasCharAt(Integer.MAX_VALUE).
 * <li>The method discardBefore can be called to release the source before a
 * given offset. Once it is called, then any attempt to access data or offsets
 * before that point will cause an exception.
 * <li> To see 
 * </ul>
 * 
 * @author markdavis
...
public interface CharList {
   ...
   * <li>replace index >= x.length() with !x.hasCharAt(index)
   * <li>replace index == x.length() also with !x.hasCharAt(index) <b>
   * [whenever we are iterating index upwards]</b>
   * </ul>
  ...
  /**
   * Get the index from the source offset; the reverse of the toSourceOffset method. It will not grow the source, however.
   * 
   * @param index
   * @return
   */
