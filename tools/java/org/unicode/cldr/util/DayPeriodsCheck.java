...
public class DayPeriodsCheck {
    ...
    private static void show(ULocale locale, int start, int limit, DayPeriods dayPeriods, DayPeriod dayPeriod) {
        if (html) {
            System.out.println("<tr><td>" + format(start) + ":00 – " + format(limit) + ":59</td><td>" + dayPeriod + "</td><td>" + dayPeriods.getSample(dayPeriod) + "</td></tr>");
        } else {
            System.out.println("||" + LanguageGroup.get(locale) + "||" + locale.getDisplayName(ULocale.ENGLISH) + "||" + locale + "||" + dayPeriod + "||" + start + ":00 – " + limit + ":59||" + dayPeriods.getSample(dayPeriod) + "||");
        }
    }
